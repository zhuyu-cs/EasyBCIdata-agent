...
from easybci_cli.timeouts import (
    get_provider_request_timeout,
    get_provider_stale_timeout,
)

_easybci_home = get_easybci_home()
_project_env = Path(__file__).parent / '.env'
_loaded_env_paths = load_easybci_dotenv(easybci_home=_easybci_home, project_env=_project_env)
...
from easybci_lib.model_tools import (
    get_tool_definitions,
    get_toolset_for_tool,
    handle_function_call,
    check_toolset_requirements,
)
from easybci_lib.tools.terminal_tool import cleanup_vm, get_active_env, is_persistent_env
from easybci_lib.tools.terminal_tool import (
    set_approval_callback as _set_approval_callback,
    set_sudo_password_callback as _set_sudo_password_callback,
    _get_approval_callback,
    _get_sudo_password_callback,
)
from easybci_lib.tools.tool_result_storage import maybe_persist_tool_result, enforce_turn_budget
from easybci_lib.tools.interrupt import set_interrupt as _set_interrupt
from easybci_lib.tools.neural_processing.progress.context import set_active_tracker
from easybci_lib.tools.neural_processing.progress.heartbeat import HeartbeatDaemon
from easybci_lib.tools.neural_processing.progress.tracker import StagedProgressTracker


# Agent internals extracted to agent/ package for modularity
from easybci_agent.memory_manager import StreamingContextScrubber, build_memory_context_block, sanitize_context
from easybci_agent.think_scrubber import StreamingThinkScrubber
from easybci_agent.retry_utils import jittered_backoff
from easybci_agent.error_classifier import classify_api_error, FailoverReason
...
from easybci_agent.subdirectory_hints import SubdirectoryHintTracker
from easybci_agent.prompt_caching import apply_anthropic_cache_control
from easybci_agent.prompt_builder import build_skills_system_prompt, build_context_files_prompt, build_environment_hints, load_soul_md, TOOL_USE_ENFORCEMENT_GUIDANCE, TOOL_USE_ENFORCEMENT_MODELS, GOOGLE_MODEL_OPERATIONAL_GUIDANCE, OPENAI_MODEL_EXECUTION_GUIDANCE
from easybci_agent.usage_pricing import estimate_usage_cost, normalize_usage
from easybci_agent.responses_api_adapter import (
    _derive_responses_function_call_id as _responses_derive_function_call_id,
    _deterministic_call_id as _responses_deterministic_call_id,
    _split_responses_tool_id as _responses_split_tool_id,
    _summarize_user_message_for_log,
)
from easybci_agent.display import (
    KawaiiSpinner, build_tool_preview as _build_tool_preview,
    get_cute_tool_message as _get_cute_tool_message_impl,
    _detect_tool_failure,
    get_tool_emoji as _get_tool_emoji,
)
from easybci_agent.tool_guardrails import (
...
from easybci_agent.tool_result_classification import (
    FILE_MUTATING_TOOL_NAMES as _FILE_MUTATING_TOOLS,
    file_mutation_result_landed,
)
from easybci_agent.trajectory import (
    convert_scratchpad_to_think, has_incomplete_scratchpad,
    save_trajectory as _save_trajectory_to_file,
)
from easybci_agent.iteration_budget import IterationBudget
from easybci_agent.stream_filter import strip_reasoning_tags as _strip_reasoning_tags_impl
from easybci_agent.system_prompt import SystemPromptBuilder
from easybci_agent.parallelism import (
...
from easybci_agent.sanitization import (
    _SURROGATE_RE,
...
from easybci_agent.tool_permissions import ToolPermissionController
from easybci_lib.utils import atomic_json_write, base_url_host_matches, base_url_hostname, env_var_enabled, normalize_proxy_url
...
class _SafeWriter:
    ...
    When easybci-agent runs as a systemd service, Docker container, or headless
...
def _is_multimodal_tool_result(value: Any) -> bool:
    ...
    Multimodal handlers (e.g. vision tools) return a dict with
...
# _summarize_user_message_for_log is imported from easybci_agent.responses_api_adapter
...
def _routermint_headers() -> dict:
    ...
    return {
        "User-Agent": f"EasybciAgent/{_EASYBCI_VERSION}",
    }
...
# =============================================================================
# Neural preprocessing contract — keyword-triggered system-prompt prepend
# =============================================================================
#
# When the user's first message looks like a neural preprocessing request
# (Chinese / English verbs OR a neural-data file extension), we append a
# short 14-step contract to the system prompt so the agent treats the task
# as a structured workflow instead of a freestyle scripting job. The
# runtime path guard (skill_compliance_guard) is the hard wall; this
# prepend is the soft signpost that the wall is there and explains the
# legal route.
#
# Activation is sticky on the agent instance — once a session is flagged,
# the contract remains in the cached system prompt across all turns and
# compression rebuilds.

_PREPROCESSING_KEYWORDS: tuple[str, ...] = (
    # Chinese verbs
    "预处理", "处理这部分数据", "清洗数据", "数据预处理", "做预处理",
    # English verbs
    "preprocess", "preprocessing", "clean my eeg", "clean my meg",
    "run pipeline", "process eeg", "process meg", "process seeg",
    # Neural data file extensions (strong signal — user supplied a data path)
    ".cdt", ".edf", ".bdf", ".set", ".vhdr", ".eeg", ".vmrk",
    ".fif", ".xdf", ".nwb", ".nev", ".ns5", ".ns3", ".ns2",
    ".mefd", ".snirf", ".ncs", ".nse", ".plx", ".pl2",
    ".smr", ".smrx", ".rhd", ".rhs", ".continuous",
)


_PREPROCESSING_CONTRACT_PREPEND: str = """
[NEURAL PREPROCESSING CONTRACT — active because the user asked for data preprocessing]

You MUST follow the pipeline skill's 14-step flow:
1. inspect_data → 2. infer analysis_goal → 3. deep_inspect → 4. proven-pipeline reuse check →
5. plan → 6. propose_pipeline (stages middle_process/proposal.staged.json; plan/ NOT written yet) →
7. CONFIRM with user, then mark_proposal_confirmed (materializes plan/ + writes proposal.confirmed) →
8. generate_code (writes code/pipeline.py + qc.py + run.py + requirements.txt) →
9. preprocess_neural (runs code/pipeline.py) → 10. save_processed (runs build_ai_ready.py) →
11. quality_check (runs code/qc.py) → 12. export_repo (assembles the final mini-repo) →
13. skill_manage (save proven-pipeline) → 14. cleanup middle_process/.

## Tool invocation — these are function-calling tools, not Python modules or CLI
The 14 names above (inspect_data, deep_inspect, propose_pipeline, generate_code,
preprocess_neural, save_processed, quality_check, export_repo, etc.) are exposed
to you through your function-calling API. Invoke them DIRECTLY as tool calls.
- They are NOT Python modules. Do NOT `import inspect_data`, `import easybci_tools`,
  or `import easybci.pipeline` — those imports will always fail.
- They are NOT CLI subcommands. Do NOT shell out to `easybci inspect`, `easybci pipelines list`,
  or any other CLI. `easybci pipelines` is administrative (list/show/stats/flag/unflag), not the pipeline runner.
- If a tool name above does NOT appear in your tool list for this session, STOP and tell
  the user the tool is unavailable. Do NOT silently re-implement the step manually with
  execute_code / write_file / terminal — a hand-rolled pipeline produces output that
  doesn't match the contract and cannot be reproduced.

## Work directory path — sibling of the data, NOT one level higher
`work_dir = <data_leaf>.parent / f"{<data_leaf>.name}_preprocess_work_dir"`, where
`<data_leaf>` is the data DIRECTORY (the immediate folder that holds the recordings —
not its parent, not a file inside it).

The work_dir sits as a SIBLING of `<data_leaf>` — same parent directory, same leaf
name with `_preprocess_work_dir` appended:

| User-provided path                                     | work_dir (CORRECT, sibling of data dir)                       | WRONG (one level too high)                        |
|--------------------------------------------------------|---------------------------------------------------------------|---------------------------------------------------|
| `<parent>/<data_dir>/recording.<ext>`        (file)    | `<parent>/<data_dir>_preprocess_work_dir/`                    | `<parent>/recording_preprocess_work_dir/`         |
| `<parent>/<data_dir>/`                       (directory) | `<parent>/<data_dir>_preprocess_work_dir/`                  | `<parent.parent>/<data_dir>_preprocess_work_dir/` |
| `<parent>/<subject>/<session>/raw.<ext>`     (file)    | `<parent>/<subject>/<session>_preprocess_work_dir/`           | `<parent>/<subject>_preprocess_work_dir/`         |

Whether the user passes a FILE path or a DIRECTORY path, identify the data DIRECTORY
(the leaf folder containing the recordings) and place work_dir as its sibling.
Do NOT climb one extra level. Do NOT mentally abbreviate this as
`<data_parent>_preprocess_work_dir/` — the suffix is appended to the data LEAF's
NAME, and the work_dir lives in the leaf's PARENT.

DO NOT use write_file/terminal/execute_code to manually create files under
the work_dir's {code,plan,preprocessed_output}/ subfolders. The standard tools above
produce contract-compliant output (BIDS sub-*/ses-*, plan/ explainability, README,
.export_manifest); bypassing them produces a broken mini-repo the user cannot reproduce.
Trying to evade the guard via `cd $work_dir && python /tmp/script.py` is a known
anti-pattern — the cwd-relative output still lands in the wrong shape, the user will
have to throw it away, and the work was wasted.

If you don't know how a tool works, call skill_view('<tool-name>') first.
"""


def _looks_like_preprocessing_request(text: str) -> bool:
    """True iff the message text contains any keyword that signals a neural
    data preprocessing task. Case-insensitive substring match.
    """
    if not text:
        return False
    lower = text.lower()
    return any(kw.lower() in lower for kw in _PREPROCESSING_KEYWORDS)
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[easybci-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        Args:
            ...
            api_mode (str): API mode override: "chat_completions" or "responses"
            ...
            platform (str): The interface platform the user is on (e.g. "cli", "api_server").
                Used to inject platform-specific formatting hints into the system prompt.
            skip_context_files (bool): If True, skip auto-injection of SOUL.md, AGENTS.md, and .cursorrules
                into the system prompt. Use this for batch processing and data generation to avoid
                polluting trajectories with user-specific persona or project instructions.
            load_soul_identity (bool): If True, still use ~/.easybci/SOUL.md as the primary
        ...
        self.platform = platform  # "cli" or "api_server"
        ...
        if api_mode in {"chat_completions", "responses", "anthropic_messages", "bedrock_converse"}:
            self.api_mode = "responses" if api_mode in {"responses"} else api_mode
        ...
        try:
            from easybci_cli.model_normalize import (
        ...
        if (
            ...
            # from chat_completions to responses after the warm at __init__.
        ...
        # both live under ~/.easybci/logs/.  Idempotent, so gateway mode
        # (which creates a new AIAgent per message) won't duplicate handlers.
        from easybci_lib.logging_setup import setup_logging, setup_verbose_logging
        setup_logging(easybci_home=_easybci_home)
        ...
        if self.verbose_logging:
            ...
            # on the fact that easybci_logging.setup_logging() does not
            ...
            # handler level inside easybci_logging.py, not here.
            pass
        
        # Internal stream callback (set via run_conversation).
        ...
        # Responses/Anthropic wrapping for all known providers.
        # raw_responses=True because the main agent needs direct responses.stream()
        # access for Responses API streaming.
        ...
        if self.api_mode == "anthropic_messages":
            from easybci_agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token
            # Bedrock + Claude → use AnthropicBedrock SDK for full feature parity
            # (prompt caching, thinking budgets, adaptive thinking).
            _is_bedrock_anthropic = self.provider == "bedrock"
            if _is_bedrock_anthropic:
                from easybci_agent.anthropic_adapter import build_anthropic_bedrock_client
            ...
            try:
                from easybci_cli.config import load_config as _load_br_cfg
            ...
            if api_key and base_url:
                ...
                if base_url_host_matches(effective_base, "openrouter.ai"):
                    from easybci_agent.auxiliary_client import build_or_headers
                    ...
                    try:
                        from services.providers import get_provider_profile as _gpf
                ...
                from easybci_agent.auxiliary_client import resolve_provider_client
                _routed_client, _ = resolve_provider_client(
                    self.provider or "auto", model=self.model, raw_responses=True)
                    ...
                    if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                        ...
                        for _fb in _fb_entries:
                            ...
                            _fb_client, _fb_model = resolve_provider_client(
                                _fb["provider"], model=_fb["model"], raw_responses=True,
                        ...
                        if not _fb_resolved:
                            raise RuntimeError(
                                ...
                                f"variable, or switch to a different provider with `easybci model`."
                            )
                    if not getattr(self, "_fallback_activated", False):
                        # No provider configured — reject with a clear message.
                        raise RuntimeError(
                            "No LLM provider configured. Run `easybci model` to "
                            "select a provider, or run `easybci setup` for first-time "
                            "configuration."
                        )
        ...
        # Session logs go into ~/.easybci/sessions/ alongside gateway sessions
        easybci_home = get_easybci_home()
        self.logs_dir = easybci_home / "sessions"
        ...
        from easybci_lib.tools.checkpoint_manager import CheckpointManager
        ...
        from easybci_lib.tools.todo_tool import TodoStore
        self._todo_store = TodoStore()
        
        # Load config once for memory, skills, and compression sections
        try:
            from easybci_cli.config import load_config as _load_agent_config
        ...
        if not skip_memory:
            try:
                ...
                if self._memory_enabled or self._user_profile_enabled:
                    from easybci_lib.tools.memory_tool import MemoryStore
        ...
        if not skip_memory:
            try:
                ...
                if _mem_provider_name:
                    from easybci_agent.memory_manager import MemoryManager as _MemoryManager
                    from services.plugins.memory import load_memory_provider as _load_mem
                    ...
                    if self._memory_manager.providers:
                        _init_kwargs = {
                            ...
                            "easybci_home": str(get_easybci_home()),
                        ...
                        try:
                            ...
                            _init_kwargs["agent_workspace"] = "easybci"
                        except Exception:
                            pass
        ...
        try:
            from easybci_agent.auxiliary_client import _compression_threshold_for_model as _cthresh_fn
        ...
        try:
            from easybci_cli.config import get_compatible_custom_providers
        ...
        if _engine_name != "compressor":
            ...
            try:
                from services.plugins.context_engine import load_context_engine
            ...
            if _selected_engine is None:
                try:
                    from easybci_cli.plugins import get_plugin_context_engine
        ...
        from easybci_agent.model_metadata import MINIMUM_CONTEXT_LENGTH
        _ctx = getattr(self.context_compressor, "context_length", 0)
        if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                f"Model {self.model} has a context window of {_ctx:,} tokens, "
                f"which is below the minimum {MINIMUM_CONTEXT_LENGTH:,} required "
                f"by EasyBCI Agent.  Choose a model with at least "
        ...
        # Wire up StagedProgressTracker + EtaEstimator, and instrument every
        # registered tool handler so turn-scope elapsed samples land in
        # progress_history.jsonl (gradually replacing the heuristic defaults
        # with local samples).
        try:
            self._init_progress_subsystem()
        except Exception as exc:
            logger.debug("init_progress_subsystem failed: %s", exc)
        try:
            from easybci_lib.tools.registry import registry as _tool_registry
            from easybci_lib.tools.registry_eta_instrumentation import (
                instrument_registry_with_eta_recording,
            )
            _n = instrument_registry_with_eta_recording(_tool_registry)
            if _n:
                logger.debug("eta-recording instrumented %d tool handlers", _n)
        except Exception as exc:
            logger.debug("eta instrumentation skipped: %s", exc)
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least EasyBCI' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from easybci_agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from easybci_cli.models import ensure_lmstudio_model_loaded
    ...
    def switch_model(self, new_model, new_provider, api_key='', base_url='', api_mode=''):
        ...
        turn-scoped).
        """
        from easybci_cli.providers import determine_api_mode
    ...
    def _safe_print(self, *args, **kwargs):
        ...
        can crash long-running sessions and lose completed work.
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        shown even during streaming playback.
        ...
        parseable single-query flows such as ``easybci chat -q``. In that mode,
    ...
    def _emit_status(self, message: str) -> None:
        """Emit a lifecycle status message. Back-compat with 2-arg callbacks.

        Three-arg callbacks ``(event_type, message, progress=None)`` receive
        progress=None on plain status emissions; progress-bearing emissions
        come through _emit_progress instead.
        """
        try:
        ...
        if self.status_callback:
            try:
                try:
                    self.status_callback("lifecycle", message, None)
                except TypeError:
    ...
    def _emit_progress(self, progress: dict) -> None:
        """Emit a structured progress payload. CLI gets one inline-renderable
        line; gateway turns this into an SSE `progress` event."""
        if self.status_callback:
            try:
                stage = progress.get("stage", "?")
                pct = progress.get("percent")
                pct_str = f" {pct:.0f}%" if isinstance(pct, (int, float)) else ""
                summary = f"[{stage}{pct_str}] {progress.get('sub_step', '')}"
                try:
                    self.status_callback("progress", summary, progress)
                except TypeError:
                    self.status_callback("progress", summary)
            except Exception:
                logger.debug("status_callback error in _emit_progress", exc_info=True)

    def _emit_whole_turn_eta(self) -> None:
        """Emit ONE turn-scope `progress` event at the very start of a turn.

        The countdown is anchored once and ticks down in both CLI and WebUI —
        it represents the expected wall-time for the whole turn (user query →
        final assistant message), not per-tool / per-step.

        Sourcing rule:
        - history bucket ``intent_kind="whole_turn"`` (mean ± stddev when n≥5)
        - cold-start fallback: 60s with confidence="low" so the UI renders a
          visible "~60s?" countdown from the first run.
        """
        ...
        try:
            ...
            try:
                ...
                store = ProgressHistoryStore(
                    path=get_easybci_home() / "progress_history.jsonl"
                )
                stats = lookup_intent_stats(store, kind="whole_turn")
                if stats.n_samples > 0 and stats.mean_s is not None:
                    if stats.confidence == "high":
                        eta_seconds = int(round(stats.mean_s + (stats.stddev_s or 0.0)))
                    else:
                        eta_seconds = int(round(stats.mean_s))
                    confidence = stats.confidence
                    source = "history"
            except Exception:
                logger.debug("whole-turn history lookup failed", exc_info=True)

            if eta_seconds is None or eta_seconds < 5:
                eta_seconds = 60
                confidence = "low"
                source = "fallback"
    ...
    def _record_whole_turn_elapsed(self) -> None:
        ...
        try:
            ...
            started = getattr(self, "_turn_started_at", 0.0)
            if not started:
                return
            elapsed = max(0.0, _time.time() - started)
            if elapsed < 1.0:
                return
            record_intent_elapsed(kind="whole_turn", elapsed_s=elapsed)
        except Exception:
            logger.debug("record_whole_turn_elapsed failed", exc_info=True)

    def _emit_turn_scope_eta(self, tool_name: str) -> None:
        """Legacy per-tool turn-scope ETA emitter.

        Now a no-op by default — the whole-turn ETA emitted once at the start
        of the turn (``_emit_whole_turn_eta``) is the single source of the
        countdown shown in CLI / WebUI. Re-emitting per tool would re-anchor
        ``emittedAtMs`` and reset the countdown, which is what users objected to.

        Kept for binary back-compat / explicit opt-in via the environment flag
        ``EASYBCI_PER_TOOL_ETA=1``.
        """
        import os as _os
        if _os.environ.get("EASYBCI_PER_TOOL_ETA") != "1":
            return
        if not self.status_callback:
            return
        tracker = getattr(self, "_progress_tracker", None)
        estimator = getattr(tracker, "_estimator", None) if tracker is not None else None
        if estimator is None:
            return
        try:
            import time as _time
            from services.gateway.turn_eta import (
                build_turn_progress_payload,
                classify_tool_use_to_intent,
            )

            intent = classify_tool_use_to_intent(tool_name)
            if intent.kind == "unknown":
                return
            est = estimator.estimate(intent)
            if est is None:
                return
            ...
            try:
                from easybci_agent.i18n import t
                msg = t("progress.estimating_next_step", kind=intent.kind)
            except Exception:
                msg = f"estimating {intent.kind} elapsed..."
            try:
                self.status_callback("progress", msg, payload)
            except TypeError:
                self.status_callback("progress", msg)
        except Exception:
            logger.debug("emit_turn_scope_eta failed", exc_info=True)

    def _init_progress_subsystem(self) -> None:
        """Wire up StagedProgressTracker + HeartbeatDaemon.

        Called once from __init__. Tracker emits via _emit_progress so all
        progress flows through the same back-compatible callback. Also
        registers the tracker in a module-level singleton so tool handlers
        in easybci_lib/tools/neural_tools.py can find it.
        """
        def _on_progress(sp) -> None:
            self._emit_progress(sp.to_payload())

        try:
            from easybci_lib.constants import get_easybci_home
            from easybci_lib.tools.neural_processing.progress import EtaEstimator
            from easybci_lib.tools.neural_processing.progress.history import (
                ProgressHistoryStore,
            )
            _history_store = ProgressHistoryStore(
                path=get_easybci_home() / "progress_history.jsonl"
            )
            _estimator = EtaEstimator(history_store=_history_store)
        except Exception as exc:
            logger.debug("EtaEstimator wiring failed; tracker runs without ETA: %s", exc)
            _estimator = None

        self._progress_tracker = StagedProgressTracker(
    ...
    def _emit_stream_drop(
        ...
        *,
        ...
        ``easybci logs --level WARNING | grep "Stream drop"`` to inspect.
        """
        ...
        try:
            ...
            if client is None or not aux_model:
                msg = (
                    ...
                    "Run `easybci setup` or set OPENROUTER_API_KEY."
                )
            ...
            if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
                raise ValueError(
                    ...
                    f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by EasyBCI "
    ...
    def _replay_compression_warning(self) -> None:
        ...
        so every connected platform receives the
        warning.
        """
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``EASYBCI_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``EASYBCI_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return float(os.getenv("EASYBCI_API_TIMEOUT", 1800.0))
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``EASYBCI_API_CALL_STALE_TIMEOUT`` env var
          4. 300.0s default
        ...
        env_timeout = os.getenv("EASYBCI_API_CALL_STALE_TIMEOUT")
    ...
    def _strip_think_blocks(self, content: str) -> str:
        ...
        Delegates to agent.stream_filter.strip_reasoning_tags() — the canonical
        implementation shared with cli.py.
        """
        return _strip_reasoning_tags_impl(content)
    ...
    def _looks_like_responses_intermediate_ack(
        self,
        user_message: str,
    ...
    def _cleanup_task_resources(self, task_id: str) -> None:
        """Clean up VM resources for a given task.
    ...
    _SKILL_REVIEW_PROMPT = (
        ...
        "  • Negative claims about tools or features ('neural tools do not "
    ...
    def _build_memory_write_metadata(
        ...
        *,
        ...
        metadata: Dict[str, Any] = {
            ...
            "platform": self.platform or os.environ.get("EASYBCI_SESSION_SOURCE", "cli"),
            "tool_name": "memory",
        }
        if task_id:
    ...
    def _repair_message_sequence(self, messages: List[Dict]) -> int:
        ...
        session resume, explicit conversation_history passed in by
        ...
        try:
            ...
            for msg in messages[flush_from:]:
                ...
                self._session_db.append_message(
                    ...
                    responses_reasoning_items=msg.get("responses_reasoning_items") if role == "assistant" else None,
                    responses_message_items=msg.get("responses_message_items") if role == "assistant" else None,
        ...
        *,
        ...
        try:
            ...
            dump_payload: Dict[str, Any] = {
                ...
                "request": {
                    "method": "POST",
                    "url": f"{self.base_url.rstrip('/')}{'/responses' if self.api_mode == 'responses' else '/chat/completions'}",
                    "headers": {
            ...
            if env_var_enabled("EASYBCI_DUMP_REQUEST_STDOUT"):
    ...
    def _file_mutation_verifier_enabled(self) -> bool:
        ...
        ``EASYBCI_FILE_MUTATION_VERIFIER`` env var overrides config.  Exposed
        as a method so tests can patch a single seam without reaching into
        the private ``_turn_failed_file_mutations`` state dict.
        """
        try:
            import os as _os
            env = _os.environ.get("EASYBCI_FILE_MUTATION_VERIFIER")
            if env is not None:
                return env.strip().lower() not in ("0", "false", "no", "off")
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
            try:
                from easybci_cli.config import load_config as _load_config
    ...
    def _capture_rate_limits(self, http_response: Any) -> None:
        ...
        if not headers:
            return
        try:
            from easybci_agent.rate_limit_tracker import parse_rate_limit_headers
    ...
    def close(self) -> None:
        ...
        try:
            from easybci_lib.tools.process_registry import process_registry
    ...
    def _build_system_prompt_parts(self, system_message: str = None) -> Dict[str, str]:
        ...
        Delegates to SystemPromptBuilder for the actual assembly logic.
        Returns a dict with keys: stable, context, volatile.
        """
        if not hasattr(self, '_prompt_builder'):
            self._prompt_builder = SystemPromptBuilder(self)
        return self._prompt_builder.build_parts(system_message=system_message)
    ...
    def _build_system_prompt(self, system_message: str = None) -> str:
        ...
        one cached block — EasyBCI never rebuilds or reinjects parts of it
        mid-session, which is the only way to keep upstream prompt caches
        warm across turns.
        """
        parts = self._build_system_prompt_parts(system_message=system_message)
        joined = "\n\n".join(p for p in (parts["stable"], parts["context"], parts["volatile"]) if p)
        # Neural-preprocessing contract: append the 14-step contract when this
        # session has been flagged as a preprocessing task by run_conversation.
        # Sticky across turns + compression rebuilds — once active, stays
        # active for the session so the cached system prompt is stable.
        if getattr(self, "_preprocessing_contract_active", False):
            joined = joined + "\n\n" + _PREPROCESSING_CONTRACT_PREPEND
        return joined
    ...
    def _cap_delegate_task_calls(tool_calls: list) -> list:
        ...
        """
        from easybci_lib.tools.delegate_tool import _get_max_concurrent_children
    ...
    def _deterministic_call_id(fn_name: str, arguments: str, index: int = 0) -> str:
        ...
        """
        return _responses_deterministic_call_id(fn_name, arguments, index)

    @staticmethod
    def _split_responses_tool_id(raw_id: Any) -> tuple[Optional[str], Optional[str]]:
        """Split a stored tool id into (call_id, response_item_id)."""
        return _responses_split_tool_id(raw_id)
    ...
    def _derive_responses_function_call_id(
        ...
        return _responses_derive_function_call_id(call_id, response_item_id)
    ...
    def _create_openai_client(self, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
        ...
        if self.provider == "google-gemini-cli" or str(client_kwargs.get("base_url", "")).startswith("cloudcode-pa://"):
            from easybci_agent.gemini_cloudcode_adapter import GeminiCloudCodeClient
        ...
        if self.provider == "gemini":
            from easybci_agent.gemini_native_adapter import GeminiNativeClient, is_native_gemini_base_url
    ...
    def _run_responses_stream(self, api_kwargs: dict, client: Any = None, on_first_delta: callable = None):
        """Execute one streaming Responses API request and return the final response."""
        import httpx as _httpx

        active_client = client or self._ensure_primary_openai_client(reason="responses_stream_direct")
        ...
        self._responses_streamed_text_parts: list = []
        for attempt in range(max_stream_retries + 1):
            if self._interrupt_requested:
                raise InterruptedError("Agent interrupted before Responses stream retry")
                ...
                with active_client.responses.stream(**api_kwargs) as stream:
                    for event in stream:
                        ...
                        if "output_text.delta" in event_type or event_type == "response.output_text.delta":
                            ...
                            if delta_text:
                                self._responses_streamed_text_parts.append(delta_text)
                        ...
                        # (chatgpt.com/backend-api) stream valid items
                        ...
                        # Log non-completed terminal events for diagnostics
                            ...
                            logger.warning(
                                "Responses stream received terminal event %s "
                                "(status=%s, incomplete_details=%s, streamed_chars=%d). %s",
                                event_type, status, incomplete_details,
                                sum(len(p) for p in self._responses_streamed_text_parts),
                                self._client_log_context(),
                            )
                    final_response = stream.get_final_response()
                    # PATCH: Responses backend streams valid output items
                    ...
                    if isinstance(_out, list) and not _out:
                        if collected_output_items:
                            ...
                            logger.debug(
                                "Responses stream: backfilled %d output items from stream events",
                                len(collected_output_items),
                            )
                        elif self._responses_streamed_text_parts and not has_tool_calls:
                            assembled = "".join(self._responses_streamed_text_parts)
                            ...
                            logger.debug(
                                "Responses stream: synthesized output from %d text deltas (%d chars)",
                                len(self._responses_streamed_text_parts), len(assembled),
                            )
                    return final_response
            except (_httpx.RemoteProtocolError, _httpx.ReadTimeout, _httpx.ConnectError, ConnectionError) as exc:
                if attempt < max_stream_retries:
                    logger.debug(
                        "Responses stream transport failed (attempt %s/%s); retrying. %s error=%s",
                        attempt + 1,
                        max_stream_retries + 1,
                        self._client_log_context(),
                        exc,
                    ...
                    continue
                logger.debug(
                    "Responses stream transport failed; falling back to create(stream=True). %s error=%s",
                    self._client_log_context(),
                    exc,
                )
                return self._run_responses_create_stream_fallback(api_kwargs, client=active_client)
    ...
    def _run_responses_create_stream_fallback(self, api_kwargs: dict, client: Any = None):
        """Fallback path for stream completion edge cases on Responses API backends."""
        active_client = client or self._ensure_primary_openai_client(reason="responses_create_stream_fallback")
                ...
                if terminal_response is not None:
                    ...
                    if isinstance(_out, list) and not _out:
                        if collected_output_items:
                            ...
                            logger.debug(
                                "Responses fallback stream: backfilled %d output items",
                                len(collected_output_items),
                            )
                            ...
                            logger.debug(
                                "Responses fallback stream: synthesized from %d deltas (%d chars)",
    ...
    def _try_refresh_anthropic_client_credentials(self) -> bool:
        ...
        try:
            from easybci_agent.anthropic_adapter import resolve_anthropic_token, build_anthropic_client
    ...
    def _apply_client_headers_for_base_url(self, base_url: str) -> None:
        from easybci_agent.auxiliary_client import _AI_GATEWAY_HEADERS, build_or_headers
        ...
        if base_url_host_matches(base_url, "openrouter.ai"):
            ...
            _ph_headers = None
            try:
                from services.providers import get_provider_profile as _gpf2
    ...
    def _swap_credential(self, entry) -> None:
        ...
        if self.api_mode == "anthropic_messages":
            from easybci_agent.anthropic_adapter import build_anthropic_client, _is_oauth_token
            ...
            try:
                if self.api_mode == "responses":
                    request_client_holder["client"] = self._create_request_openai_client(
                        reason="responses_stream_request",
                        api_kwargs=api_kwargs,
                    )
                    result["response"] = self._run_responses_stream(
                        api_kwargs,
                        client=request_client_holder["client"],
                        on_first_delta=getattr(self, "_responses_on_first_delta", None),
                    )
    ...
    def _interruptible_streaming_api_call(
        ...
        - responses: delegates to _run_responses_stream (already streaming)
        ...
        if self.api_mode == "responses":
            # Responses API streams internally via _run_responses_stream. The main dispatch
            # in _interruptible_api_call already calls it; we just need to
            # ensure on_first_delta reaches it. Store it on the instance
            # temporarily so _run_responses_stream can pick it up.
            self._responses_on_first_delta = on_first_delta
            try:
                return self._interruptible_api_call(api_kwargs)
            finally:
                self._responses_on_first_delta = None
        ...
        def _call_chat_completions():
            ...
            # wins over the EASYBCI_API_TIMEOUT env default if the user set it.
            _provider_timeout_cfg = get_provider_request_timeout(self.provider, self.model)
            _base_timeout = (
                _provider_timeout_cfg
                if _provider_timeout_cfg is not None
                else float(os.getenv("EASYBCI_API_TIMEOUT", 1800.0))
            )
            # Read timeout: config wins here too.  Otherwise use
            # EASYBCI_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
            if _provider_timeout_cfg is not None:
                _stream_read_timeout = _provider_timeout_cfg
            else:
                _stream_read_timeout = float(os.getenv("EASYBCI_STREAM_READ_TIMEOUT", 120.0))
                # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
                # prefill on large contexts before producing the first token.
                # Auto-increase the httpx read timeout unless the user explicitly
                # overrode EASYBCI_STREAM_READ_TIMEOUT.
        ...
        def _call():
            ...
            _max_stream_retries = int(os.getenv("EASYBCI_STREAM_RETRIES", 2))
            ...
            try:
                for _stream_attempt in range(_max_stream_retries + 1):
                    ...
                    # interrupt entirely.  On slow providers (local) each
        ...
        _stream_stale_timeout_base = float(os.getenv("EASYBCI_STREAM_STALE_TIMEOUT", 180.0))
        # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
        # for prefill on large contexts.  Disable the stale detector unless
        # the user explicitly set EASYBCI_STREAM_STALE_TIMEOUT.
    ...
    def _try_activate_fallback(self, reason: "FailoverReason | None" = None) -> bool:
        ...
        # raw_responses=True because the main agent needs direct responses.stream()
        # access for Responses API providers.
        try:
            from easybci_agent.auxiliary_client import resolve_provider_client
            ...
            if not fb_api_key_hint:
                ...
                # _normalize_custom_provider_entry in easybci_cli/config.py).
                fb_key_env = (fb.get("key_env") or fb.get("api_key_env") or "").strip()
                if fb_key_env:
                    fb_api_key_hint = os.getenv(fb_key_env, "").strip() or None
            # For local Ollama endpoints, pull OLLAMA_API_KEY from env
            # when no explicit key is in the fallback config.
            if fb_base_url_hint and "localhost" in fb_base_url_hint and not fb_api_key_hint:
                fb_api_key_hint = os.getenv("OLLAMA_API_KEY") or None
            fb_client, _resolved_fb_model = resolve_provider_client(
                fb_provider, model=fb_model, raw_responses=True,
            ...
            try:
                from easybci_cli.model_normalize import normalize_model_for_provider
            ...
            if fb_provider == "anthropic" or fb_base_url.rstrip("/").lower().endswith("/anthropic"):
                ...
                fb_api_mode = "responses"
            elif self._provider_model_requires_responses_api(
                fb_model,
                provider=fb_provider,
            ):
                # GPT-5.x models usually need Responses API, but keep
                # provider-specific exceptions on chat completions.
                fb_api_mode = "responses"
            ...
            if fb_api_mode == "anthropic_messages":
                ...
                from easybci_agent.anthropic_adapter import build_anthropic_client, resolve_anthropic_token, _is_oauth_token
    ...
    def _describe_image_for_anthropic_fallback(self, image_url: str, role: str) -> str:
        ...
        description = ""
        try:
            from easybci_lib.tools.vision_tools import vision_analyze_tool
    ...
    def _model_supports_vision(self) -> bool:
        ...
        """
        try:
            from easybci_agent.models_dev import get_model_capabilities
    ...
    def _get_transport(self, api_mode: str = None):
        ...
        t = cache.get(mode)
        if t is None:
            from easybci_agent.transports import get_transport
    ...
    def _prepare_messages_for_non_vision_model(self, api_messages: list) -> list:
        ...
        Runs on the chat.completions / responses paths. Vision-capable
    ...
    def _try_shrink_image_parts_in_messages(self, api_messages: list) -> bool:
        ...
        try:
            from easybci_lib.tools.vision_tools import _resize_image_for_vision
        except Exception as exc:
        ...
        def _shrink_data_url(url: str) -> Optional[str]:
            ...
            try:
                ...
                tmp = tempfile.NamedTemporaryFile(
                    prefix="easybci_shrink_", suffix=suffix, delete=False,
                )
                try:
                    tmp.write(raw)
                    tmp.close()
    ...
    def _build_api_kwargs(self, api_messages: list) -> dict:
        ...
        if self.api_mode == "responses":
            ...
            _msgs_for_responses = self._prepare_messages_for_non_vision_model(api_messages)
            return _ct.build_kwargs(
                model=self.model,
                messages=_msgs_for_responses,
                tools=tools_for_api,
                reasoning_config=self.reasoning_config,
        ...
        _is_gh = base_url_host_matches(self._base_url_lower, "models.github.ai")
        ...
        try:
            from easybci_agent.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE
        ...
        if _is_or and "claude" in (self.model or "").lower():
            try:
                from easybci_agent.anthropic_adapter import _get_anthropic_max_output
        ...
        if _is_qwen:
            _qwen_meta = {
                "sessionId": self.session_id or "easybci",
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from easybci_cli.models import lmstudio_model_reasoning_options
    ...
    def _resolve_lmstudio_summary_reasoning_effort(self) -> Optional[str]:
        ...
        """
        from easybci_agent.lmstudio_reasoning import resolve_lmstudio_effort
    ...
    def _build_assistant_message(self, assistant_message, finish_reason: str) -> dict:
        ...
        # Responses API: preserve encrypted reasoning items for
        # multi-turn continuity. These get replayed as input on the next turn.
        responses_items = getattr(assistant_message, "responses_reasoning_items", None) or getattr(assistant_message, "codex_reasoning_items", None)
        if responses_items:
            msg["responses_reasoning_items"] = responses_items

        # Responses API: preserve exact assistant message items (with
        ...
        responses_message_items = getattr(assistant_message, "responses_message_items", None) or getattr(assistant_message, "codex_message_items", None)
        if responses_message_items:
            msg["responses_message_items"] = responses_message_items
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict) -> dict:
        """Strip Responses API fields from tool_calls for strict providers.
        ...
        original messages list retains call_id/response_item_id for OpenAI
        Responses API compatibility (e.g. if the session falls back to a
        Responses API provider later).
    ...
    def _should_sanitize_tool_calls(self) -> bool:
        ...
        OpenAI Responses API uses fields like call_id and response_item_id
        ...
        Returns:
            bool: True if sanitization is needed (non-Responses API), False otherwise.
        """
    ...
    def _compress_context(self, messages: list, system_message: str, *, approx_tokens: int = None, task_id: str = "default", focus_topic: str = None) -> tuple:
        ...
        try:
            from easybci_lib.tools.file_tools import reset_file_dedup
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        """
        from easybci_lib.tools.delegate_tool import delegate_task as _delegate_task
    ...
    def _invoke_tool(self, function_name: str, function_args: dict, effective_task_id: str,
        ...
        if not pre_tool_block_checked:
            try:
                from easybci_cli.plugins import get_pre_tool_call_block_message
        ...
        if function_name == "pipeline_tracker":
            from easybci_lib.tools.todo_tool import todo_tool as _todo_tool
            return _todo_tool(
            ...
            if not session_db:
                from easybci_lib.state import format_session_db_unavailable
                return json.dumps({"success": False, "error": format_session_db_unavailable()})
            from easybci_lib.tools.session_search_tool import session_search as _session_search
            ...
            from easybci_lib.tools.memory_tool import memory_tool as _memory_tool
            result = _memory_tool(
            ...
            from easybci_lib.tools.clarify_tool import clarify_tool as _clarify_tool
    ...
    def _execute_tool_calls_concurrent(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        for tc, name, args, block_result, blocked_by_guardrail in parsed_calls:
            ...
            self._emit_turn_scope_eta(name)
        ...
        def _run_tool(index, tool_call, function_name, function_args):
            ...
            try:
                from easybci_lib.tools.environments.base import set_activity_callback
        ...
        for i, (tc, name, args, block_result, blocked_by_guardrail) in enumerate(parsed_calls):
            ...
            if r is None:
                ...
                if is_error:
                    ...
                    # Whitelist denials (e.g. background-review fork tries a
                    # non-allowed tool) are expected guardrail behavior — the
                    # next LLM turn sees the deny message in the tool result
            ...
            # tool_status/tool_duration: r is None → execution was cancelled
            # or threading dropped the result; treat as error with 0 duration.
            _ts = "error" if (r is None or is_error) else "done"
            tool_msg = {
                "role": "tool",
                "name": name,
                ...
                "tool_call_id": tc.id,
                "tool_status": _ts,
                "tool_duration": float(tool_duration) if tool_duration is not None else None,
            }
        ...
        for i, tool_call in enumerate(assistant_message.tool_calls, 1):
            ...
            if not _execution_blocked and self.tool_progress_callback:
                try:
                    ...
                    logging.debug(f"Tool progress callback error: {cb_err}")
            if not _execution_blocked:
                self._emit_turn_scope_eta(function_name)
            ...
            if _block_msg is not None:
                ...
                tool_duration = 0.0
            elif function_name == "pipeline_tracker":
                from easybci_lib.tools.todo_tool import todo_tool as _todo_tool
            ...
            if isinstance(function_result, str):
                # Tool error / completion logs always include the full
                # tool result so errors.log entries are usable for offline
                # diagnosis. agent.log + errors.log rotate (5MB / 2MB
                # respectively) so unbounded entries don't accumulate.
            ...
            if not _execution_blocked:
                ...
                result_preview = function_result
            if _is_error_result:
                # Whitelist denials are expected guardrail behavior — see
            ...
            tool_msg = {
                ...
                "content": _tool_content,
                "tool_call_id": tool_call.id,
                "tool_status": "error" if _is_error_result else "done",
                "tool_duration": float(tool_duration) if tool_duration is not None else None,
            }
            messages.append(tool_msg)
    ...
    def _handle_max_iterations(self, messages: list, api_call_count: int) -> str:
        ...
        try:
            ...
            summary_extra_body = {}
            try:
                from easybci_agent.auxiliary_client import _fixed_temperature_for_model, OMIT_TEMPERATURE as _OMIT_TEMP
            except Exception:
            ...
            if self.api_mode == "responses":
                resp_kwargs = self._build_api_kwargs(api_messages)
                resp_kwargs.pop("tools", None)
                summary_response = self._run_responses_stream(resp_kwargs)
            ...
            if final_response:
                ...
                if self.api_mode == "responses":
                    resp_kwargs = self._build_api_kwargs(api_messages)
                    resp_kwargs.pop("tools", None)
                    retry_response = self._run_responses_stream(resp_kwargs)
    ...
    def _run_conversation_impl(
        ...
        # Record turn start for turn-scope ETA elapsed reporting.
        self._turn_started_at = time.time()
        # Anchor a single whole-turn countdown (CLI + WebUI). One emit, no
        # per-step re-anchoring — see _emit_whole_turn_eta docstring.
        try:
            self._emit_whole_turn_eta()
        except Exception:
            logger.debug("whole-turn ETA emit failed", exc_info=True)

        # Neural-preprocessing contract activation: if this turn's user
        # message looks like a preprocessing request (keyword / file-ext
        # match), flag the agent so _build_system_prompt appends the 14-
        # step contract. Sticky on the agent instance — once activated
        # it stays on for the session (and survives context compression
        # rebuilds, because the flag is what triggers re-injection).
        if not getattr(self, "_preprocessing_contract_active", False):
            _candidate_text = persist_user_message or user_message or ""
            if _looks_like_preprocessing_request(_candidate_text):
                self._preprocessing_contract_active = True
                # Invalidate the cached prompt so the next rebuild includes
                # the contract. Without this, the cached prompt computed
                # before activation would persist for the rest of the session.
                try:
                    self._cached_system_prompt = None
                except Exception:
                    pass
        ...
        # Register a callback so any tool that records a mini-repo work_dir
        # (propose_pipeline / export_repo / migrate_work_dir / …) auto-persists
        # the path against the active session row. The WebUI /artifacts
        # endpoint then resolves the path across restarts. Best-effort; any
        # failure is swallowed inside the callback.
        try:
            from easybci_lib.tools.neural_processing.export.finalize import (
                register_work_dir_persist_callback,
            )

            _sdb = self._session_db
            _sid = self.session_id

            def _persist_work_dir(path):
                if _sdb is None or not _sid:
                    return
                try:
                    _sdb.set_session_work_dir(_sid, path)
                except Exception:
                    logger.debug(
                        "set_session_work_dir(%s, %r) failed", _sid, path,
                        exc_info=True,
                    )

            register_work_dir_persist_callback(_persist_work_dir)
        except Exception:
            logger.debug("register_work_dir_persist_callback failed", exc_info=True)
        ...
        try:
            from easybci_agent.auxiliary_client import set_runtime_main
            set_runtime_main(
        ...
        # ``easybci logs --session <id>`` can filter a single conversation.
        from easybci_lib.logging_setup import set_session_context
        ...
        from easybci_lib.tools.skill_provenance import set_current_write_origin
        ...
        self._responses_incomplete_retries = 0
        ...
        if self._cached_system_prompt is None:
            ...
            if stored_prompt:
                ...
                try:
                    from easybci_cli.plugins import invoke_hook as _invoke_hook
                    _invoke_hook(
                        "on_session_start",
                        session_id=self.session_id,
                        model=self.model,
        ...
        # are reused.  The system prompt is EasyBCI's territory; plugins
        ...
        while (api_call_count < self.max_iterations and self.iteration_budget.remaining > 0) or self._budget_grace_call:
            ...
            for idx, msg in enumerate(messages):
                ...
                # Strip OpenAI Responses API fields (call_id, response_item_id) for
            ...
            # cache prefix.  The system prompt is reserved for EasyBCI internals.
            #
            # EasyBCI invariant: the system prompt is built ONCE per session
            ...
            while retry_count < max_retries:
                try:
                    ...
                    if env_var_enabled("EASYBCI_DUMP_REQUESTS"):
                    ...
                    if self.api_mode == "responses":
                        ...
                        if not _ct_v.validate_response(response):
                            if response is None:
                                ...
                                if _resp_status in {"failed", "cancelled"}:
                                    _resp_error_obj = getattr(response, "error", None)
                                    _resp_error_msg = (
                                        _resp_error_obj.get("message") if isinstance(_resp_error_obj, dict)
                                        else str(_resp_error_obj) if _resp_error_obj
                                        else f"Responses API returned status '{_resp_status}'"
                                    )
                                    logging.warning(
                                        "Responses API response status='%s' (error=%s). Routing to fallback. %s",
                                        _resp_status, _resp_error_msg,
                                        self._client_log_context(),
                                    )
                                    response_invalid = True
                                    error_details.append(f"response.status={_resp_status}: {_resp_error_msg}")
                                else:
                                    ...
                                    if _out_text_stripped:
                                        logger.debug(
                                            "Responses API response.output is empty but output_text is present "
                                        ...
                                        logger.warning(
                                            "Responses API response.output is empty after stream backfill "
                    ...
                    if hasattr(response, 'usage') and response.usage:
                        ...
                        # sessions (gateway, delegated runs) cannot lose
                        ...
                        if self._session_db and self.session_id:
                            try:
                                ...
                                # Under concurrent load, the initial
                        ...
                        # three API shapes (Anthropic / Responses / OpenAI-chat)
                    ...
                    if (
                        ...
                        _dhh = _dhh_fn()
                        print(f"{self.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for EasyBCI-managed OAuth/setup tokens")
                        print(f"{self.log_prefix}     • Check ANTHROPIC_API_KEY in {_dhh}/.env for API keys or legacy token values")
                        print(f"{self.log_prefix}     • For API keys: verify at https://platform.claude.com/settings/keys")
                        print(f"{self.log_prefix}     • For Claude Code: run 'claude /login' to refresh, then retry")
                        print(f"{self.log_prefix}     • Legacy cleanup: easybci config set ANTHROPIC_TOKEN \"\"")
                        print(f"{self.log_prefix}     • Clear stale keys: easybci config set ANTHROPIC_API_KEY \"\"")
                    ...
                    if (
                        ...
                        try:
                            from easybci_lib.tools.schema_sanitizer import strip_pattern_and_format
                    ...
                    if is_client_error:
                        ...
                        if classified.is_auth or classified.reason == FailoverReason.billing:
                            ...
                            self._vprint(f"{self.log_prefix}      • Is the key valid? Run: easybci setup", force=True)
            ...
            try:
                ...
                if self.api_mode == "responses" and finish_reason == "incomplete":
                    self._responses_incomplete_retries += 1

                    interim_msg = self._build_assistant_message(assistant_message, finish_reason)
                    interim_has_content = bool((interim_msg.get("content") or "").strip())
                    interim_has_reasoning = bool(interim_msg.get("reasoning", "").strip()) if isinstance(interim_msg.get("reasoning"), str) else False
                    interim_has_responses_reasoning = bool(interim_msg.get("responses_reasoning_items"))
                    interim_has_responses_message_items = bool(interim_msg.get("responses_message_items"))

                    if (
                        interim_has_content
                        or interim_has_reasoning
                        or interim_has_responses_reasoning
                        or interim_has_responses_message_items
                    ):
                        ...
                        last_resp_items = last_msg.get("responses_reasoning_items") if isinstance(last_msg, dict) else None
                        interim_resp_items = interim_msg.get("responses_reasoning_items")
                        last_resp_message_items = last_msg.get("responses_message_items") if isinstance(last_msg, dict) else None
                        interim_resp_message_items = interim_msg.get("responses_message_items")
                        duplicate_interim = (
                            ...
                            and last_resp_items == interim_resp_items
                            and last_resp_message_items == interim_resp_message_items
                        )
                        if not duplicate_interim:
                            messages.append(interim_msg)
                            self._emit_interim_assistant_message(interim_msg)

                    if self._responses_incomplete_retries < 3:
                        if not self.quiet_mode:
                            self._vprint(f"{self.log_prefix}↻ Responses API response incomplete; continuing turn ({self._responses_incomplete_retries}/3)")
                        self._session_messages = messages
                        self._save_session_log(messages)
                        continue
                    ...
                    return {
                        ...
                        "completed": False,
                        "partial": True,
                        "error": "Responses API response remained incomplete after 3 continuation attempts",
                    }
                elif hasattr(self, "_responses_incomplete_retries"):
                    self._responses_incomplete_retries = 0
                ...
                if assistant_message.tool_calls:
                    ...
                    if turn_content and self._has_content_after_think_block(turn_content):
                        ...
                        _HOUSEKEEPING_TOOLS = frozenset({
                            "memory", "pipeline_tracker", "skill_manage", "session_search",
                        })
                    ...
                    if (
                        ...
                        and responses_ack_continuations < 2
                        and self._looks_like_responses_intermediate_ack(
                            user_message=user_message,
                            assistant_content=final_response,
                            messages=messages,
                        )
                    ):
                        responses_ack_continuations += 1
        ...
        # Clean up VM resources for this task after conversation completes
        self._cleanup_task_resources(effective_task_id)
        ...
        # Persist whole-turn elapsed sample so the next turn's ETA gets sharper.
        self._record_whole_turn_elapsed()
...
def main(
    query: str = None,
        ...
        app-server` subprocess and projects its events back into EasyBCI'
        messages list so memory/skill review keep working.

        Called from run_conversation() when self.api_mode == "codex_app_server".
        Returns the same dict shape as the chat_completions path.
        """
        from easybci_agent.transports.codex_app_server_session import CodexAppServerSession
        ...
        if not hasattr(self, "_codex_session") or self._codex_session is None:
            ...
            # Approval callback: defer to EasyBCI' standard prompt flow if a
            # CLI thread has installed one. Gateway contexts get the
            # codex-side fail-closed default.
            try:
                from easybci_lib.tools.terminal_tool import _get_approval_callback
...
def _finalize_neural_work_dir(
    status: str = "ok",
    partial_reason: Optional[str] = None,
) -> None:
    """Auto-finalize safety net for the CLI entrypoint.

    Looks up the work_dir last touched by a preprocessing tool in this run
    (via the thread-local registry in ``tools/neural_processing/export/finalize.py``)
    and runs ``build_mini_repo`` with the supplied ``status``. Silently no-ops
    when no work_dir was registered (chat-only sessions) or when finalize
    itself fails (the user's run must never crash because of the safety net).

    After a successful finalize, surface the analysis_goal
    and web_evidence as one-line CLI banners (via ``cli_output.print_info``)
    so the user sees what goal/evidence the LLM committed to. ``--no-color``
    is honoured automatically via ``Colors.should_use_color`` and the
    ``NO_COLOR`` env var.
    """
    try:
        from easybci_lib.tools.neural_processing.export.finalize import (
    ...
    if not work_dir:
        return
    finalize_work_dir(work_dir, status=status, partial_reason=partial_reason)
    try:
        _print_neural_run_banner(work_dir)
    except Exception:
        # Banner is purely informational — never fail the run because of it.
        logging.getLogger(__name__).debug(
            "_print_neural_run_banner failed for %s", work_dir, exc_info=True,
        )


def _read_neural_run_summary(work_dir: str) -> Dict[str, Any]:
    """Collect the goal + web_evidence summary for a finished
    work_dir. Read order matches the canonical single-source-of-truth
    chain: pipeline_record.json (authoritative once written) → plan/goal.json
    + plan/web_evidence.json (side files written before pipeline_record).

    Returns a dict suitable for both human display and JSON-mode export::

        {
            "work_dir": "/.../foo_preprocess_work_dir",
            "analysis_goal": "classification" | "generic" | None,
            "web_evidence": {"status": "ok"|"unavailable", ...} | None,
        }

    Missing fields are returned as ``None`` rather than raising — finalize
    runs even on partial / chat-only sessions where these files don't exist.
    """
    ...
    # 1. pipeline_record.json — authoritative once finalize has stamped it.
    rec_path = wd / "plan" / "pipeline_record.json"
    if rec_path.exists():
        try:
            rec = json.loads(rec_path.read_text(encoding="utf-8"))
            if isinstance(rec, dict):
                if isinstance(rec.get("analysis_goal"), str) and rec["analysis_goal"]:
                    summary["analysis_goal"] = rec["analysis_goal"]
                if isinstance(rec.get("web_evidence"), dict):
                    summary["web_evidence"] = rec["web_evidence"]
        except (OSError, json.JSONDecodeError):
            pass

    # 2. plan/goal.json — side file written by propose_pipeline.
    if not summary["analysis_goal"]:
        goal_path = wd / "plan" / "goal.json"
        if goal_path.exists():
            try:
                payload = json.loads(goal_path.read_text(encoding="utf-8"))
                if isinstance(payload, dict) and isinstance(payload.get("analysis_goal"), str):
                    summary["analysis_goal"] = payload["analysis_goal"]
            except (OSError, json.JSONDecodeError):
                pass

    # 3. plan/web_evidence.json — side file written at propose_pipeline time.
    if summary["web_evidence"] is None:
        ev_path = wd / "plan" / "web_evidence.json"
        if ev_path.exists():
            try:
                payload = json.loads(ev_path.read_text(encoding="utf-8"))
                if isinstance(payload, dict):
                    summary["web_evidence"] = payload
            except (OSError, json.JSONDecodeError):
                pass
...
def _format_neural_run_banner_lines(summary: Dict[str, Any]) -> list:
    """Render the goal + web_evidence one-liners.

    Pure formatter; no I/O. Returns the sequence of strings that
    ``_print_neural_run_banner`` hands to ``cli_output.print_info``. Exposed
    separately so the smoke test can validate the strings without
    monkey-patching stdout.
    """
    from easybci_agent.i18n import t as _t

    lines: list = []
    goal = (summary.get("analysis_goal") or "").strip()
    if goal:
        if goal == "generic":
            lines.append(_t("cli.preprocessing.goal_line_generic"))
        else:
            lines.append(_t("cli.preprocessing.goal_line", goal=goal))

    ev = summary.get("web_evidence")
    if isinstance(ev, dict) and ev:
        if ev.get("status") == "ok":
            provider = str(ev.get("provider") or "web")
            confidence = ev.get("confidence")
            try:
                conf_text = f"{float(confidence):.2f}" if confidence is not None else "n/a"
            except (TypeError, ValueError):
                conf_text = "n/a"
            applied_list = ev.get("applied_to_steps") or []
            if isinstance(applied_list, list) and applied_list:
                applied = ", ".join(str(x) for x in applied_list)
            else:
                # When the resolver didn't tag specific params, surface what
                # the recommendations referenced so the line still names
                # something concrete instead of a useless "n/a".
                recos = ev.get("recommendations") or []
                applied = ", ".join(
                    str(r.get("param")) for r in recos
                    if isinstance(r, dict) and r.get("param")
                ) or "n/a"
            lines.append(
                _t(
                    "cli.preprocessing.web_evidence_line",
                    provider=provider,
            ...
            reason = ev.get("reason") or "no provider configured"
            lines.append(
                _t("cli.preprocessing.web_evidence_unavailable", reason=str(reason))
            )

    return lines


def _print_neural_run_banner(work_dir: str) -> None:
    """Surface goal + web_evidence after finalize completes."""
    summary = _read_neural_run_summary(work_dir)
    lines = _format_neural_run_banner_lines(summary)
    if not lines:
        return
    try:
        from easybci_cli.cli_output import print_info
    except Exception:
        # CLI-output unavailable (e.g. headless gateway worker) — fall back
        # to plain stderr so the banner is still visible in logs.
        for ln in lines:
...
def main(
    ...
    # Handle tool listing
    if list_tools:
        from easybci_lib.model_tools import get_all_tool_names, get_available_toolsets
        from easybci_lib.toolsets import get_all_toolsets, get_toolset_info
    ...
    # Run conversation, wrapped in the auto-finalize safety net.
    # The hook only acts when a preprocessing tool registered a work_dir;
    # chat-only runs are silently skipped.
    try:
        result = agent.run_conversation(user_query)
        _finalize_neural_work_dir(status="ok", partial_reason=None)
    except KeyboardInterrupt:
        _finalize_neural_work_dir(
            status="partial", partial_reason="keyboard_interrupt"
        )
        raise
