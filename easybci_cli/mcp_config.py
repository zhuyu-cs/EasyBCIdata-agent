...
from easybci_cli.config import (
    ...
    get_env_value,
    save_env_value,
    get_easybci_home,  # noqa: F401 — used by test mocks
)
...
def _prompt(question: str, *, password: bool = False, default: str = "") -> str:
    from easybci_cli.cli_output import prompt as _shared_prompt
...
def _probe_single_server(
    ...
    """
    from easybci_lib.tools.mcp_tool import (
        _ensure_mcp_loop,
        _run_on_mcp_loop,
        _connect_server,
        _stop_mcp_loop,
...
def cmd_mcp_add(args):
    ...
    # easybci_cli/main.py for why the dest is renamed.
    ...
    if not url and not command:
        ...
        _info("Examples:")
        _info('  easybci mcp add ink --url "https://mcp.ml.ink/mcp"')
        _info('  easybci mcp add github --command npx --args @modelcontextprotocol/server-github')
        _info('  easybci mcp add myserver --preset mypreset')
        return
            ...
            if auth_type == "header" or not auth_type:
                ...
                if existing_key:
                    ...
                    if api_key:
                        ...
                        _success(f"Saved to {display_easybci_home()}/.env as {env_key}")
    ...
    try:
        ...
        if _confirm("Save config anyway (you can test later)?", default=False):
            ...
            _info("Fix the issue, then: easybci mcp test " + name)
        return
    ...
    print()
    _success(f"Saved '{name}' to {display_easybci_home()}/config.yaml ({tool_count}/{total} tools enabled)")
...
def cmd_mcp_remove(args):
    ...
    # earlier `easybci mcp test` in the same session) is evicted too.
    try:
        from easybci_lib.tools.mcp_oauth_manager import get_manager
...
def cmd_mcp_list(args=None):
    ...
    if not servers:
        ...
        print()
        _info("Add one with:")
        _info('  easybci mcp add <name> --url <endpoint>')
        _info('  easybci mcp add <name> --command <cmd> --args <args...>')
        print()
        return
...
def cmd_mcp_login(args):
    ...
    if server_config.get("auth") != "oauth":
        ...
        _info("Use `easybci mcp remove` + `easybci mcp add` to reconfigure auth.")
        return
...
# ─── easybci mcp configure ────────────────────────────────────────────────────

def cmd_mcp_configure(args):
    """Reconfigure which tools are enabled for an existing MCP server."""
    import sys as _sys
    if not _sys.stdin.isatty():
        print("Error: 'easybci mcp configure' requires an interactive terminal.", file=_sys.stderr)
        _sys.exit(1)
    name = args.name
    servers = _get_mcp_servers()
...
def mcp_command(args):
    """Main dispatcher for ``easybci mcp`` subcommands."""
    ...
    if handler:
        ...
        _info("easybci mcp serve                              Run as MCP server")
        _info("easybci mcp add <name> --url <endpoint>        Add an MCP server")
        _info("easybci mcp add <name> --command <cmd>         Add a stdio server")
        _info("easybci mcp add <name> --preset <preset>       Add from a known preset")
        _info("easybci mcp remove <name>                      Remove a server")
        _info("easybci mcp list                               List servers")
        _info("easybci mcp test <name>                        Test connection")
        _info("easybci mcp configure <name>                   Toggle tools")
        _info("easybci mcp login <name>                       Re-authenticate OAuth")
        print()
