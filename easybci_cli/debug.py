"""``easybci debug`` debug tools for EasyBCI Agent.

Currently supports:
    easybci debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.easybci/logs/*.log`` are not leaked into
...
_REDACTION_BANNER = (
    "[easybci debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.easybci/pastes/pending.json``.
    ...
    ran ``easybci debug share`` repeatedly.

    Deletion is now driven by the gateway's background ticker
    (``gateway/run.py::_start_background_ticker``) which calls
    ``_sweep_expired_pastes`` once per hour.  ``easybci debug share`` also
    runs an opportunistic sweep on entry as a fallback for CLI-only users
    who never start the gateway.
    """
    return get_easybci_home() / "pastes" / "pending.json"
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``easybci debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``easybci debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
⚠️  This will upload the following to a public paste service:
  • System info (OS, Python version, EasyBCI version, provider, which API keys
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `easybci debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "easybci-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``easybci debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.easybci/pastes/pending.json``
    and the gateway's background ticker sweeps expired entries once per hour.
    ``easybci debug share`` also runs an opportunistic sweep as a fallback
...
def _delete_hint(url: str) -> str:
    ...
    paste_id = _extract_paste_id(url)
    if paste_id:
        return f"easybci debug delete {url}"
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "easybci-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----EasybciDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from easybci_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_easybci_home() / "logs" / filename) if filename else None
...
def _capture_dump() -> str:
    """Run ``easybci dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``easybci dump`` is run
        internally.
...
def run_debug_share(args):
    ...
    if redact:
        logger.info(
            "easybci debug share: applied force-mode redaction to log snapshots before upload"
        )
    ...
    print(f"To delete now:  easybci debug delete <url>")

    print(f"\nShare these links with the EasyBCI team for support.")
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: easybci debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'easybci debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``easybci debug`` call.
    ...
    # best-effort — any failure is swallowed so ``easybci debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: easybci debug <command>")
        print()
        print("Commands:")
