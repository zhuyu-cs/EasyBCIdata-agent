...
The EasyBCI docs site hosts a JSON manifest of curated models for providers
...
def _cache_path() -> Path:
    ...
    return get_easybci_home() / "cache" / "model_catalog.json"
...
def get_curated_bci_team_models() -> list[str] | None:
    """Stub — returns empty list.

    Returns ``None`` when the manifest is unavailable.
    """
    block = _get_provider_block("bci_team")
    if not block:
        return None
    out: list[str] = []
    for m in block.get("models", []):
...
def reset_cache() -> None:
    """Clear the in-process cache. Used by tests and ``easybci model --refresh``."""
    global _catalog_cache, _catalog_cache_source_mtime
    _catalog_cache = None
