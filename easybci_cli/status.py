"""
Status command for easybci CLI.

Shows the status of all EasyBCI Agent components.
"""
...
from easybci_cli.auth import AuthError, resolve_provider
from easybci_cli.colors import Colors, color
from easybci_cli.config import get_env_path, get_env_value, get_easybci_home, load_config
from easybci_cli.models import provider_label
from easybci_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``easybci config``'s
...
def show_status(args):
    """Show status of all EasyBCI Agent components."""
    show_all = getattr(args, 'all', False)
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 🧠 EasyBCI Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: easybci auth add minimax-oauth)'}"
    )
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: easybci model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from easybci_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    if terminal_env == "ssh":
        ...
        auth_label = "configured" if auth_ok else "not configured"
        sdk_ok = importlib.util.find_spec("vercel") is not None
        sdk_label = "installed" if sdk_ok else "missing (install: pip install 'easybci-agent[vercel]')"
        print(f"  Runtime:      {runtime}")
        print(f"  SDK:          {check_mark(sdk_ok)} {sdk_label}")
        print(f"  Auth:         {check_mark(auth_ok)} {auth_label}")
    ...
    try:
        from easybci_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   easybci gateway")
    ...
    sessions_file = get_easybci_home() / "sessions" / "sessions.json"
    ...
    print(color("  Run 'easybci doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'easybci setup' to configure", Colors.DIM))
    print()
