#!/usr/bin/env python3
"""
EasyBCI CLI - Main entry point.

Usage:
    easybci                     # Interactive chat (default)
    easybci chat                # Interactive chat
    easybci gateway             # Run gateway in foreground
    easybci gateway start       # Start gateway as service
    easybci gateway stop        # Stop gateway service
    easybci gateway status      # Show gateway status
    easybci gateway install     # Install gateway service
    easybci gateway uninstall   # Uninstall gateway service
    easybci setup               # Interactive setup wizard
    easybci logout              # Clear stored authentication
    easybci status              # Show status of all components
    easybci doctor              # Check configuration and dependencies
    easybci honcho setup                    # Configure Honcho AI memory integration
    easybci honcho status                   # Show Honcho config and connection status
    easybci honcho sessions                 # List directory → session name mappings
    easybci honcho map <name>               # Map current directory to a session name
    easybci honcho peer                     # Show peer names and dialectic settings
    easybci honcho peer --user NAME         # Set user peer name
    easybci honcho peer --ai NAME           # Set AI peer name
    easybci honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    easybci honcho mode                     # Show current memory mode
    easybci honcho mode [hybrid|honcho|local]  # Set memory mode
    easybci honcho tokens                   # Show token budget settings
    easybci honcho tokens --context N       # Set session.context() token cap
    easybci honcho tokens --dialectic N     # Set dialectic result char cap
    easybci honcho identity                 # Show AI peer identity representation
    easybci honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    easybci honcho migrate                  # Step-by-step migration guide: OpenClaw native → EasyBCI + Honcho
    easybci version             Show version
    easybci update              Update to latest version
    easybci uninstall           Uninstall EasyBCI Agent
    easybci acp                 Run as an ACP server for editor integration
    easybci sessions browse     Interactive session picker with search

    easybci claw migrate --dry-run  # Preview migration without changes
"""
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to EASYBCI_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (easybci tools, easybci setup, easybci model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'easybci {command_name}' requires an interactive terminal.\n"
...
# ---------------------------------------------------------------------------
# Profile override — MUST happen before any easybci module import.
#
# Many modules cache EASYBCI_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("EASYBCI_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.easybci/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set EASYBCI_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # Mirrors easybci_cli.profiles._PROFILE_ID_RE so we never call
    ...
    # 1.5 If EASYBCI_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.easybci/profiles/coder or
    # /opt/data/profiles/coder).  If EASYBCI_HOME points to the easybci root
    ...
    # 2. If no flag, check active_profile in the easybci root
    if profile_name is None:
        try:
            from easybci_lib.constants import get_default_easybci_root

            active_path = get_default_easybci_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set EASYBCI_HOME
    if profile_name is not None:
        try:
            from easybci_cli.profiles import resolve_profile_env

            easybci_home = resolve_profile_env(profile_name)
        except (ValueError, FileNotFoundError) as exc:
            print(f"Error: {exc}", file=sys.stderr)
            sys.exit(1)
        except Exception as exc:
            # A bug in profiles.py must NEVER prevent easybci from starting
            print(
                f"Warning: profile override failed ({exc}), using default",
                file=sys.stderr,
            )
            return
        os.environ["EASYBCI_HOME"] = easybci_home
...
from easybci_cli.experience_commands import (
    cmd_experience_flag_negative,
    cmd_experience_list_negatives,
    cmd_experience_normalize_keys,
    cmd_experience_revoke_negative,
)
from easybci_cli.federation_commands import (
    cmd_federation_init,
    cmd_federation_list_sources,
    cmd_federation_pull,
    cmd_federation_push,
    cmd_federation_status,
    cmd_federation_subscribe,
    cmd_federation_unsubscribe,
)
from easybci_cli.internalization_commands import (
    cmd_internalization_show,
    cmd_internalizations_list,
    cmd_revoke_internalization,
)
from easybci_cli.pipelines_commands import (
    cmd_pipelines_flag,
    cmd_pipelines_list,
    cmd_pipelines_migrate_layout,
    cmd_pipelines_resume,
    cmd_pipelines_show,
    cmd_pipelines_stats,
...
# Bridge security.redact_secrets from config.yaml → EASYBCI_REDACT_SECRETS env
# var BEFORE easybci_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "EASYBCI_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_easybci_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["EASYBCI_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
    pass  # best-effort — redaction stays at default (enabled) on config errors

# Initialize centralized file logging early — all `easybci` subcommands
# (chat, setup, gateway, config, etc.) write to agent.log + errors.log.
try:
    from easybci_lib.logging_setup import setup_logging as _setup_logging
...
try:
    from easybci_cli.config import load_config as _load_config_early
    from easybci_lib.constants import apply_ipv4_preference as _apply_ipv4
...
from easybci_lib.constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from easybci_cli.config import get_env_path, get_easybci_home, load_config
    from easybci_cli.auth import get_auth_status

    # Determine whether EasyBCI itself has been explicitly configured (model
    # in config that isn't the hardcoded default). 
    ...
    if isinstance(model_cfg, dict):
        ...
        _model_name = ""
    _has_easybci_config = _model_name and _model_name != _DEFAULT_MODEL
    ...
    # Check provider-specific auth fallbacks (for example, via gh auth).
    try:
    ...
    # Check for BCI Team Portal OAuth credentials
    auth_file = get_easybci_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
    ...
    # Only count these if EasyBCI has been explicitly configured — Claude Code
    # being installed doesn't mean the user wants EasyBCI to use their tokens.
    if _has_easybci_config:
        try:
...
def _exec_in_container(container_info: dict, cli_args: list):
    ...
    Args:
        container_info: dict with backend, container_name, exec_user, easybci_bin
        cli_args: the original CLI arguments (everything after 'easybci')
    """

    backend = container_info["backend"]
    container_name = container_info["container_name"]
    exec_user = container_info["exec_user"]
    easybci_bin = container_info["easybci_bin"]
        ...
        if sudo_path:
            ...
            if probe2.returncode != 0:
                print(
                    ...
                    f"  }}];\n"
                    f"\n"
                    f"Or run: sudo easybci {' '.join(cli_args)}",
                    file=sys.stderr,
                )
                sys.exit(1)
        else:
            print(
                f"Error: container '{container_name}' not found via {backend}.\n"
                f"The container may be running under root. Try: sudo easybci {' '.join(cli_args)}",
                file=sys.stderr,
            )
            sys.exit(1)
...
def _print_tui_exit_summary(
    ...
    print(f"  easybci --tui --resume {target}")
    if title:
        print(f'  easybci --tui -c "{title}"')
    print()
...
def _tui_need_npm_install(root: Path) -> bool:
    """True when @easybci/ink is missing or node_modules is behind package-lock.json.
    ...
    ink = root / "node_modules" / "@easybci" / "ink" / "package.json"
    if not ink.is_file():
        return True
    if not lock.is_file():
        return False
...
def _ensure_tui_node() -> None:
    ...
    ``EASYBCI_SKIP_NODE_BOOTSTRAP=1`` to disable auto-install.
    """
    if shutil.which("node") and shutil.which("npm"):
        return
    if os.environ.get("EASYBCI_SKIP_NODE_BOOTSTRAP"):
        return
    ...
    easybci_home = os.environ.get("EASYBCI_HOME") or str(Path.home() / ".easybci")
    try:
        ...
        result = subprocess.run(
            ...
            env={**os.environ, "EASYBCI_HOME": easybci_home},
            capture_output=True,
            text=True,
            check=False,
        )
    ...
    extras.extend([Path(easybci_home) / "node" / "bin", Path.home() / ".local" / "bin"])
...
def _make_tui_argv(tui_dir: Path, tui_dev: bool) -> tuple[list[str], Path]:
    """TUI: --dev → tsx src; else node dist (EASYBCI_TUI_DIR prebuilt or esbuild)."""
    _ensure_tui_node()

    def _node_bin(bin: str) -> str:
        if bin == "node":
            env_node = os.environ.get("EASYBCI_NODE")
    ...
    ext_dir = os.environ.get("EASYBCI_TUI_DIR")
    if tui_dev and ext_dir:
        print(
            f"Error: --dev is incompatible with EASYBCI_TUI_DIR={ext_dir}\n"
            f"The prebuilt TUI has no source code to hot-reload.\n"
            f"Unset EASYBCI_TUI_DIR (e.g. `unset EASYBCI_TUI_DIR`) to use --dev from a checkout.",
            file=sys.stderr,
        )
        sys.exit(1)
    ...
    if _tui_need_npm_install(tui_dir):
        npm = _node_bin("npm")
        if not os.environ.get("EASYBCI_QUIET"):
...
def _normalize_tui_toolsets(toolsets: object) -> list[str]:
    ...
    try:
        from easybci_cli.oneshot import _normalize_toolsets
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="easybci-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["EASYBCI_TUI_ACTIVE_SESSION_FILE"] = active_session_file
    env["EASYBCI_PYTHON_SRC_ROOT"] = os.environ.get(
        "EASYBCI_PYTHON_SRC_ROOT", str(PROJECT_ROOT)
    )
    env.setdefault("EASYBCI_PYTHON", sys.executable)
    env.setdefault("EASYBCI_CWD", os.getcwd())
    env.setdefault("NODE_ENV", "development" if tui_dev else "production")

    wt_info = None
    if worktree:
        try:
            from easybci_lib.cli import (
                _cleanup_worktree,
                _git_repo_root,
        ...
        if not wt_info:
            sys.exit(1)
        env["EASYBCI_CWD"] = wt_info["path"]
        env["TERMINAL_CWD"] = wt_info["path"]

    if model:
        env["EASYBCI_MODEL"] = model
        env["EASYBCI_INFERENCE_MODEL"] = model
    if provider:
        env["EASYBCI_TUI_PROVIDER"] = provider
        env["EASYBCI_INFERENCE_PROVIDER"] = provider
    tui_toolsets = _normalize_tui_toolsets(toolsets)
    if tui_toolsets:
        env["EASYBCI_TUI_TOOLSETS"] = ",".join(tui_toolsets)
    if skills:
        if isinstance(skills, (list, tuple)):
            ...
            if flattened:
                env["EASYBCI_TUI_SKILLS"] = ",".join(flattened)
        else:
            value = str(skills).strip()
            if value:
                env["EASYBCI_TUI_SKILLS"] = value
    if query:
        env["EASYBCI_TUI_QUERY"] = query
    if image:
        env["EASYBCI_TUI_IMAGE"] = image
    if checkpoints:
        env["EASYBCI_TUI_CHECKPOINTS"] = "1"
    if pass_session_id:
        env["EASYBCI_TUI_PASS_SESSION_ID"] = "1"
    if max_turns is not None:
        env["EASYBCI_TUI_MAX_TURNS"] = str(max_turns)
    if verbose:
        env["EASYBCI_TUI_TOOL_PROGRESS"] = "verbose"
    elif quiet:
        env["EASYBCI_TUI_TOOL_PROGRESS"] = "off"
    if accept_hooks:
        env["EASYBCI_ACCEPT_HOOKS"] = "1"
    ...
    if resume_session_id:
        env["EASYBCI_TUI_RESUME"] = resume_session_id
...
def cmd_chat(args):
    ...
    use_tui = getattr(args, "tui", False) or os.environ.get("EASYBCI_TUI") == "1"
    ...
    if continue_val and not getattr(args, "resume", None):
        if isinstance(continue_val, str):
            ...
            if resolved:
                ...
                print("Use 'easybci sessions list' to see available sessions.")
                sys.exit(1)
        else:
    ...
    if not _has_any_provider_configured():
        print()
        print(
            "It looks like EasyBCI isn't configured yet -- no API keys or providers found."
        )
        print()
        print("  Run:  easybci setup")
        print()

        from easybci_cli.setup import (
        ...
        if reply in {"", "y", "yes"}:
            ...
            return
        print()
        print("You can run 'easybci setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from easybci_cli.banner import prefetch_update_check
    ...
    try:
        from easybci_lib.tools.skills_sync import sync_skills
    ...
    if getattr(args, "yolo", False):
        os.environ["EASYBCI_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.easybci/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["EASYBCI_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["EASYBCI_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["EASYBCI_SESSION_SOURCE"] = args.source
    ...
    from easybci_lib.cli import main as cli_main
...
def cmd_gateway(args):
    ...
    from easybci_cli.gateway import gateway_command
...
def cmd_proxy(args):
    ...
    from easybci_cli.proxy.cli import cmd_proxy as _cmd_proxy
...
def cmd_setup(args):
    ...
    from easybci_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``easybci model``) and the setup wizard
    ...
    from easybci_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("EASYBCI_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'easybci model' for "
                "available providers, or run 'easybci doctor' to diagnose config "
                "issues."
            )
    ...
    from easybci_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.easybci/.env if the active provider is not 'custom'.
...
#
# EasyBCI uses lightweight "auxiliary" models for side tasks (vision analysis,
...
# `easybci model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `easybci model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
_AUX_TASKS: list[tuple[str, str, str]] = [
    ...
    ("triage_specifier", "Triage specifier", "reserved aux slot (templated to compression)"),
]

# Sanity check: keys must be unique. The list is consulted by per-task
# pickers, the apply-to-all writer, and the setup wizard — a duplicate
# would silently double-write and over-report counts.
assert len({_k for _k, _, _ in _AUX_TASKS}) == len(_AUX_TASKS), (
    "_AUX_TASKS contains duplicate task keys"
)
...
def _aux_apply_routing_to_all_tasks(
    routing: dict,
    *,
    exclude: list[str] | None = None,
) -> int:
    """Write `routing` to every task in _AUX_TASKS except those in `exclude`.

    Single save_config() call — atomic from the user's POV. Returns the
    number of tasks written. Used by setup wizard's `_setup_aux_general`
    so one provider+model choice fans out to all general aux slots
    (vision is excluded — it has its own picker).
    """
    from easybci_cli.config import load_config, save_config

    exclude_set = set(exclude or [])
    known_keys = {task_key for task_key, _, _ in _AUX_TASKS}
    unknown = exclude_set - known_keys
    if unknown:
        # Typo in caller — log it but proceed (the exclude is a no-op
        # for keys not in _AUX_TASKS, so this is informational only).
        logger.debug(
            "_aux_apply_routing_to_all_tasks: exclude contains unknown task keys: %s",
            sorted(unknown),
        )
    cfg = load_config()
    aux = cfg.setdefault("auxiliary", {})
    ...
    for task_key, _name, _desc in _AUX_TASKS:
        if task_key in exclude_set:
            continue
        entry = aux.setdefault(task_key, {})
        if not isinstance(entry, dict):
            entry = {}
            aux[task_key] = entry
        entry["provider"] = routing.get("provider", "auto") or "auto"
        entry["model"] = routing.get("model", "") or ""
        entry["base_url"] = routing.get("base_url", "") or ""
        entry["api_key"] = routing.get("api_key", "") or ""
        count += 1
    save_config(cfg)
    return count
...
def _aux_config_menu() -> None:
    ...
    while True:
        ...
        print("  EasyBCI only falls back to a lightweight backend (OpenRouter,")
        print("  BCI Team Portal) if the main model is unavailable.  Override a")
...
def _aux_select_for_task(task: str) -> None:
    ...
    Thin wrapper around `_aux_pick_routing` (which does the prompting)
    plus `_save_aux_choice` (which writes one task slot). Behavior is
    identical to the pre-refactor implementation; the shared
    `_aux_pick_routing` lets the setup wizard reuse the same picker.
    """
    from easybci_cli.config import load_config
    ...
    routing = _aux_pick_routing(
        title=f"Configure {display_name} — current: {_format_aux_current(task_cfg)}",
        current_provider=current_provider,
        current_model=current_model,
        current_base_url=current_base_url,
        require_vision_capable=(task == "vision"),
    )
    if routing is None:
        return
    ...
    if provider == "auto":
        ...
        print(f"{display_name}: custom ({short_url}){suffix}")
    elif model:
        print(f"{display_name}: {provider} · {model}")
    else:
        print(f"{display_name}: {provider} (provider default model)")


def _aux_prompt_for_provider(
    provider_slug: str,
    curated_models: list,
    current_model: str = "",
) -> dict | None:
    """Pure prompt — returns routing dict or None on cancel; no I/O writes.

    Mirrors the model-selection logic of `_aux_flow_provider_model` but
    returns the picked model as a dict instead of saving it to disk.
    Used by `_aux_pick_routing` so a single picker can drive both
    per-task saves (`_aux_select_for_task`) and apply-to-all flows
    (setup wizard's `_setup_aux_general`).
    """
    from easybci_cli.auth import _prompt_model_selection
    from easybci_cli.models import get_pricing_for_provider
...
def _aux_prompt_custom_endpoint(
    current_base_url: str = "",
    current_model: str = "",
) -> dict | None:
    """Pure prompt — returns routing dict for a custom OpenAI-compatible endpoint.

    Mirrors `_aux_flow_custom_endpoint` but returns the routing dict
    instead of saving. Returns None if cancelled or no URL provided.
    """
    import getpass
...
def _aux_pick_routing(
    *,
    title: str,
    current_provider: str = "auto",
    current_model: str = "",
    current_base_url: str = "",
    require_vision_capable: bool = False,
    allow_auto: bool = True,
) -> dict | None:
    """Run provider+model picker, return routing dict or None on cancel.

    Provider menu = `auto` (when allow_auto=True) + already-authenticated
    providers (via `list_authenticated_providers`) + `custom endpoint`
    (manual URL/key/model). Per the spec, this matches the existing
    `_aux_select_for_task` UX — custom-endpoint is the path that
    satisfies the URL/key/model constraint when the user wants a
    provider not yet authenticated.

    When `allow_auto=False` (used by the first-run setup wizard), the
    "auto (recommended)" entry is suppressed: the user must commit to a
    concrete provider/endpoint or hit Back to skip. The "auto" routing
    silently falls through to main_model at runtime, which on a fresh
    install often means "no vision" / "wrong endpoint" without the user
    realizing — so the wizard forces a deliberate choice.

    When `require_vision_capable=True`:
      - canonical providers in `_PROVIDERS_WITHOUT_VISION` are tagged
        '(no image input)' in the menu
      - if the user picks one anyway, a warning is printed but the
        routing is still returned (deliberate user override)
      - custom-endpoint entries are not tagged (we can't know their
        image support)

    Returned dict shape:
      {"provider": str, "model": str, "base_url": str, "api_key": str}
    """
    from easybci_agent.auxiliary_client import _PROVIDERS_WITHOUT_VISION
    from easybci_cli.cli_output import print_warning
    from easybci_cli.model_switch import list_authenticated_providers
    ...
    for p in providers:
        ...
        vision_warn = (
            "  (no image input)"
            if require_vision_capable and slug in _PROVIDERS_WITHOUT_VISION
            else ""
        )
        entries.append(
            (slug, f"{name}{model_hint}{marker}{vision_warn}", list(models))
        )
    ...
    if slug == "__back__":
        return None
    if slug == "__auto__":
        return {"provider": "auto", "model": "", "base_url": "", "api_key": ""}
    if slug == "__custom__":
        routing = _aux_prompt_custom_endpoint(current_base_url, current_model)
    else:
        routing = _aux_prompt_for_provider(slug, models, current_model)
    if routing is None:
        return None
    if (
        require_vision_capable
        and routing.get("provider") in _PROVIDERS_WITHOUT_VISION
    ):
        print_warning(
            f"Provider '{routing['provider']}' does not accept image input — "
            "vision calls will fail. You can change this later via "
            "`easybci model → Configure auxiliary models → Vision`."
        )
    return routing
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from easybci_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    # in-flow (K/R/C) instead of having to edit ~/.easybci/.env by hand. The
    # previous bypass-when-key-exists branch left no way to recover from a
    # bad paste short of re-running `easybci setup` from scratch. OpenRouter
    ...
    from easybci_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    # Route through _prompt_api_key so users can replace a stale/broken key
    # in-flow (K/R/C) instead of having to edit ~/.easybci/.env by hand.
    ...
    from easybci_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_bci_team(config, current_model="", args=None):
    """BCI Team Portal provider: removed."""
    print("The BCI Team provider has been removed.")
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from easybci_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from easybci_cli.models import _PROVIDER_MODELS
...
def _model_flow_google_gemini_cli(_config, current_model=""):
    ...
    if not status.get("logged_in"):
        try:
            from easybci_agent.google_oauth import resolve_project_id_from_env, start_oauth_flow
...
def _model_flow_custom(config):
    ...
    """
    from easybci_cli.auth import _save_model_choice, deactivate_provider
    from easybci_cli.config import get_env_value, load_config, save_config
    ...
    from easybci_cli.models import probe_api_models
    ...
    # Prompt for API compatibility mode explicitly so Responses-API-compatible
    # custom providers don't silently fall back to chat_completions.
    ...
    if model_name:
        ...
        if base_url or api_key:
            deactivate_provider()
        # Persist the custom endpoint to disk even without a model name.
        cfg = load_config()
        _disk_model = cfg.get("model")
        if not isinstance(_disk_model, dict):
            _disk_model = {"default": _disk_model} if _disk_model else {}
            cfg["model"] = _disk_model
        _disk_model["provider"] = "custom"
        _disk_model["base_url"] = effective_url
        if effective_key:
            _disk_model["api_key"] = effective_key
        if api_mode:
            _disk_model["api_mode"] = api_mode
        else:
            _disk_model.pop("api_mode", None)
        save_config(cfg)

        # Also sync the caller's config dict so the setup wizard's final
        # save_config(config) doesn't overwrite with stale values.
        config["model"] = dict(_disk_model)
        print("Endpoint saved. Use `/model` in chat or `easybci model` to set a model.")
...
def _prompt_custom_api_mode_selection(base_url: str, current_api_mode: str = "") -> Optional[str]:
    ...
    """
    from easybci_cli.runtime_provider import _detect_api_mode_for_url
    ...
    mode_options = [
        (
            "",
            "Auto-detect",
            "Use EasyBCI URL heuristics; best for standard OpenAI-compatible endpoints.",
        ),
        (
            "chat_completions",
            "Chat Completions",
        ...
        (
            "responses",
            "Responses API",
            "Use /responses for OpenAI Responses API-compatible tool-calling backends.",
        ),
        (
            "anthropic_messages",
            "Anthropic Messages",
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from easybci_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from easybci_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from easybci_cli import azure_detect
    import getpass
    ...
    print("Anthropic-style API endpoints.  EasyBCI will probe your")
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from easybci_cli.curses_ui import flush_stdin
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``easybci setup`` / ``easybci model``.
    ...
    recover from a malformed paste without editing ``~/.easybci/.env`` by hand.
    ...
    from easybci_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from easybci_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `easybci setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_bedrock_api_key(config, region, current_model=""):
    ...
    if selected:
        ...
        # Save the API key env var name so easybci knows where to find it
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    from easybci_cli.models import (
        fetch_api_models,
    ...
    if provider_id == "gemini" and existing_key:
        try:
            from easybci_agent.gemini_native_adapter import probe_gemini_tier
        except Exception:
        ...
        if probe_gemini_tier is not None:
            ...
            if tier == "free":
                ...
                print(
                    "   EasyBCI typically makes 3-10 API calls per user turn "
                ...
                print()
                print(
                    "   To use Gemini with EasyBCI, enable billing on your "
                ...
                print(
                    ...
                    "OpenRouter (free models), Groq, BCI Team."
                )
                print()
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from easybci_lib.constants import display_easybci_home as _dhh_fn

            print(
                f"    EasyBCI will use Claude's credential store directly instead of copying a setup-token into {_dhh_fn()}/.env."
            )
            return True
        return False
...
def _model_flow_anthropic(config, current_model=""):
    ...
    if selected:
        ...
        # providers if the user switches without running 'easybci model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    """Authenticate EasyBCI CLI with a provider."""
    from easybci_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from easybci_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from easybci_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from easybci_cli.status import show_status
...
def cmd_hooks(args):
    ...
    from easybci_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    if subject == "websearch":
        from easybci_cli.doctor import run_doctor_websearch
        run_doctor_websearch(args)
        return
    if subject == "goals":
        # Report analysis_goals REGISTRY conflicts
        return run_doctor_goals(args)
    if subject == "layout":
        from easybci_cli.doctor import doctor_layout
        from easybci_cli.cli_output import print_error

        wd = getattr(args, "work_dir", None)
        if not wd:
            print_error("doctor layout: work_dir positional argument required")
            return 2
        dry_run = bool(getattr(args, "dry_run", False))
        apply_ = bool(getattr(args, "apply", False))
        if dry_run == apply_:
            print_error(
                "doctor layout: exactly one of --dry-run / --apply is required"
            )
            return 2
        json_mode = bool(getattr(args, "json", False))
        return doctor_layout(wd, dry_run=dry_run, json_mode=json_mode)
    from easybci_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from easybci_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from easybci_cli.debug import run_debug

    run_debug(args)


def cmd_finalize(args):
    """Manually run the mini-repo finalize hook on an existing work_dir.

    Used to upgrade a ``status=partial`` work_dir (produced when a run was
    interrupted or failed) to a full ``status=ok`` build with consolidated
    intermediates, or to bring a manually-curated work_dir back into contract
    compliance. Wraps the shared ``finalize_work_dir`` helper so behavior
    matches the auto-finalize hook exactly.
    """
    from pathlib import Path
    ...
    if not work_dir.exists() or not work_dir.is_dir():
        print_error(f"work_dir does not exist or is not a directory: {work_dir}")
        return 1
    ...
    if result is None:
        print_error(
            f"finalize failed for {work_dir} — see logs (~/.easybci/logs/) for details"
        )
        return 1

    n_files = result.get("n_files", 0)
    print_info(
        f"Finalized {work_dir} (status={result.get('status', args.status)}, "
        f"{n_files} files in mini-repo)"
    )
    if result.get("cached"):
        print_info("(no changes — work_dir was already finalized as status=ok)")
    pending = result.get("pending_consolidations") or []
    if pending:
        print_info(
            f"Note: {len(pending)} item(s) pending consolidation — re-run with "
            f"--status ok to finish."
        )
    return 0


def cmd_migrate_work_dir(args):
    """Migrate a legacy/violating *_preprocess_work_dir to the contract layout.

    Default is dry-run (just prints the planned actions). Pass --apply to
    actually execute, which writes a tar.gz backup first (use --no-backup
    to skip). With --recursive, treats the path as a parent directory and
    migrates every *_preprocess_work_dir under it.
    """
    from pathlib import Path

    from easybci_cli.cli_output import print_error, print_info, print_warning
    from easybci_lib.tools.neural_processing.migration.migrate_work_dir import (
        apply_migration, find_work_dirs_recursive, is_work_dir,
    )
    ...
    if args.recursive:
        work_dirs = find_work_dirs_recursive(target)
        if not work_dirs:
            print_warning(f"no *_preprocess_work_dir found under {target}")
            return 0
        print_info(f"Found {len(work_dirs)} work_dir(s) under {target}")
    else:
        if not is_work_dir(target):
            print_error(
                f"not a *_preprocess_work_dir: {target}\n"
                f"Pass --recursive to scan a parent directory."
            )
            return 1
        work_dirs = [target]
    ...
    for wd in work_dirs:
        ...
        if result.skipped_reason and not result.applied:
            print_warning(f"  skipped: {result.skipped_reason}")
            failures += 1

    if not args.apply:
        print_info("\nDry-run complete. Re-run with --apply to execute.")
    elif failures == 0:
        print_info(f"\nAll {len(work_dirs)} work_dir(s) migrated successfully.")
    else:
        print_warning(f"\n{failures}/{len(work_dirs)} work_dir(s) had issues.")
        return 1
    return 0
...
def cmd_config(args):
    ...
    from easybci_cli.config import config_command

    config_command(args)


def cmd_backup(args):
    """Back up EasyBCI home directory to a zip file."""
    if getattr(args, "quick", False):
        from easybci_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from easybci_cli.backup import run_backup

        run_backup(args)


def cmd_import(args):
    """Restore a EasyBCI backup from a zip file."""
    from easybci_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"EasyBCI Agent v{__version__} ({__release_date__})")
    ...
    try:
        from easybci_cli.banner import check_for_updates
        from easybci_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    """Uninstall EasyBCI Agent."""
    _require_tty("uninstall")
    from easybci_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``easybci update --gateway`` so interactive prompts (stash restore,
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    The Vite build outputs to ``easybci_cli/web_dist/`` (per vite.config.ts
    outDir: "../easybci_cli/web_dist"), NOT to ``web/dist/``.  Uses the Vite
    manifest as the sentinel because it is written last and therefore has the
    newest mtime of any build output.
    """
    dist_dir = web_dir.parent / "easybci_cli" / "web_dist"
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``easybci update`` to stash the
    lockfile — repeatedly.
    """
...
def _build_web_ui(web_dir: Path, *, fatal: bool = False) -> bool:
    ...
    Args:
        ...
        fatal: If True, print error guidance and return False on failure
               instead of a soft warning (used by ``easybci web``).
    ...
    # build path usable on a stock `py -m easybci_cli.main web` invocation.
    ...
    if r1.returncode != 0:
        _say(
            ...
            + ("" if fatal else " (easybci web will not be available)")
        )
        _relay(r1)
        if fatal:
...
def _find_stale_dashboard_pids() -> list[int]:
    """Return PIDs of ``easybci dashboard`` processes other than ourselves.

    ``easybci dashboard`` is a long-lived server process commonly started and
    forgotten.  When ``easybci update`` replaces files on disk, the running
    ...
    patterns = [
        "easybci dashboard",
        "easybci_cli.main dashboard",
        "easybci_cli/main.py dashboard",
    ]
    self_pid = os.getpid()
    dashboard_pids: list[int] = []

    try:
        # Scan the process table via ps and match against explicit patterns.
        # Using ps (rather than `pgrep.*easybci.*dashboard"`) keeps us
        # consistent with `easybci_cli.gateway._scan_gateway_pids` and avoids
        # the greedy regex matching unrelated cmdlines that merely contain
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `easybci update`.
    ...
    """
    try:
        from easybci_agent import curator
    except Exception:
        return
    try:
    ...
    print("  Preview now:  easybci curator run --dry-run")
    print("  Pause it:     easybci curator pause")
    print(
        "  Docs:         https://github.com/zhuyu-cs/EasyBCI-Data"
    )
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``easybci update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``easybci update`` invocations skip the block until a newer
    ...
    print(
        ...
        "View anytime: easybci curator status)"
    )
...
def _kill_stale_dashboard_processes(
    ...
    """Kill running ``easybci dashboard`` processes.

    Called at the end of ``easybci update`` (default ``reason``) and also
    from ``easybci dashboard --stop`` (which overrides ``reason``).  The
    ...
    if killed:
        ...
        print("    easybci dashboard --port <port>")
...
def _update_via_zip(args):
    """Update EasyBCI Agent by downloading a ZIP archive.
    ...
    branch = "main"
    zip_url = (
        f"https://github.com/zhuyu-cs/EasyBCI-Data/archive/refs/heads/{branch}.zip"
    )

    print("→ Downloading latest version...")
    try:
        tmp_dir = tempfile.mkdtemp(prefix="easybci-update-")
        zip_path = os.path.join(tmp_dir, f"easybci-agent-{branch}.zip")
        ...
        # GitHub ZIPs extract to easybci-agent-<branch>/
        extracted = os.path.join(tmp_dir, f"easybci-agent-{branch}")
...
def _stash_local_changes_if_needed(git_cmd: list[str], cwd: Path) -> Optional[str]:
    ...
    stash_name = datetime.now(timezone.utc).strftime(
        "easybci-update-autostash-%Y%m%d-%H%M%S"
    )
...
def _restore_stashed_changes(
    ...
    if prompt_user:
        ...
        print("  Review the result afterward if EasyBCI behaves unexpectedly.")
    ...
    if restore.returncode != 0 or has_conflicts:
        ...
        # files makes easybci completely unrunnable (SyntaxError on import).
    ...
    if stash_selector is None:
        print(
            "⚠ Local changes were restored, but EasyBCI couldn't find the stash entry to drop."
        )
        print(
        ...
        if drop.returncode != 0:
            print(
                "⚠ Local changes were restored, but EasyBCI couldn't drop the saved stash entry."
            )
    ...
    print("  Review `git diff` / `git status` if EasyBCI behaves unexpectedly.")
    return True


# =========================================================================
# Fork detection and upstream management for `easybci update`
# =========================================================================

OFFICIAL_REPO_URLS = {
    "https://github.com/zhuyu-cs/EasyBCI-Data.git",
    "git@github.com:zhuyu-cs/EasyBCI-Data.git",
    "https://github.com/zhuyu-cs/EasyBCI-Data",
    "git@github.com:zhuyu-cs/EasyBCI-Data",
}
OFFICIAL_REPO_URL = "https://github.com/zhuyu-cs/EasyBCI-Data.git"
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_easybci_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_easybci_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _sync_with_upstream_if_needed(git_cmd: list[str], cwd: Path) -> None:
    ...
    if not has_upstream:
        ...
        print()
        print("ℹ Your fork is not tracking the official EasyBCI repository.")
        print("  This means you may miss updates from zhuyu-cs/EasyBCI-Data.")
        print()
        try:
            response = (
        ...
        if response in {"", "y", "yes"}:
            ...
            if _add_upstream_remote(git_cmd, cwd):
                print(
                    "  ✓ Added upstream: https://github.com/zhuyu-cs/EasyBCI-Data.git"
                )
                has_upstream = True
            else:
            ...
            print(
                "  Skipped. Run 'git remote add upstream https://github.com/zhuyu-cs/EasyBCI-Data.git' to add later."
            )
...
def _invalidate_update_cache():
    ...
    ``easybci update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from easybci_lib.constants import get_default_easybci_root

    default_home = get_default_easybci_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``easybci update`` is still progressing even if pip/uv itself is silent.
    """
...
def _easybci_exe_shims(scripts_dir: Path) -> list[Path]:
    """Entry-point shims that uv may try to rewrite during ``pip install -e .``.

    On POSIX they're regular Python scripts which can be replaced atomically —
    no self-replacement hazard exists. Returns empty on Linux.
    """
    return []


def _quarantine_running_easybci_exe(scripts_dir: Path) -> list[tuple[Path, Path]]:
    """Pre-empt Windows file lock on the running ``easybci.exe``.

    No-op on Linux — POSIX allows atomic replacement of running executables.
    """
    return []


def _restore_quarantined_exes(moved: list[tuple[Path, Path]]) -> None:
    """Roll back ``_quarantine_running_easybci_exe`` if uv didn't write replacements."""
...
def _cleanup_quarantined_exes(scripts_dir: Path | None = None) -> None:
    """Sweep ``easybci.exe.old.*`` left by prior updates.

    No-op on Linux — the quarantine mechanism is Windows-only.
    """
    return
...
def _refresh_active_lazy_features() -> None:
    ...
    first use. ``easybci update`` runs ``uv pip install -e .[all]`` which
    ...
    """
    try:
        from easybci_lib.tools import lazy_deps
    except Exception as exc:
    ...
    if failed:
        ...
        print("  `easybci update` once the upstream issue is resolved.")
    ...
    *,
    ...
    def _install(args: list[str]) -> None:
        ...
        if scripts_dir is not None:
            moved = _quarantine_running_easybci_exe(scripts_dir)
        try:
...
class _UpdateOutputStream:
    """Stream wrapper used during ``easybci update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.easybci/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``easybci update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``easybci update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.easybci/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``easybci update --gateway``) the update is already
    ...
    try:
        ...
        # easybci_cli.config.get_easybci_home to simulate setup failure.
        from easybci_cli.config import get_easybci_home as _get_easybci_home

        logs_dir = _get_easybci_home() / "logs"
...
def _cmd_update_check():
    """Implement ``easybci update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``easybci update`` without requiring a reinstall.  The
    ...
    ``easybci`` prints ``command not found`` even though the symlink lives
    at /usr/local/bin/easybci.

    Silent no-op on: non-Linux, non-root, non-FHS installs, and any system
    where ``bash -i -c 'command -v easybci'`` already resolves.  Idempotent.
    """
    ...
    # /usr/local/bin/easybci, code at /usr/local/lib/easybci-agent).
    fhs_link = Path("/usr/local/bin/easybci")
    ...
    path_comment = (
        "# EasyBCI Agent — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of EASYBCI_HOME before running the update.
    ...
    EASYBCI_HOME directories.  The ``--backup`` flag on ``easybci update``
    ...
    try:
        from easybci_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_easybci_home so the user sees ~/.easybci/...
    try:
        from easybci_lib.constants import get_easybci_home, display_easybci_home

        home = get_easybci_home()
        try:
            display_path = f"{display_easybci_home()}/{out_path.relative_to(home)}"
        except ValueError:
    ...
    print(f"  Restore:  easybci import {out_path}")
...
def cmd_update(args):
    """Update EasyBCI Agent to the latest version.
    ...
    if is_managed():
        managed_error("update EasyBCI Agent")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    # Try git-based update.
    git_dir = PROJECT_ROOT / ".git"

    if not git_dir.exists():
        print("✗ Not a git repository. Please reinstall:")
        print(
            "  curl -fsSL https://raw.githubusercontent.com/zhuyu-cs/EasyBCI-Data/main/scripts/install.sh | bash"
        )
        sys.exit(1)
    ...
    try:
        ...
        try:
            from easybci_cli.backup import create_quick_snapshot
        ...
        # the old bytecode (e.g. get_easybci_home added to easybci_lib.constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # modules in this process.  Reload easybci_lib.constants so that any lazy
        # import executed below (skills sync, gateway restart) sees new
        # attributes like display_easybci_home() added since the last release.
        try:
            import importlib
            import easybci_lib.constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit EASYBCI_HOME so
        # it is not affected by sync_skills()'s module-level EASYBCI_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's EASYBCI_HOME env var points at the default or a named profile.
        try:
            from easybci_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        try:
            from services.plugins.memory.honcho.cli import sync_honcho_profiles_quiet
        ...
        if needs_migration:
            ...
            if response in {"", "y", "yes", "auto"}:
                ...
                if (gateway_mode or assume_yes or response == "auto") and missing_env:
                    print("  ℹ API keys require manual entry: easybci config migrate")
            else:
                print()
                print("Skipped. Run 'easybci config migrate' later to configure.")
        else:
        ...
        # check `easybci curator status`. Self-stamps after printing so it
        ...
        # When running as ``easybci update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_easybci_home() / ".update_exit_code"
            try:
        ...
        try:
            ...
            try:
                from easybci_lib.constants import (
            ...
            # Discover all easybci-gateway* units (default + profiles)
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            unit = parts[
                                0
                            ]  # e.g. easybci-gateway.service or easybci-gateway-coder.service
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: easybci gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: easybci -p <profile> gateway run  for each profile)"
                        )
        ...
        try:
            from easybci_cli.gateway import (
                has_legacy_easybci_units,
                _find_legacy_easybci_units,
                supports_systemd_services,
            )

            if supports_systemd_services() and has_legacy_easybci_units():
                print()
                print("⚠ Legacy EasyBCI gateway unit(s) detected:")
                for name, path, is_sys in _find_legacy_easybci_units():
                    scope = "system" if is_sys else "user"
                    print(f"    {path}  ({scope} scope)")
                print()
                print("  These pre-rename units (easybci.service) fight the current")
                print("  easybci-gateway.service for the bot token and cause SIGTERM")
                print("  flap loops. Remove them with:")
                print()
                print("    easybci gateway migrate-legacy")
                print()
        ...
        print("  easybci model              # Select provider and model")
...
def _coalesce_session_name_args(argv: list) -> list:
    ...
    When a user types ``easybci -c Pokemon Agent Dev`` without quoting the
...
def cmd_profile(args):
    ...
    if action is None:
        # Bare `easybci profile` — show current profile status
        profile_name = get_active_profile_name()
        dhh = display_easybci_home()
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → easybci -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.easybci)")
            else:
        ...
        try:
            ...
            profile_dir = create_profile(
                ...
                no_alias=no_alias,
                no_skills=no_skills,
                share_skills=getattr(args, "share_skills", False),
                share_memories=getattr(args, "share_memories", False),
                share_config=getattr(args, "share_config", False),
            )
            print(f"\nProfile '{name}' created at {profile_dir}")
            if getattr(args, "share_skills", False):
                print(f"  ↳ skills/    → symlinked to ~/.easybci/skills (shared)")
            if getattr(args, "share_memories", False):
                print(f"  ↳ memories/  → symlinked to ~/.easybci/memories (shared)")
            if getattr(args, "share_config", False):
                print(f"  ↳ config.yaml + .env → symlinked to ~/.easybci/ (shared)")
            ...
            if clone or clone_all:
                try:
                    from services.plugins.memory.honcho.cli import clone_honcho_for_profile
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(
                        f"  Choose a custom alias:  easybci profile alias {name} --name <custom>"
                    )
                    print(f"  Or access via flag:     easybci -p {name} chat")
                else:
        ...
        if dist_name:
            ...
            print(f"  (run `easybci profile info {name}` for full manifest)")
        if wrapper.exists():
        ...
        from easybci_cli.profiles import profile_exists
        ...
        from easybci_cli.profiles import rename_profile
        ...
        from easybci_cli.profiles import export_profile
        ...
        from easybci_cli.profiles import import_profile
        ...
        import tempfile
        from easybci_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            with tempfile.TemporaryDirectory(prefix="easybci_dist_preview_") as tmp:
                plan = plan_install(
                    args.source,
                    Path(tmp),
            ...
            print(f"\n  Use with:      easybci -p {plan.manifest.name} chat")
        ...
        from easybci_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if current is None:
                print(
                    ...
                    "Only profiles installed via `easybci profile install` can be updated."
                )
                sys.exit(1)
        ...
        from easybci_cli.profile_distribution import describe_distribution, DistributionError
        ...
        if data.get("easybci_requires"):
            print(f"Requires:     EasyBCI {data['easybci_requires']}")
        if data.get("source"):
...
def _render_distribution_plan(plan) -> None:
    ...
    from easybci_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
    ...
    if mf.easybci_requires:
        print(f"  Requires: EasyBCI {mf.easybci_requires}")
    ...
    if plan.existing:
        ...
        if existing_is_distribution:
            ...
            print(
                ...
                "    overwrite its SOUL.md, skills/, and mcp.json.\n"
...
def _report_dashboard_status() -> int:
    """Print ``easybci dashboard`` PIDs and return the count.

    Uses the same detection logic as ``_find_stale_dashboard_pids`` (the
    current process is excluded, but since ``easybci dashboard --status``
    runs in a short-lived CLI process that never matches the pattern,
    the exclusion is irrelevant here).
    """
    pids = _find_stale_dashboard_pids()
    if not pids:
        print("No easybci dashboard processes running.")
        return 0

    print(f"{len(pids)} easybci dashboard process(es) running:")
    for pid in pids:
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No easybci dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `easybci update`.
    ...
    # --- Resolve dashboard + gateway ports (kill stale, slide on conflict) ---
    def _port_owner_pid(port: int) -> Optional[int]:
        """Return PID of process listening on 127.0.0.1:<port>, or None."""
        try:
            import psutil
        except ImportError:
            return None
        try:
            for conn in psutil.net_connections(kind="inet"):
                if (conn.status == psutil.CONN_LISTEN
                        ...
                        and conn.laddr.ip in ("127.0.0.1", "0.0.0.0", "::", "::1")):
                    return conn.pid
        except (psutil.AccessDenied, OSError):
            pass
        return None

    def _is_easybci_proc(pid: int) -> bool:
        try:
            import psutil
            p = psutil.Process(pid)
            cmdline = " ".join(p.cmdline()).lower()
            return ("easybci" in cmdline) or ("services.gateway.run" in cmdline) or ("run_agent" in cmdline)
        except Exception:
            return False

    def _is_responsive(port: int, role: str) -> bool:
        """Probe whether a port has a live easybci server (responsive HTTP)."""
        import urllib.request, urllib.error
        url = (f"http://127.0.0.1:{port}/health" if role == "Gateway"
    ...
    def _resolve_port(requested: int, role: str, user_explicit: bool) -> int:
        """Pick a usable port: reclaim stale easybci procs, slide on live conflict.

        - Free port → use it.
        - Easybci process holding it AND responsive to HTTP → live concurrent instance,
          slide to next port (don't kill peers!).
        - Easybci process holding it but UNRESPONSIVE → stale/zombie, terminate & reuse.
        - Foreign process → slide (or error if user pinned --port).
        """
        import socket as _sk, time as _time
        for offset in range(0, 50):
            candidate = requested + offset
            owner = _port_owner_pid(candidate)
            if owner is None:
                with _sk.socket(_sk.AF_INET, _sk.SOCK_STREAM) as s:
                    ...
                    try:
                        ...
                        continue
                if offset > 0:
                    print(f"  · {role} port {requested} taken, using {candidate}")
                return candidate
            if _is_easybci_proc(owner):
                if _is_responsive(candidate, role):
                    if user_explicit:
                        raise SystemExit(
                            f"✗ {role} port {candidate} is held by a live easybci instance "
                            f"(PID {owner}). Stop it first or omit --port to use another."
                        )
                    print(f"  · {role} port {candidate} held by live easybci PID {owner}, sliding...")
                    continue
                print(f"  · Found stale easybci {role} (PID {owner}) on :{candidate}, terminating...")
                try:
                    import psutil
                    ...
                    print(f"    ⚠ Could not stop PID {owner}: {_e}")
                _time.sleep(0.4)
                if _port_owner_pid(candidate) is None:
                    return candidate
                continue
            if user_explicit:
                raise SystemExit(
                    f"✗ {role} port {candidate} is in use by foreign PID {owner}. "
                    f"Pick a different --port."
                )
            print(f"  · {role} port {candidate} held by foreign PID {owner}, sliding...")
        raise SystemExit(f"✗ Could not find a free {role} port near {requested}")

    _user_explicit_dash = any(a == "--port" or a.startswith("--port=") for a in sys.argv)
    args.port = _resolve_port(args.port, "Dashboard", _user_explicit_dash)

    # --- Auto-start Gateway if not running (unless --no-gateway) ---
    gateway_proc = None
    if not getattr(args, "no_gateway", False):
        # Check if a live gateway is already running for this EASYBCI_HOME.
        # Gateway enforces single-instance per home via PID file, so multiple
        # dashboard instances must share one gateway (it's stateless / multi-session).
        # BUT — that's only true when the live gateway has the SAME EASYBCI_HOME
        # as us. A profile-scoped dashboard must NOT reuse a gateway tied to a
        # different profile (sessions/logs would land in the wrong home).
        from easybci_lib.constants import get_easybci_home as _get_home_fn
        _my_home = str(_get_home_fn().resolve())

        def _gateway_matches_home(port: int) -> bool:
            try:
                import urllib.request, json as _json
                with urllib.request.urlopen(
                    f"http://127.0.0.1:{port}/health/detailed", timeout=1.5
                ) as r:
                    payload = _json.loads(r.read().decode("utf-8"))
                gw_pid = payload.get("pid")
                if not gw_pid:
                    return False
                env_path = Path(f"/proc/{gw_pid}/environ")
                if not env_path.exists():
                    # Non-Linux or permissions issue — fall back to "trust it"
                    # rather than over-fragmenting; this only affects the rare
                    # profile case anyway.
                    return True
                ...
                gw_home = env.get(b"EASYBCI_HOME", b"").decode("utf-8", "ignore")
                if not gw_home:
                    # Gateway running in default home (no env var set)
                    gw_home = str(Path.home() / ".easybci")
                return str(Path(gw_home).resolve()) == _my_home
            except Exception:
                return False

        _existing_gw_port = None
        for _probe_port in (int(os.environ.get("EASYBCI_GATEWAY_PORT", "8642")), 8642):
            if _is_responsive(_probe_port, "Gateway") and _gateway_matches_home(_probe_port):
                _existing_gw_port = _probe_port
                break

        if _existing_gw_port is not None:
            print(f"  · Reusing live Gateway on :{_existing_gw_port} (same EASYBCI_HOME)")
            gateway_port = _existing_gw_port
            os.environ["EASYBCI_GATEWAY_PORT"] = str(gateway_port)
        else:
            _requested_gateway_port = int(os.environ.get("EASYBCI_GATEWAY_PORT", "8642"))
            _user_explicit_gw = "EASYBCI_GATEWAY_PORT" in os.environ
            gateway_port = _resolve_port(_requested_gateway_port, "Gateway", _user_explicit_gw)
            os.environ["EASYBCI_GATEWAY_PORT"] = str(gateway_port)

            print(f"→ Starting Gateway API server on port {gateway_port}...")
            import subprocess
            from easybci_lib.constants import get_easybci_home as _get_home
            _logs_dir = _get_home() / "logs"
            _logs_dir.mkdir(parents=True, exist_ok=True)
            _gw_stdout = open(_logs_dir / "gateway.stdout.log", "ab")
            _gw_stderr = open(_logs_dir / "gateway.stderr.log", "ab")
            _gw_env = {
                **os.environ,
                "API_SERVER_ENABLED": "true",
                "API_SERVER_PORT": str(gateway_port),
                "API_SERVER_HOST": os.environ.get("EASYBCI_GATEWAY_HOST", "127.0.0.1"),
                "GATEWAY_ALLOW_ALL_USERS": os.environ.get("GATEWAY_ALLOW_ALL_USERS", "true"),
                "API_SERVER_CORS_ORIGINS": os.environ.get(
                    "API_SERVER_CORS_ORIGINS",
                    # Allow all dashboard ports in the slide range (+50) so
                    # multi-instance dashboards can talk to the shared gateway.
                    ",".join(f"http://127.0.0.1:{9119 + i}" for i in range(0, 50))
                    + "," + ",".join(f"http://localhost:{9119 + i}" for i in range(0, 50)),
                ),
            }
            gateway_proc = subprocess.Popen(
                [sys.executable, "-m", "services.gateway.run"],
            ...
            if gateway_proc.poll() is not None:
                print("  ⚠ Gateway failed to start (exit code {}). See {}".format(
                    gateway_proc.returncode, _logs_dir / "gateway.stderr.log"))
                gateway_proc = None
            else:
                print(f"  ✓ Gateway started (PID {gateway_proc.pid})")

    if "EASYBCI_WEB_DIST" not in os.environ and not getattr(args, "skip_build", False):
        ...
        _dist_root = (
            ...
            else PROJECT_ROOT / "easybci_cli" / "web_dist"
        )
    ...
    from easybci_cli.web_server import start_server

    embedded_chat = args.tui or os.environ.get("EASYBCI_DASHBOARD_TUI") == "1"
    try:
        start_server(
            host=args.host,
            port=args.port,
        ...
        if gateway_proc is not None and gateway_proc.poll() is None:
            print("→ Stopping Gateway (PID {})...".format(gateway_proc.pid))
...
def cmd_completion(args, parser=None):
    ...
    from easybci_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    """View and filter EasyBCI log files."""
    from easybci_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from easybci_cli.models import CANONICAL_PROVIDERS as _cp
...
_BUILTIN_SUBCOMMANDS = frozenset(
    {
        ...
        "config", "curator", "dashboard", "debug", "doctor",
        "dump", "fallback", "finalize", "gateway", "hooks", "import", "insights",
        "login", "logout", "logs", "lsp", "mcp", "memory", "migrate-work-dir",
...
# so that in ``easybci -m gpt5 chat``, ``gpt5`` is correctly skipped as a
# flag value rather than misclassified as a subcommand. Kept in sync with
# the top-level flags declared in ``easybci_cli/_parser.py``.
#
...
def _first_positional_argv() -> str | None:
    ...
    ``easybci -m gpt5 --provider openai chat "msg"`` by skipping the
...
def _plugin_cli_discovery_needed() -> bool:
    ...
    if first is None:
        # Bare ``easybci`` or only flags → defaults to ``chat``.
        return False
...
def main():
    ...
    model_parser.add_argument(
        "--portal-url",
        help="Portal base URL for BCI Team login (default: production portal)",
    )
    model_parser.add_argument(
        "--inference-url",
        help="Inference API base URL for BCI Team login (default: production inference API)",
    )
    model_parser.add_argument(
        "--client-id",
        default=None,
        help="OAuth client id to use for BCI Team login (default: easybci-cli)",
    )
    model_parser.add_argument(
        "--scope", default=None, help="OAuth scope to request for BCI Team login"
    )
    model_parser.add_argument(
        "--no-browser",
        action="store_true",
        help="Do not attempt to open the browser automatically during BCI Team login",
    )
    model_parser.add_argument(
        "--timeout",
        type=float,
        default=15.0,
        help="HTTP request timeout in seconds for BCI Team login (default: 15)",
    )
    model_parser.add_argument(
        "--ca-bundle", help="Path to CA bundle PEM file for BCI Team TLS verification"
    )
    model_parser.add_argument(
        "--insecure",
        action="store_true",
        help="Disable TLS verification for BCI Team login (testing only)",
    )
    ...
    # =========================================================================
    from easybci_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `easybci model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    gateway_parser = subparsers.add_parser(
        "gateway",
        help="(Removed) Messaging gateway was removed from this build",
    )
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. BCI Team Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="anthropic",
        help="Upstream provider (default: anthropic). See `easybci proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
    ...
    # =========================================================================
    try:
        from easybci_agent.lsp.cli import register_subparser as _lsp_register
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure EasyBCI Agent with an interactive wizard. "
        "Run a specific section: easybci setup model|terminal|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'easybci setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    login_parser = subparsers.add_parser(
        ...
        description="Run OAuth device authorization flow for EasyBCI CLI",
    )
    ...
    login_parser.add_argument(
        "--client-id", default=None, help="OAuth client id to use (default: easybci-cli)"
    )
    ...
    auth_add.add_argument(
        "provider",
        help="Provider id (for example: anthropic, openrouter)",
    )
    auth_add.add_argument(
        "--type",
        dest="auth_type",
    ...
    auth_add.add_argument("--portal-url", help="BCI Team portal base URL")
    auth_add.add_argument("--inference-url", help="BCI Team inference base URL")
    ...
    auth_spotify = auth_subparsers.add_parser(
        "spotify", help="Authenticate EasyBCI with Spotify via PKCE"
    )
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set EASYBCI_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of EasyBCI Agent components",
    )
    ...
    # =========================================================================
    # finalize command — manually run the mini-repo auto-finalize hook
    # =========================================================================
    finalize_parser = subparsers.add_parser(
        "finalize",
        help="Run the mini-repo finalize hook on an existing work_dir",
        description=(
            "Upgrade a partial work_dir to a contract-conformant mini-repo, "
            "or re-run the auto-finalize hook manually. Idempotent: re-running "
            "with status=ok on an already-ok work_dir is a no-op."
        ),
    )
    finalize_parser.add_argument(
        "work_dir",
        help="Path to the *_preprocess_work_dir/ produced by a preprocessing run",
    )
    finalize_parser.add_argument(
        "--status",
        choices=["ok", "partial", "migrated"],
        default="ok",
        help=(
            "Lifecycle status to stamp. 'ok' (default) runs full consolidation; "
            "'partial' preserves debug artifacts; 'migrated' marks a legacy-layout upgrade."
        ),
    )
    finalize_parser.add_argument(
        "--partial-reason",
        default=None,
        help="Free-form reason recorded alongside --status when not 'ok'",
    )
    finalize_parser.set_defaults(func=cmd_finalize)

    # =========================================================================
    # migrate-work-dir command — bring legacy/violating work_dirs into compliance
    # =========================================================================
    migrate_parser = subparsers.add_parser(
        "migrate-work-dir",
        help="Migrate a legacy *_preprocess_work_dir/ to the contract layout",
        description=(
            "Move scatter scripts (pipeline_v3.py at root) into code/ and binary "
            "outputs (*.pkl) into preprocessed_output/. Generates pipeline_record.json "
            "and README.md if missing. Default is dry-run; pass --apply to execute "
            "(writes a tar.gz backup first unless --no-backup is given). Idempotent."
        ),
    )
    migrate_parser.add_argument(
        "path",
        help=(
            "Path to a *_preprocess_work_dir/, or (with --recursive) a parent "
            "directory containing one or more such dirs"
        ),
    )
    migrate_parser.add_argument(
        "--apply",
    ...
    migrate_parser.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the tar.gz backup before --apply (faster, but no easy rollback)",
    )
    migrate_parser.add_argument(
        "--recursive",
        action="store_true",
        help="Treat 'path' as a parent dir and scan for *_preprocess_work_dir/ children",
    )
    migrate_parser.set_defaults(func=cmd_migrate_work_dir)
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.easybci/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.easybci/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with EasyBCI Agent setup",
    )
    doctor_parser.add_argument(
        ...
        default=None,
        help=(
            "Optional sub-target. Use 'websearch' to focus on the web search "
            "backend (tavily / exa); use 'layout <work_dir>' to verify + "
            "repair a preprocess work_dir; omit for the full system check."
        ),
    )
    doctor_parser.add_argument(
        "work_dir",
        ...
        default=None,
        help=(
            "Path to a *_preprocess_work_dir/ (only used with subject=layout)."
        ),
    )
    doctor_parser.add_argument(
        "--dry-run", action="store_true",
        help="Layout mode: report what would be done without touching disk.",
    )
    doctor_parser.add_argument(
        "--apply", action="store_true",
        help="Layout mode: apply repairs (write to disk).",
    )
    doctor_parser.add_argument(
    ...
    doctor_parser.add_argument(
        ...
        default=None,
        help=(
            "Lazy-install the named web search backend (exa). Only "
            "valid with subject=websearch."
        ),
    )
    doctor_parser.add_argument(
        "--json",
        action="store_true",
        help="Emit machine-readable JSON instead of the colored table.",
    )
    doctor_parser.add_argument(
        "--ack",
        metavar="ADVISORY_ID",
        ...
        help=(
            ...
            "advisory will no longer trigger startup banners. Run `easybci "
    ...
    dump_parser = subparsers.add_parser(
        ...
        description="Output a compact, plain-text summary of your EasyBCI setup "
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for EasyBCI Agent. Use 'easybci debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    easybci debug share              Upload debug report and print URL
    easybci debug share --lines 500  Include more log lines
    easybci debug share --expire 30  Keep paste for 30 days
    easybci debug share --local      Print report locally (no upload)
    easybci debug share --no-redact  Disable upload-time secret redaction
    easybci debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'easybci debug share'",
    )
    ...
    backup_parser = subparsers.add_parser(
        "backup",
        help="Back up EasyBCI home directory to a zip file",
        description="Create a zip archive of your entire EasyBCI configuration, "
        "skills, sessions, and data (excludes the easybci-agent codebase). "
        "Use --quick for a fast snapshot of just critical state files.",
    )
    backup_parser.add_argument(
        "-o",
        "--output",
        help="Output path for the zip file (default: ~/easybci-backup-<timestamp>.zip)",
    )
    backup_parser.add_argument(
        "-q",
        "--quick",
        action="store_true",
        help="Quick snapshot: only critical state files (config, state.db, .env, auth)",
    )
    backup_parser.add_argument(
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.easybci/checkpoints/",
        description="Manage the filesystem checkpoint store — the shadow git "
        "repo easybci uses to snapshot working directories before "
        "write_file/patch/terminal calls. Lets you see how much "
        "space checkpoints occupy, force a prune, or wipe the base.",
    )
    from easybci_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    import_parser = subparsers.add_parser(
        "import",
        help="Restore a EasyBCI backup from a zip file",
        description="Extract a previously created EasyBCI backup into your "
        "EasyBCI home directory, restoring configuration, skills, "
        "sessions, and data",
    )
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage EasyBCI Agent configuration",
    )
    ...
    _layer_group = skills_list.add_mutually_exclusive_group()
    _layer_group.add_argument(
        "--by-layer",
        action="store_true",
        default=True,
        help="Group BCI skills by L0/L1/L2/L3 layer (default). "
        "Uses SKILLS_INDEX.yaml; falls back to flat listing if unavailable.",
    )
    _layer_group.add_argument(
        "--flat",
        action="store_true",
        help="Force flat listing — same output as pre-T9.",
    )
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.easybci/skills/.bundled_manifest) "
            "so future 'easybci update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    # Internalization management subcommands
    skills_inter = skills_subparsers.add_parser(
        "internalizations",
        help="List all internalized knowledge entries",
    )
    skills_inter.add_argument("--json", action="store_true", help="Output JSON")
    skills_inter.set_defaults(func=lambda args: cmd_internalizations_list(args, json_mode=args.json))

    skills_inter_show = skills_subparsers.add_parser(
        "internalization-show",
        help="Show one internalized knowledge entry",
    )
    skills_inter_show.add_argument("id", help="Internalization id (8 hex chars)")
    skills_inter_show.add_argument("--json", action="store_true")
    skills_inter_show.set_defaults(func=cmd_internalization_show)

    skills_revoke = skills_subparsers.add_parser(
        "revoke-internalization",
        help="Revoke an internalized knowledge entry",
    )
    skills_revoke.add_argument("id", help="Internalization id (8 hex chars)")
    skills_revoke.add_argument("--reason", default="", help="Free-text reason")
    skills_revoke.add_argument(
        "--allow-unsafe",
        action="store_true",
        help="Force revoke even when marker body was hand-edited",
    )
    skills_revoke.set_defaults(func=cmd_revoke_internalization)

    def cmd_skills(args):
        # Route 'config' action to skills_config module
        if getattr(args, "skills_action", None) == "config":
            _require_tty("skills config")
            from easybci_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from easybci_cli.skills_hub import skills_command
    ...
    plugins_install.add_argument(
        "identifier",
        help="Git URL or owner/repo shorthand (e.g. anpicasso/easybci-plugin-chrome-profiles)",
    )
    plugins_install.add_argument(
        "--force",
        "-f",
    ...
    _install_enable_group.add_argument(
        "--no-enable",
        action="store_true",
        help="Install disabled (skip confirmation prompt); enable later with `easybci plugins enable <name>`",
    )
    ...
    def cmd_plugins(args):
        from easybci_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``easybci --help``, ``easybci version``, ``easybci logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            from services.plugins.memory import discover_plugin_cli_commands
            from easybci_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from easybci_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_easybci_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_easybci_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_easybci_home()}/memories/\n")
        else:
            from easybci_cli.memory_setup import memory_command
    ...
    tools_parser = subparsers.add_parser(
        ...
        description=(
            "Enable, disable, or list tools for CLI, etc.\n\n"
            "Built-in toolsets use plain names (e.g. web, memory).\n"
            "MCP tools use server:tool notation (e.g. github:create_issue).\n\n"
            "Run 'easybci tools' with no subcommand for the interactive configuration UI."
        ),
    )
    ...
    # easybci tools list [--platform cli]
    ...
    # easybci tools disable <name...> [--platform cli]
    ...
    # easybci tools enable <name...> [--platform cli]
    ...
    def cmd_tools(args):
        ...
        if action in {"list", "disable", "enable"}:
            from easybci_cli.tools_config import tools_disable_enable_command
    ...
    mcp_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "Use 'easybci mcp add' to connect to a new server."
        ),
    )
    ...
    # flag is omitted, causing `easybci mcp add ...` to fall through to
    # interactive chat.
    ...
    def cmd_mcp(args):
        from easybci_cli.mcp_config import mcp_command
    ...
    sessions_list.add_argument(
        "--source", help="Filter by source (cli, api_server)"
    )
    sessions_list.add_argument(
    ...
    def cmd_sessions(args):
        ...
        if action == "list":
            ...
            # Launch easybci --resume <id> by replacing the current process
            print(f"Resuming session: {selected_id}")
            from easybci_cli.relaunch import relaunch
    ...
    insights_parser.add_argument(
        "--source", help="Filter by platform (cli, api_server)"
    )

    def cmd_insights(args):
        try:
            from easybci_lib.state import SessionDB
            from easybci_agent.insights import InsightsEngine
    ...
    claw_parser = subparsers.add_parser(
        ...
        description="Migrate settings, memories, skills, and API keys from OpenClaw to EasyBCI",
    )
    ...
    claw_migrate = claw_subparsers.add_parser(
        "migrate",
        help="Migrate from OpenClaw to EasyBCI",
    ...
    claw_migrate.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Include allowlisted secrets (API keys, etc.). "
        "Required even under --preset full.",
    )
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.easybci/ (by default a "
        "single restore-point archive is written to ~/.easybci/backups/ "
        "before apply; restorable with 'easybci import').",
    )
    claw_migrate.add_argument(
    ...
    def cmd_claw(args):
        from easybci_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update EasyBCI Agent to the latest version",
    ...
    update_parser.add_argument(
        ...
        action="store_true",
        default=False,
        help="Assume yes for interactive prompts (config migration, stash restore). API-key entry is skipped; run 'easybci config migrate' separately for those.",
    )
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall EasyBCI Agent",
        description="Remove EasyBCI Agent from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run EasyBCI Agent as an ACP (Agent Client Protocol) server",
        description="Start EasyBCI Agent in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    ...
    acp_parser.add_argument(
        ...
        action="store_true",
        dest="acp_version",
        help="Print EasyBCI ACP version and exit",
    )
    acp_parser.add_argument(
        "--check",
        action="store_true",
    ...
    acp_parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive EasyBCI provider/model setup for ACP terminal auth",
    )

    def cmd_acp(args):
        """Launch EasyBCI Agent as an ACP server."""
        try:
    ...
    profile_parser = subparsers.add_parser(
        "profile",
        help="Manage profiles — multiple isolated EasyBCI instances",
    )
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `easybci update` skill sync)",
    )
    profile_create.add_argument(
        "--share-skills",
        action="store_true",
        help=(
            "Symlink this profile's skills/ to the global ~/.easybci/skills/ so "
            "proven-pipeline accumulation (the flywheel) is visible across all "
            "profiles using this flag. Mutually exclusive with --no-skills / --clone / --clone-all."
        ),
    )
    profile_create.add_argument(
        "--share-memories",
        action="store_true",
        help=(
            "Symlink this profile's memories/ to the global ~/.easybci/memories/ so "
            "long-term agent memory is shared. Mutually exclusive with --clone / --clone-all."
        ),
    )
    profile_create.add_argument(
        "--share-config",
        action="store_true",
        help=(
            "Symlink this profile's config.yaml and .env to the global root so model / API-key "
            "configuration is shared across instances. Mutually exclusive with --clone / --clone-all."
        ),
    )
    ...
    # Set cohort_tag on a session's data_profile.json
    profile_set_cohort = profile_subparsers.add_parser(
        "set-cohort",
        help="Set cohort_tag for a session's data profile (overrides BIDS participants.tsv)",
    )
    profile_set_cohort.add_argument("session_id", help="Session id from ~/.easybci/sessions/")
    profile_set_cohort.add_argument("tag", help="Cohort tag (e.g. healthy, clinical-stroke)")

    def cmd_profile_set_cohort(args):
        from easybci_lib.constants import get_easybci_home
        import json
        sess_dir = get_easybci_home() / "sessions" / args.session_id
        profile_path = sess_dir / "data_profile.json"
        if not profile_path.exists():
            print(f"[set-cohort] no data_profile.json under {sess_dir}")
            return 1
        try:
            profile = json.loads(profile_path.read_text(encoding="utf-8"))
        except json.JSONDecodeError:
            print(f"[set-cohort] data_profile.json malformed at {profile_path}")
            return 1
        profile["cohort_tag"] = args.tag
        profile_path.write_text(json.dumps(profile, ensure_ascii=False, indent=2), encoding="utf-8")
        print(f"[set-cohort] Set cohort_tag={args.tag} for session {args.session_id}")
        return 0

    profile_set_cohort.set_defaults(func=cmd_profile_set_cohort)

    # Pipelines subcommand group: list, show, stats, flag, unflag.
    pipelines_parser = subparsers.add_parser("pipelines", help="Manage proven-pipeline library")
    pipelines_subparsers = pipelines_parser.add_subparsers(dest="pipelines_action")

    # Pipelines list / show / stats / flag / unflag
    p_list = pipelines_subparsers.add_parser("list", help="List proven pipelines + stats")
    p_list.add_argument("--json", action="store_true")
    p_list.set_defaults(func=cmd_pipelines_list)

    p_show = pipelines_subparsers.add_parser("show", help="Show one pipeline's full metadata")
    p_show.add_argument("name")
    p_show.add_argument("--json", action="store_true")
    p_show.set_defaults(func=cmd_pipelines_show)

    p_stats = pipelines_subparsers.add_parser("stats", help="Library health summary")
    p_stats.add_argument("--json", action="store_true")
    p_stats.set_defaults(func=cmd_pipelines_stats)

    p_flag = pipelines_subparsers.add_parser("flag", help="Manually flag a pipeline as suspicious")
    p_flag.add_argument("name")
    p_flag.add_argument("--reason", default="", help="Free-text reason")
    p_flag.set_defaults(func=cmd_pipelines_flag)

    p_unflag = pipelines_subparsers.add_parser("unflag", help="Remove manual flag")
    p_unflag.add_argument("name")
    p_unflag.set_defaults(func=cmd_pipelines_unflag)

    p_resume = pipelines_subparsers.add_parser(
        "resume",
        help="Check / resume an incomplete preprocessing run in a work_dir",
    )
    p_resume.add_argument(
        "work_dir",
        help="Path to work_dir (must contain code/pipeline.py and middle_process/inputs_routing.json)",
    )
    p_resume.add_argument(
        "--check-only",
        action="store_true",
        help="Report pending entries without running anything",
    )
    p_resume.add_argument(
        "--force",
        action="store_true",
        help="Set EASYBCI_FORCE_REPROCESS=1 in subprocess env (bypass per-file skip)",
    )
    p_resume.add_argument(
        "--timeout",
        type=int,
        default=None,
        help="Wall-clock timeout per stage in seconds (default: 900)",
    )
    p_resume.add_argument("--json", action="store_true", help="Emit raw JSON payload")
    p_resume.set_defaults(func=cmd_pipelines_resume)

    p_migrate = pipelines_subparsers.add_parser(
        "migrate-layout",
        help="Scan a directory tree for *_preprocess_work_dir/ and repair layout drift",
    )
    p_migrate.add_argument("root", type=str)
    mig_mode = p_migrate.add_mutually_exclusive_group(required=True)
    mig_mode.add_argument("--dry-run", action="store_true")
    ...
    reg_check = registry_subparsers.add_parser("check", help="Sweep registry for retracted citations")
    reg_check.add_argument("--operator", default="", help="Only check one operator's params")
    reg_check.add_argument("--paradigm", default="", help="Only check one paradigm")
    reg_check.add_argument("--json", action="store_true")
    reg_check.set_defaults(func=cmd_registry_check)

    # experience: list-negatives / flag-negative / revoke-negative
    experience_parser = subparsers.add_parser(
        "experience", help="Manage experience store (positive + negative)"
    )
    experience_subparsers = experience_parser.add_subparsers(dest="experience_action")

    exp_list_neg = experience_subparsers.add_parser(
        "list-negatives", help="List recorded negative examples",
    )
    exp_list_neg.add_argument("--json", action="store_true")
    exp_list_neg.set_defaults(func=cmd_experience_list_negatives)

    exp_flag = experience_subparsers.add_parser(
        "flag-negative", help="Manually record a negative example",
    )
    exp_flag.add_argument("--modality", required=True)
    exp_flag.add_argument("--paradigm", required=True)
    exp_flag.add_argument("--analysis-goal", default="classification", dest="analysis_goal")
    exp_flag.add_argument("--cohort", default="")
    exp_flag.add_argument("--failed-step", required=True, dest="failed_step")
    exp_flag.add_argument("--params", default="{}", help="JSON dict of step parameters")
    exp_flag.add_argument("--reason", required=True)
    exp_flag.set_defaults(func=cmd_experience_flag_negative)

    exp_revoke = experience_subparsers.add_parser(
        "revoke-negative", help="Remove a negative example by id",
    )
    exp_revoke.add_argument("id")
    exp_revoke.add_argument("--reason", default="")
    exp_revoke.set_defaults(func=cmd_experience_revoke_negative)

    exp_normalize = experience_subparsers.add_parser(
        "normalize-keys",
        help="One-shot: rewrite legacy failed_step strings through canonical_step",
    )
    exp_normalize.set_defaults(func=cmd_experience_normalize_keys)

    # federation: cross-lab keypair + subscriptions + push/pull
    federation_parser = subparsers.add_parser(
        "federation", help="Cross-lab federation",
    )
    fed_subparsers = federation_parser.add_subparsers(dest="federation_action")

    fed_init = fed_subparsers.add_parser("init")
    fed_init.add_argument("--lab-id", dest="lab_id", default="local")
    fed_init.set_defaults(func=cmd_federation_init)

    fed_sub = fed_subparsers.add_parser("subscribe")
    fed_sub.add_argument("source_id")
    fed_sub.add_argument("url")
    fed_sub.add_argument("--transport", default="git", choices=["git", "http"])
    fed_sub.set_defaults(func=cmd_federation_subscribe)

    fed_unsub = fed_subparsers.add_parser("unsubscribe")
    fed_unsub.add_argument("source_id")
    fed_unsub.set_defaults(func=cmd_federation_unsubscribe)

    fed_list = fed_subparsers.add_parser("list-sources")
    fed_list.add_argument("--json", action="store_true")
    fed_list.set_defaults(func=cmd_federation_list_sources)

    fed_pull = fed_subparsers.add_parser("pull")
    fed_pull.add_argument("--source-id", dest="source_id", default="")
    fed_pull.set_defaults(func=cmd_federation_pull)

    fed_push = fed_subparsers.add_parser("push")
    fed_push.add_argument("--source-id", dest="source_id", required=True)
    fed_push.set_defaults(func=cmd_federation_push)

    fed_status = fed_subparsers.add_parser("status")
    fed_status.set_defaults(func=cmd_federation_status)
    ...
    profile_install = profile_subparsers.add_parser(
        ...
        description=(
            "Install a EasyBCI profile distribution. SOURCE can be a git URL "
    ...
    profile_update = profile_subparsers.add_parser(
        ...
        description=(
            ...
            "distribution-owned files (SOUL.md, skills/, mcp.json). "
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the EasyBCI Agent web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `easybci --tui` via PTY/WebSocket). "
            "Alternatively set EASYBCI_DASHBOARD_TUI=1."
        ),
    )
    dashboard_parser.add_argument(
        "--skip-build",
    ...
    # for `easybci dashboard` cmdlines and SIGTERM them directly — the same
    # path `easybci update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
        help="Stop all running easybci dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--status",
        action="store_true",
        help="List running easybci dashboard processes and exit",
    )
    dashboard_parser.add_argument(
        "--no-gateway",
        action="store_true",
        help="Don't auto-start the Gateway API server",
    )
    ...
    logs_parser = subparsers.add_parser(
        "logs",
        help="View and filter EasyBCI log files",
        description="View, tail, and filter agent.log / errors.log / gateway.log",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    easybci logs                    Show last 50 lines of agent.log
    easybci logs -f                 Follow agent.log in real time
    easybci logs errors             Show last 50 lines of errors.log
    easybci logs gateway -n 100     Show last 100 lines of gateway.log
    easybci logs --level WARNING    Only show WARNING and above
    easybci logs --session abc123   Filter by session ID
    easybci logs --component tools  Only show tool-related lines
    easybci logs --since 1h         Lines from the last hour
    easybci logs --since 30m -f     Follow, starting from 30 min ago
    easybci logs list               List available log files with sizes
""",
    )
    ...
    # e.g. ``easybci -c Pokemon Agent Dev`` → ``easybci -c 'Pokemon Agent Dev'``
    ...
    from easybci_cli.config import get_container_exec_info
    ...
    # fails (e.g. 'easybci -c model' where 'model' is consumed as the
    ...
    # gateway status, mcp add, ...) don't pay discovery cost or
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from easybci_cli.plugins import discover_plugins
        ...
        try:
            from easybci_cli.config import load_config
            from easybci_agent.shell_hooks import register_from_config
    ...
    if getattr(args, "oneshot", None):
        from easybci_cli.oneshot import run_oneshot
