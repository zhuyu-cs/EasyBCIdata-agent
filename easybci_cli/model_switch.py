...
- ``easybci_cli.providers``        -- canonical provider identity + overlays
- ``easybci_cli.model_normalize``  -- per-provider name formatting
...
from easybci_agent.models_dev import (
    ModelCapabilities,
    ModelInfo,
...
def is_bci_team_easybci_non_agentic(model_name: str) -> bool:
    """Stub — always returns False. BCI Team models removed."""
    return False


def _check_easybci_model_warning(model_name: str) -> str:
    """Stub — always returns empty string. BCI Team models removed."""
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``easybci config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from easybci_cli.model_switch import
...
def resolve_display_context_length(
    ...
    but provider-enforced limits can be lower (e.g. some providers cap the
    same slug at 272k). The authoritative source is
    ``agent.model_metadata.get_model_context_length`` which falls back to models.dev for the
    rest.
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'easybci model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'easybci doctor' — config issues detected:"
    ...
    # Mirrors the same logic in easybci_cli.runtime_provider.resolve_runtime_provider;
    ...
    easybci_warn = _check_easybci_model_warning(new_model)
    if easybci_warn:
        warnings.append(easybci_warn)
...
def list_authenticated_providers(
    ...
    Uses the curated model lists from easybci_cli/models.py (OPENROUTER_MODELS,
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from easybci_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by easybci provider ID
    ...
    # --- 1. Check EasyBCI-mapped providers ---
    for easybci_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        pconfig = PROVIDER_REGISTRY.get(easybci_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (EASYBCI_OVERLAYS) with proper auth store checking.
        ...
        if not has_creds:
            try:
                ...
                if store and easybci_id in store.get("credential_pool", {}):
                    has_creds = True
            except Exception:
                pass
        if not has_creds:
        ...
        # show up in the picker without requiring a EasyBCI release.
        model_ids = curated.get(easybci_id, [])
        if easybci_id in _MODELS_DEV_PREFERRED:
            model_ids = _merge_with_models_dev(easybci_id, model_ids)
        total = len(model_ids)
    ...
    # --- 2. Check EasyBCI-only providers (opencode-go) ---
    from easybci_cli.providers import EASYBCI_OVERLAYS
    from easybci_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → EasyBCI provider ID.
    _mdev_to_easybci = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in EASYBCI_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve EasyBCI slug
        easybci_slug = _mdev_to_easybci.get(pid, pid)
        if easybci_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(easybci_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, easybci_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or easybci_slug in providers_store):
                    has_creds = True
        ...
        # _seed_from_singletons() imports on demand but aren't in the raw
        # auth.json yet.
        if not has_creds:
            try:
                from easybci_agent.credential_pool import load_pool
                pool = load_pool(easybci_slug)
                if pool.has_credentials():
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", easybci_slug, exc)
        ...
        # configured.
        if not has_creds and easybci_slug == "anthropic":
            try:
                ...
                if (easybci_creds and easybci_creds.get("accessToken")) or \
        ...
        if overlay.auth_type == "aws_sdk":
            try:
                ...
                model_ids = _ids if _ids is not None else (curated.get(easybci_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(easybci_slug, []) or curated.get(pid, [])
        else:
            # Use curated list — look up by EasyBCI slug, fall back to overlay key
            model_ids = curated.get(easybci_slug, []) or curated.get(pid, [])
            # Merge with models.dev for preferred providers (same rationale as above).
            if easybci_slug in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(easybci_slug, model_ids)
        total = len(model_ids)
        top = model_ids[:max_models]

        results.append({
            "slug": easybci_slug,
            "name": get_label(easybci_slug),
            "is_current": easybci_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "easybci",
        ...
        seen_slugs.add(easybci_slug.lower())
        _record_builtin_endpoint(easybci_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or EASYBCI_OVERLAYS (keeps /model in sync
    # with `easybci model`).
    try:
        from easybci_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is EasyBCI's canonical write key (matches
            ...
            # EasyBCI writes ``models:`` as a dict keyed by model id
            # (see easybci_cli/main.py::_save_custom_provider); older
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # EasyBCI's own writer uses; a hyphen variant is accepted
            ...
            # active model. EasyBCI's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, easybci_cli/config.py) already consume that dict.
...
def list_picker_providers(
    ...
    Post-processes the base list so the ``/model`` picker
    only surfaces models that are actually callable in the
    current install:

    - OpenRouter's model list is replaced with the output of
      :func:`easybci_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from easybci_cli.models import fetch_openrouter_models
