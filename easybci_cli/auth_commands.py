...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "qwen-oauth", "google-gemini-cli", "minimax-oauth"}
...
def _provider_base_url(provider: str) -> str:
    ...
    if provider.startswith(CUSTOM_POOL_PREFIX):
        from easybci_agent.credential_pool import _get_custom_provider_config
...
def auth_add_command(args) -> None:
    ...
    if not requested_type:
        if provider.startswith(CUSTOM_POOL_PREFIX):
            ...
            requested_type = AUTH_TYPE_OAUTH if provider in {"anthropic", "qwen-oauth", "google-gemini-cli", "minimax-oauth"} else AUTH_TYPE_API_KEY
    ...
    # (shell-exported vars), claude_code, qwen-cli,
    # device_code, etc.  One consistent re-engagement pattern.
    ...
    if provider == "anthropic":
        from easybci_agent import anthropic_adapter as anthropic_mod

        creds = anthropic_mod.run_easybci_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:easybci_pkce",
    ...
    if provider == "google-gemini-cli":
        from easybci_agent.google_oauth import run_gemini_oauth_login_pure
    ...
    raise SystemExit(f"`easybci auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source EasyBCI reads from
    ...
    from easybci_agent.credential_sources import find_removal_step
    from easybci_cli.auth import suppress_credential_source
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `easybci auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `easybci auth` is called bare."""
    ...
    try:
        from easybci_agent.bedrock_adapter import has_aws_credentials, resolve_aws_auth_env_var, resolve_bedrock_region
