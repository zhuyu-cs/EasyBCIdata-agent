"""
easybci fallback — manage the fallback provider chain.
...
Subcommands:
  easybci fallback [list]   Show the current fallback chain (default when no subcommand)
  easybci fallback add      Pick provider + model via the same picker as `easybci model`,
                           then append the selection to the chain
  easybci fallback remove   Pick an entry to delete from the chain
  easybci fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.easybci/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from easybci_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `easybci model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  easybci fallback add")
        print()
        return
    ...
    print("  Docs: https://github.com/zhuyu-cs/EasyBCI-Data")
    print()
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `easybci model`, then append the selection to the chain."""
    from easybci_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `easybci model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `easybci fallback list` to view, or `easybci fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``easybci fallback [subcommand]``."""
