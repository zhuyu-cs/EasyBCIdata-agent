"""``easybci logs`` — view and filter EasyBCI log files.
...
Usage examples::
    ...
    easybci logs                    # last 50 lines of agent.log
    easybci logs -f                 # follow agent.log in real time
    easybci logs errors             # last 50 lines of errors.log
    easybci logs gateway -n 100    # last 100 lines of gateway.log
    easybci logs --level WARNING    # only WARNING+ lines
    easybci logs --session abc123   # filter by session ID substring
    easybci logs --component tools  # only tool-related lines
    easybci logs --since 1h         # lines from the last hour
    easybci logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_easybci_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print(f"(Logs are created when EasyBCI runs — try 'easybci chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_easybci_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_easybci_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_easybci_home()}/logs/")
        return

    print(f"Log files in {display_easybci_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'easybci chat' to generate logs)")
