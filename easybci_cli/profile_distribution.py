"""Profile distributions — shareable, packaged EasyBCI profiles via git.

A distribution is a EasyBCI profile published as a git repository (or
...
* ``easybci profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``easybci skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``easybci profile``, not a parallel tree):

    easybci profile install <source> [--name N] [--alias] [--force] [--yes]
    easybci profile update  <name>  [--force-config] [--yes]
    easybci profile info    <name>
...
* Distribution-owned paths (SOUL.md, mcp.json, skills/,
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "easybci_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".easybci_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "easybci-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            easybci_requires=str(data.get("easybci_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.easybci_requires:
            out["easybci_requires"] = self.easybci_requires
        if self.author:
...
def check_easybci_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires EasyBCI {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this EasyBCI distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``easybci profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a EasyBCI profile distribution."
            )
...
def plan_install(
    ...
    from easybci_cli import __version__ as easybci_version
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a EasyBCI distribution."
        )

    # Version check up-front so we fail fast
    check_easybci_requires(manifest.easybci_requires, easybci_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.easybci).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="easybci_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `easybci profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`easybci profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="easybci_dist_update_") as tmp:
        plan = plan_install(
