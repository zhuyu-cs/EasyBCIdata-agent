...
into prompt_toolkit's event loop. Each function takes the EasybciCLI instance
...
from easybci_cli.banner import cprint, _DIM, _RST
from easybci_cli.config import save_env_value_secure
...
def prompt_for_secret(cli, var_name: str, prompt: str, metadata=None) -> dict:
    ...
    The secret is stored in ~/.easybci/.env and never exposed to the model.
    """
...
def approval_callback(cli, command: str, description: str) -> str:
    ...
    with lock:
        from easybci_lib.cli import CLI_CONFIG
        timeout = CLI_CONFIG.get("approvals", {}).get("timeout", 0)
        response_queue = queue.Queue()
