"""Shell completion script generation for easybci CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_easybci_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# EasyBCI Agent bash completion
# Add to ~/.bashrc:
#   eval "$(easybci completion bash)"

_easybci_profiles() {{
    local profiles_dir="$HOME/.easybci/profiles"
...
_easybci_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_easybci_profiles)" -- "$cur"))
        return
    fi
...
complete -F _easybci_completion easybci
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef easybci
# EasyBCI Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(easybci completion zsh)"

_easybci_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.easybci/profiles" ]]; then
        profiles+=("${{(@f)$(ls $HOME/.easybci/profiles 2>/dev/null)}}")
    fi
...
_easybci() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_easybci_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'easybci command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# EasyBCI Agent fish completion",
        "# Add to your config:",
        "#   easybci completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __easybci_profiles",
        "    echo default",
        "    if test -d $HOME/.easybci/profiles",
        "        ls $HOME/.easybci/profiles 2>/dev/null",
        "    end",
        "end",
        "",
        "# Disable file completion by default",
        "complete -c easybci -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c easybci -f -s p -l profile"
        " -d 'Profile name' -xa '(__easybci_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__easybci_profiles)' -d 'Profile name'"
                )
