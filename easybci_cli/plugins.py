"""
EasyBCI Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/services/plugins/<name>/`` (shipped with
   easybci-agent; ``memory/`` and ``context_engine/`` subdirs are excluded —
   they have their own discovery paths)
2. **User plugins**   – ``~/.easybci/plugins/<name>/``
3. **Project plugins** – ``./.easybci/plugins/<name>/`` (opt-in via
   ``EASYBCI_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``easybci_agent.plugins``
   entry-point group.
...
from easybci_lib.utils import env_var_enabled
from easybci_cli.config import cfg_get


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``services/plugins/`` directory.

    Honours ``EASYBCI_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("EASYBCI_BUNDLED_PLUGINS")
    if env_override:
        return Path(env_override)
    return Path(__file__).resolve().parent.parent / "services" / "plugins"
...
# ---------------------------------------------------------------------------
#
# Set ``EASYBCI_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.easybci/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("EASYBCI_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When EASYBCI_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other EasyBCI loggers.
    """
    ...
    logger.debug(
        "EASYBCI_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # and for gateway/ACP approvals (TUI, etc.).
...
ENTRY_POINTS_GROUP = "easybci_agent.plugins"

_NS_PARENT = "easybci_plugins"
...
class PluginManifest:
    ...
    #              in ~/.easybci/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``easybci plugins list``. For a flat plugin at
...
class PluginContext:
    ...
    def llm(self) -> Any:
        ...
        if self._llm is None:
            from easybci_agent.plugin_llm import PluginLlm
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``easybci honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``easybci <subcommand>``
        ...
        command with an argument field in platform-native slash command
        pickers. Plugin commands without ``args_hint`` register as
        parameterless and still accept trailing text when invoked
        as free-form chat.
    ...
    def register_web_search_provider(self, provider) -> None:
        ...
        from easybci_agent.web_search_registry import register_provider as _register_web_provider
    ...
    # -- platform adapter registration (removed — messaging platforms deleted) --

    def register_platform(self, *args, **kwargs) -> None:
        """No-op — messaging platform adapters removed."""
        pass
    ...
    def register_skill(
        ...
        ``~/.easybci/skills/`` tree and is **not** listed in the system
        ...
        """
        from easybci_agent.skill_utils import _NAMESPACE_RE
...
class PluginManager:
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        # 1. Bundled plugins (<repo>/services/plugins/<name>/)
        #
        # Repo-shipped plugins live under services/plugins/. Two layouts are
        # supported (see ``_scan_directory`` for details):
        #
        #   - flat: ``services/plugins/disk-cleanup/plugin.yaml`` (standalone)
        #   - category: ``services/plugins/web/exa/plugin.yaml`` (backend)
        #
        # ``memory/``, ``context_engine/``, and ``model-providers/`` are
        # skipped at the top level — they have their own discovery systems
        # (services/plugins/memory/__init__.py, services/providers/__init__.py).
        ...
        # 2. User plugins (~/.easybci/plugins/)
        user_dir = get_easybci_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.easybci/plugins/)
        if _env_enabled("EASYBCI_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".easybci" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set EASYBCI_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        # winner. Keys are path-derived (``web/firecrawl``,
        # ``disk-cleanup``) so plugins don't collide even when both
        ...
        for manifest in winners.values():
            ...
            # Built-in backends auto-load — they ship with easybci and must
            # just work. Selection among them (e.g. which image_gen backend
            # services calls) is driven by ``<category>.provider`` config,
            # enforced by the tool wrapper.
            #
            # Bundled platform plugins (gateway adapters like IRC) auto-load
            # for the same reason: every platform EasyBCI ships must be
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `easybci plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``easybci_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``web/firecrawl``) import as
        ``easybci_plugins.web__firecrawl`` without colliding with plugins
        in other categories.
        """
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="easybci-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``easybci tools`` TUI so plugin-provided toolsets appear
