...
from easybci_cli import auth as auth_mod
from easybci_agent.credential_pool import CredentialPool, PooledCredential, get_custom_provider_pool_key, load_pool
...
from easybci_cli.config import get_compatible_custom_providers, load_config
from easybci_lib.constants import OPENROUTER_BASE_URL
from easybci_lib.utils import base_url_host_matches, base_url_hostname
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "qwen-oauth":
        ...
        # Model-family inference for GPT-5.x / o1-o4: Azure rejects
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from easybci_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Kimi /coding, api.openai.com → responses, api.x.ai →
            # responses).
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = os.getenv("EASYBCI_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    if not requested_norm.startswith("custom:"):
        try:
            ...
            # the canonical provider itself (``bci_team``, ``openrouter``, …) so
            # accidentally shadowing a canonical provider still resolves to
            # the built-in. See tests/easybci_cli/test_runtime_provider_resolution.py
        ...
        for ep_name, entry in providers.items():
            ...
            if requested_norm in {ep_name, name_norm, f"custom:{name_norm}"}:
                ...
                if base_url:
                    ...
                    # silently downgrades responses / anthropic_messages
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'easybci doctor' for details."
        )
        return None
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    # Model-family inference: Azure Foundry deploys GPT-5.x / o1-o4
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'easybci model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.easybci/.env or run 'easybci model' to configure."
        )
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "anthropic":
        ...
        api_key = explicit_api_key
        if not api_key:
            from easybci_agent.anthropic_adapter import resolve_anthropic_token
...
def resolve_runtime_provider(
    *,
    ...
    # Direct custom endpoint short-circuit: when config.yaml has provider=custom
    # with explicit base_url + api_key, skip all complex resolution paths.
    if requested_provider == "custom" and not explicit_api_key and not explicit_base_url:
        _model_cfg = _get_model_config()
        _cfg_base = str(_model_cfg.get("base_url") or "").strip().rstrip("/")
        _cfg_key = str(_model_cfg.get("api_key") or "").strip()
        if _cfg_base and _cfg_key:
            return {
                "provider": "custom",
                "api_mode": _parse_api_mode(_model_cfg.get("api_mode")) or "chat_completions",
                "base_url": _cfg_base,
                "api_key": _cfg_key,
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from easybci_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        # base_url (e.g. openai endpoint) would leak into Anthropic requests.
        cfg_provider = str(model_cfg.get("provider") or "").strip().lower()
        cfg_base_url = ""
        if cfg_provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (EasyBCI canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most EasyBCI-compatible importers).
    ...
    if pconfig and pconfig.auth_type == "api_key":
        ...
        if provider == "xai":
            ...
            if provider in {"opencode-zen", "opencode-go"}:
                ...
                # plus api.openai.com → responses and api.x.ai → responses.
                detected = _detect_api_mode_for_url(base_url)
                if detected:
                    api_mode = detected
