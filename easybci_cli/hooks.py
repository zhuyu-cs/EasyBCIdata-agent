"""easybci hooks — inspect and manage shell-script hooks.

Usage::

    easybci hooks list
    easybci hooks test <event> [--for-tool X] [--payload-file F]
    easybci hooks revoke <command>
    easybci hooks doctor

Consent records live under ``~/.easybci/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.easybci/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``easybci hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: easybci hooks {list|test|revoke|doctor}")
        print("Run 'easybci hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    from easybci_agent import shell_hooks

    specs = shell_hooks.iter_configured_hooks(load_config())

    if not specs:
        print("No shell hooks configured in ~/.easybci/config.yaml.")
        print("See `easybci hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `easybci hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `easybci hooks test` and `easybci hooks doctor`
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (EasyBCI wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `easybci hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `easybci hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `easybci hooks doctor`.")
