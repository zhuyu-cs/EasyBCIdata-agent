"""`easybci checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.easybci/checkpoints/``.  Actions:

    easybci checkpoints               # same as `status`
    easybci checkpoints status        # total size, project count, breakdown
    easybci checkpoints list          # per-project checkpoint counts + workdir
    easybci checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    easybci checkpoints clear [-f]    # nuke the entire base (asks first)
    easybci checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    easybci checkpoints
    easybci checkpoints prune --retention-days 3 --max-size-mb 200
    easybci checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    from easybci_lib.tools.checkpoint_manager import store_status
    ...
    if legacy:
        ...
        print()
        print("Clear with: easybci checkpoints clear-legacy")
    return 0
...
def cmd_prune(args: argparse.Namespace) -> int:
    from easybci_lib.tools.checkpoint_manager import prune_checkpoints
...
def cmd_clear(args: argparse.Namespace) -> int:
    from easybci_lib.tools.checkpoint_manager import CHECKPOINT_BASE, clear_all, store_status
...
def cmd_clear_legacy(args: argparse.Namespace) -> int:
    from easybci_lib.tools.checkpoint_manager import clear_legacy, store_status
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``easybci checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `easybci checkpoints` → status
