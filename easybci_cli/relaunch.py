"""
Unified self-relaunch for EasyBCI CLI.

Preserves critical flags (--tui, --dev, --profile, --model, etc.) across
process replacement so that ``easybci sessions browse`` or post-setup relaunch
doesn't silently drop the user's UI mode or other preferences.

Also works when ``easybci`` is not on PATH (e.g. ``nix run`` or ``python -m``).
"""
...
from easybci_cli._parser import (
...
def _extract_inherited_flags(argv: Sequence[str]) -> list[str]:
    """Pull out flags that should carry over into a self-relaunched easybci."""
    flags: list[str] = []
    i = 0
    while i < len(argv):
        arg = argv[i]
...
def resolve_easybci_bin() -> Optional[str]:
    """Find the easybci entry point.

    Priority:
      1. ``sys.argv[0]`` if it resolves to a real executable.
      2. ``shutil.which("easybci")`` on PATH.
      3. ``None`` → caller should fall back to ``python -m easybci_cli.main``.
    ...
    ``easybci.exe`` on PATH or the ``sys.executable -m easybci_cli.main``
    fallback.
    """
    argv0 = sys.argv[0]
...
def build_relaunch_argv(
    ...
    *,
    ...
    """Construct an argv list for replacing the current process with easybci.
    ...
    Args:
        ...
        original_argv: The original argv to scan for flags (defaults to
            ``sys.argv[1:]``).
    """
    bin_path = resolve_easybci_bin()

    if bin_path:
        argv = [bin_path]
    else:
        argv = [sys.executable, "-m", "easybci_cli.main"]
...
def relaunch(
    ...
    *,
    ...
    """Replace the current process with a fresh easybci invocation.
    ...
    relaunch contract wants: "run easybci again as if the user had typed
    the new argv".
    ...
    *emulates* exec by spawning the child and exiting the parent, but
    ...
    is usually ``easybci.exe`` (a Python console-script shim that wraps
    ``python -m easybci_cli.main``) or a ``.cmd`` batch file, and both
    ...
    That's functionally equivalent — the user sees "easybci exited, then
    new easybci started" — just with two PIDs in play instead of one.
    """
