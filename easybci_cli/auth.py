"""
Multi-provider authentication system for EasyBCI Agent.

Supports traditional API key providers (OpenRouter, custom endpoints). 
Auth state is persisted in
~/.easybci/auth.json with cross-process file locking.
...
from easybci_cli.config import get_easybci_home, get_config_path, read_raw_config
from easybci_lib.constants import OPENROUTER_BASE_URL
from easybci_lib.utils import atomic_replace, atomic_yaml_write, is_truthy_value
...
DEFAULT_OLLAMA_CLOUD_BASE_URL = "http://localhost:11434/v1"
...
SPOTIFY_DOCS_URL = "https://docs.easybci.dev/features/spotify"
...
try:
    from services.providers import list_providers as _list_providers_for_registry
    for _pp in _list_providers_for_registry():
        ...
        # in resolve_provider() (kimi/zai have bespoke token refresh;
        # openrouter/custom are aggregator/user-supplied and handled outside
        # the registry).
        if _pp.name in {"kimi-coding", "kimi-coding-cn", "zai", "openrouter", "custom"}:
            continue
...
def _resolve_api_key_provider_secret(
    ...
    for env_var in pconfig.api_key_env_vars:
        # Check both os.environ and ~/.easybci/.env file
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `easybci model` to re-authenticate."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("EASYBCI_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.easybci/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_easybci_home() / "auth.json"
    # Seat belt: if pytest is running and EASYBCI_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch EASYBCI_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".easybci" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set EASYBCI_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    try:
        ...
        global_root = get_default_easybci_root()
    except Exception:
        return None
    profile_home = get_easybci_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.easybci/auth.json`` even when EASYBCI_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_easybci_root()`` for a profile-shaped EASYBCI_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".easybci" / "auth.json"
            try:
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``easybci auth add <provider>`` inside the profile, profile entries
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in EasyBCI.
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `easybci logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    try:
        ...
        lines = ["Config issue detected — run 'easybci doctor' for full diagnostics:"]
        for ci in issues:
...
def resolve_provider(
    ...
    *,
    ...
    _PROVIDER_ALIASES = {
        ...
        "ollama": "custom", "ollama_cloud": "custom", "ollama-cloud": "custom",
        "vllm": "custom", "llamacpp": "custom",
        "llama.cpp": "custom", "llama-cpp": "custom",
        "local": "custom", "local-llm": "custom",
    }
    ...
    try:
        from services.providers import list_providers as _lp
        for _pp in _lp():
            for _alias in _pp.aliases:
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'easybci model' for available providers, or run 'easybci doctor' to diagnose config issues."
    ...
    try:
        from easybci_agent.bedrock_adapter import has_aws_credentials
    ...
    raise AuthError(
        "No inference provider configured. Run 'easybci model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.easybci/.env.",
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("EASYBCI_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
#
# Tokens live in ~/.easybci/auth/google_oauth.json (managed by agent.google_oauth).
...
def resolve_gemini_oauth_runtime_credentials(
    *,
    ...
    try:
        from easybci_agent.google_oauth import (
            GoogleOAuthError,
            _credentials_path,
        ...
        raise AuthError(
            f"easybci_agent.google_oauth is not importable: {exc}",
...
def get_gemini_oauth_auth_status() -> Dict[str, Any]:
    """Return a status dict for `easybci auth list` / `easybci status`."""
    try:
        from easybci_agent.google_oauth import _credentials_path, load_credentials
    except ImportError:
        return {"logged_in": False, "error": "easybci_agent.google_oauth unavailable"}
...
# Spotify auth — PKCE tokens stored in ~/.easybci/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("EASYBCI_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set EASYBCI_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("EASYBCI_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("EASYBCI_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("EASYBCI_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `easybci auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `easybci auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `easybci auth spotify` first.",
                provider="spotify",
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `easybci auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.easybci/.env, and return it.
    ...
    print("       App name:     anything (e.g. easybci-agent)")
    ...
    # Persist so subsequent `easybci auth spotify` runs skip the wizard.
    save_env_value("EASYBCI_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("EASYBCI_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved EASYBCI_SPOTIFY_CLIENT_ID to ~/.easybci/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "EASYBCI_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize EasyBCI:")
    print(authorize_url)
    print()
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("EASYBCI_CA_BUNDLE")
...
# =============================================================================
# BCI Team Portal — REMOVED (stubs for backward compatibility)
# =============================================================================


def fetch_bci_team_models(
    *,
    api_key: Optional[str] = None,
    base_url: Optional[str] = None,
    timeout_seconds: float = 15.0,
) -> List[str]:
    """Stub — BCI Team Portal support has been removed. Returns empty list."""
    return []


def resolve_bci_team_access_token(
    *,
    timeout_seconds: float = 15.0,
    auth_store: Optional[Dict[str, Any]] = None,
) -> Optional[str]:
    """Stub — BCI Team Portal support has been removed. Returns None."""
    return None


def resolve_bci_team_runtime_credentials(
    *,
    min_key_ttl_seconds: int = 300,
    timeout_seconds: float = 15.0,
    auth_store: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """Stub — BCI Team Portal support has been removed. Returns empty dict."""
    return {}


def get_bci_team_auth_status() -> Dict[str, Any]:
    """Stub — BCI Team Portal support has been removed."""
    return {"logged_in": False, "provider": "bci_team"}


def invalidate_bci_team_auth_status_cache() -> None:
    """Stub — BCI Team Portal support has been removed. No-op."""
    pass


def persist_bci_team_credentials(state: Dict[str, Any], *, label: str = "") -> None:
    """Stub — BCI Team Portal support has been removed. No-op."""
    pass
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not base_url:
        raise AuthError(
            f"No base URL configured for external-process provider '{provider_id}'.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # need a specific api_mode (xai) set it at request-resolution
    # time via `_detect_api_mode_for_url`, so
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `easybci logout` historically keyed off auth.json.active_provider only.
    ...
    was no active auth provider to target, so logout printed "No provider is
    currently logged in" and never reset model.provider.
    """
    return None
...
def _prompt_model_selection(
    ...
    """
    from easybci_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from easybci_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'easybci model' or 'easybci setup' instead."""
    print("The 'easybci login' command has been removed.")
    print("Use 'easybci auth' to manage credentials,")
    print("'easybci model' to select a provider, or 'easybci setup' for full setup.")
    raise SystemExit(0)
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to EasyBCI auth store (~/.easybci/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting EasyBCI login via MiniMax ({region}) OAuth...")
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    if not state or not state.get("access_token"):
        raise AuthError(
            "Not logged into MiniMax OAuth. Run `easybci model` and select "
            "MiniMax (OAuth).",
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("EasyBCI will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `easybci model` or configure an API key to use EasyBCI.")
        else:
