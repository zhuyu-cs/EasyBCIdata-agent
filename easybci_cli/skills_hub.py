#!/usr/bin/env python3
"""
Skills Hub CLI — Unified interface for the EasyBCI Skills Hub.

Powers both:
  - `easybci skills <subcommand>` (CLI argparse entry point)
...
def _resolve_short_name(name: str, sources, console: Console) -> str:
    ...
    """
    from easybci_lib.tools.skills_hub import unified_search
...
def _existing_categories() -> List[str]:
    """Return sorted subdirectory names under ``~/.easybci/skills/`` that look
    ...
    """
    from easybci_lib.tools.skills_hub import SKILLS_DIR
    out: List[str] = []
    try:
...
def _prompt_for_category(c: Console, existing: List[str]) -> str:
    ...
    if existing:
        ...
        c.print(
            "[bold]Category[/] [dim](optional — press Enter to install flat at ~/.easybci/skills/<name>/)[/]"
        )
    try:
...
def do_search(query: str, source: str = "all", limit: int = 10,
    ...
    from easybci_lib.tools.skills_hub import GitHubAuth, create_source_router, unified_search
    ...
    c.print(table)
    c.print("[dim]Use: easybci skills inspect <identifier> to preview, "
            "easybci skills install <identifier> to install[/]\n")
...
def do_browse(page: int = 1, page_size: int = 20, source: str = "all",
    ...
    if official_count > 0 and page == 1:
        c.print(f"[bright_cyan]★ {official_count} official optional skill(s) from BCI Team[/]")
    c.print()
    ...
    c.print("[dim]Tip: 'easybci skills search <query>' searches deeper across all registries[/]\n")
...
def do_install(identifier: str, category: str = "", force: bool = False,
    ...
    from easybci_lib.tools.skills_guard import scan_skill, should_allow_install, format_scan_report
    ...
    if bundle.source == "url" and (not bundle.name or bundle_meta.get("awaiting_name")):
        if name_override and _is_valid_installed_skill_name(name_override):
            ...
            c.print(
                ...
                f"  [bold]easybci skills install {url} --name <your-name>[/]\n\n"
    ...
    try:
        ...
        from easybci_lib.tools.skills_hub import append_audit_log
    ...
    if not force and not skip_confirm:
        ...
        if bundle.source == "official":
            c.print(Panel(
                "[bold bright_cyan]This is an official optional skill maintained by BCI Team.[/]\n\n"
                "It ships with easybci-agent but is not activated by default.\n"
                "Installing will copy it to your skills directory where the agent can use it.\n\n"
                f"Files will be at: [cyan]{display_easybci_home()}/skills/{category + '/' if category else ''}{bundle.name}/[/]",
    ...
    if invalidate_cache:
        ...
        try:
            from easybci_agent.prompt_builder import clear_skills_system_prompt_cache
...
def do_inspect(identifier: str, console: Optional[Console] = None) -> None:
    ...
    from easybci_lib.tools.skills_hub import GitHubAuth, create_source_router
    ...
    if bundle and "SKILL.md" in bundle.files:
        ...
        c.print(Panel(preview, title="SKILL.md Preview", subtitle="easybci skills install <id> to install"))
...
def do_list(source_filter: str = "all",
    ...
    Args:
        ...
        by_layer: If True (default), render the bundled BCI skills in a
            four-layer (L0/L1/L2/L3) view sourced from
            ``SKILLS_INDEX.yaml``. Falls back to the flat listing for
            non-BCI bundled skills and when the index is unavailable.

    Enabled/disabled state is resolved against the currently active profile's
    config — ``easybci -p <profile> skills list`` reads that profile's
    ``skills.disabled`` list because ``-p`` swaps ``EASYBCI_HOME`` at process
    start.  No explicit profile flag needed here.
    """
    c = console or _console
    if by_layer:
        rendered = _render_by_layer(c)
        if not rendered:
            c.print(
                "[dim]Falling back to flat listing — SKILLS_INDEX.yaml not found "
                "or empty. Run `python -m easybci_lib.tools.neural_processing."
                "regen_skills_index` to rebuild it.[/]"
            )
    _do_list_flat(source_filter, enabled_only, c)


def _render_by_layer(c: Console) -> bool:
    """Render the BCI four-layer view from ``SKILLS_INDEX.yaml``.

    Returns True if the rendering succeeded, False to signal callers to fall
    back to the flat listing.
    """
    import yaml as _yaml

    try:
        from easybci_lib.tools.neural_processing import regen_skills_index  # noqa: F401
    except Exception:
        return False
    index_path = Path(__file__).parent.parent / "easybci_lib" / "tools" / "neural_processing" / "SKILLS_INDEX.yaml"
    ...
    layer_titles = {
        "L0": "L0 — IO format loaders",
        "L1": "L1 — Orchestration",
        "L2": "L2 — Paradigm / modality knowledge",
        "L3": "L3 — Atomic operators",
    }
    total = 0
    for layer in ("L0", "L1", "L2", "L3"):
        groups = layers.get(layer) or {}
        if not isinstance(groups, dict) or not groups:
            continue
        for group, entries in groups.items():
            if not isinstance(entries, list) or not entries:
                continue
            table = Table(title=f"{layer_titles.get(layer, layer)}  /  {group}  ({len(entries)})")
            table.add_column("Name", style="bold cyan")
            ...
            for e in entries:
                ...
                total += 1
            c.print(table)
    c.print(f"[dim]{total} bundled BCI skills across the four layers.[/]\n")
    return True


def _do_list_flat(source_filter: str,
                  enabled_only: bool,
                  c: Console) -> None:
    from easybci_lib.tools.skills_hub import HubLockFile, ensure_hub_dirs
    from easybci_lib.tools.skills_sync import _read_manifest
...
def do_check(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    from easybci_lib.tools.skills_hub import check_for_skill_updates
...
def do_update(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    from easybci_lib.tools.skills_hub import HubLockFile, check_for_skill_updates
...
def do_audit(name: Optional[str] = None, console: Optional[Console] = None) -> None:
    ...
    from easybci_lib.tools.skills_hub import HubLockFile, SKILLS_DIR
    from easybci_lib.tools.skills_guard import scan_skill, format_scan_report
...
def do_uninstall(name: str, console: Optional[Console] = None,
    ...
    from easybci_lib.tools.skills_hub import uninstall_skill
...
def do_reset(name: str, restore: bool = False,
    ...
    from easybci_lib.tools.skills_sync import reset_bundled_skill
...
def do_tap(action: str, repo: str = "", console: Optional[Console] = None) -> None:
    ...
    from easybci_lib.tools.skills_hub import TapsManager
    ...
    if action == "list":
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: easybci skills tap add owner/repo\n")
            return
        if mgr.add(repo):
        ...
        if not repo:
            c.print("[bold red]Error:[/] Repo required. Usage: easybci skills tap remove owner/repo\n")
            return
        if mgr.remove(repo):
...
def do_publish(skill_path: str, target: str = "github", repo: str = "",
    ...
    from easybci_lib.tools.skills_hub import GitHubAuth, SKILLS_DIR
    from easybci_lib.tools.skills_guard import scan_skill, format_scan_report
    ...
    if target == "github":
        if not repo:
            c.print("[bold red]Error:[/] --repo required for GitHub publish.\n"
                    "Usage: easybci skills publish <path> --to github --repo owner/repo\n")
            return

        auth = GitHubAuth()
        if not auth.is_authenticated():
            c.print("[bold red]Error:[/] GitHub authentication required.\n"
                    f"Set GITHUB_TOKEN in {display_easybci_home()}/.env or run 'gh auth login'.\n")
            return
    ...
    try:
        resp = httpx.post(
            ...
            json={
                ...
                "body": f"Submitting the `{skill_name}` skill via EasyBCI Skills Hub.\n\n"
                        f"This skill was scanned by the EasyBCI Skills Guard before submission.",
...
def do_snapshot_export(output_path: str, console: Optional[Console] = None) -> None:
    ...
    from easybci_lib.tools.skills_hub import HubLockFile, TapsManager
...
def skills_command(args) -> None:
    """Router for `easybci skills <subcommand>` — called from easybci_cli/main.py."""
    ...
    if action == "browse":
        ...
        do_list(
            ...
            by_layer=(not getattr(args, "flat", False)),
        )
    elif action == "check":
        ...
        if snap_action == "export":
            ...
            _console.print("Usage: easybci skills snapshot [export|import]\n")
    elif action == "tap":
        tap_action = getattr(args, "tap_action", None)
        repo = getattr(args, "repo", "") or getattr(args, "name", "")
        if not tap_action:
            _console.print("Usage: easybci skills tap [list|add|remove]\n")
            return
        do_tap(tap_action, repo=repo)
    else:
        _console.print("Usage: easybci skills [browse|search|install|inspect|list|check|update|audit|uninstall|reset|publish|snapshot|tap]\n")
        _console.print("Run 'easybci skills <command> --help' for details.\n")
