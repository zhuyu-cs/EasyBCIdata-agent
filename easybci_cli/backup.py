"""
Backup and import commands for easybci CLI.

`easybci backup` creates a zip archive of the entire ~/.easybci/ directory
(excluding the easybci-agent repo and transient files).

`easybci import` restores from a backup zip, overlaying onto the current
EASYBCI_HOME root.
"""
...
from easybci_lib.constants import get_default_easybci_root, get_easybci_home, display_easybci_home
...
_EXCLUDED_DIRS = {
    "easybci-agent",     # the codebase repo — re-clone instead
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to easybci root) should be skipped."""
...
def run_backup(args) -> None:
    """Create a zip backup of the EasyBCI home directory."""
    easybci_root = get_default_easybci_root()

    if not easybci_root.is_dir():
        print(f"Error: EasyBCI home directory not found at {easybci_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"easybci-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"easybci-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_easybci_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(easybci_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(easybci_root)
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(easybci_root)

            if _should_exclude(rel):
                continue

            # Skip the output zip itself if it happens to be inside easybci root
            try:
    ...
    print(f"\nRestore with: easybci import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a EasyBCI backup.
    ...
    # Look for telltale files that an easybci home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a EasyBCI backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.easybci/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like an easybci dir name
        if prefix in {".easybci", "easybci"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_easybci_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (easybci_root / "config.yaml").exists()
        has_env = (easybci_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has EasyBCI configuration.")
        ...
        easybci_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            target = easybci_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(easybci_root.resolve())
            except ValueError:
        ...
        profiles_dir = easybci_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # easybci_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print(f"\n  Profiles detected but aliases could not be created.")
                    print(f"  Run: easybci profile list  (after installing easybci)")

        # Guidance
        print()
        if not (easybci_root / "easybci-agent").is_dir():
            print("Note: The easybci-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: easybci update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  easybci -p {pname} gateway install")

        print("Done. Your EasyBCI configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and easybci backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to EASYBCI_HOME).
...
def _quick_snapshot_root(easybci_home: Optional[Path] = None) -> Path:
    home = easybci_home or get_easybci_home()
    return home / _QUICK_SNAPSHOTS_DIR


def create_quick_snapshot(
    label: Optional[str] = None,
    easybci_home: Optional[Path] = None,
) -> Optional[str]:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(easybci_home)
    if not root.exists():
        return []
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(easybci_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for easybci backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_easybci_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, easybci_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``easybci_root`` to ``out_path``.
...
def _pre_update_backup_dir(easybci_home: Optional[Path] = None) -> Path:
    home = easybci_home or get_easybci_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of EASYBCI_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<EASYBCI_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``easybci update``) should continue even if the backup fails.
    """
    easybci_root = easybci_home or get_default_easybci_root()
    if not easybci_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(easybci_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, easybci_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `easybci claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of EASYBCI_HOME under ``backups/`` before a
    ``easybci claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``easybci import <archive>``.  Writes to
    ``<EASYBCI_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `easybci import` and the
