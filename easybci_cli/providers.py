"""
Single source of truth for provider identity in EasyBCI Agent.
...
2. **EasyBCI overlays** — transport type, auth patterns, aggregator flags,
...
# -- EasyBCI overlay ----------------------------------------------------------
# EasyBCI-specific metadata that models.dev doesn't provide.

@dataclass(frozen=True)
class EasybciOverlay:
    """EasyBCI-specific provider metadata layered on top of models.dev."""

    transport: str = "openai_chat"        # openai_chat | anthropic_messages | responses
...
EASYBCI_OVERLAYS: Dict[str, EasybciOverlay] = {
    "openrouter": EasybciOverlay(
        transport="openai_chat",
        is_aggregator=True,
        extra_env_vars=("OPENAI_API_KEY",),
        base_url_env_var="OPENROUTER_BASE_URL",
    ),
    "qwen-oauth": EasybciOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="https://portal.qwen.ai/v1",
        base_url_env_var="EASYBCI_QWEN_BASE_URL",
    ),
    "google-gemini-cli": EasybciOverlay(
        transport="openai_chat",
        auth_type="oauth_external",
        base_url_override="cloudcode-pa://google",
    ),
    "lmstudio": EasybciOverlay(
        transport="openai_chat",
        auth_type="api_key",
    ...
    "kimi-for-coding": EasybciOverlay(
        transport="openai_chat",
        base_url_env_var="KIMI_BASE_URL",
    ),
    "stepfun": EasybciOverlay(
        transport="openai_chat",
        extra_env_vars=("STEPFUN_API_KEY",),
        base_url_override="https://api.stepfun.ai/step_plan/v1",
        base_url_env_var="STEPFUN_BASE_URL",
    ),
    "minimax": EasybciOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_BASE_URL",
    ),
    "minimax-oauth": EasybciOverlay(
        transport="anthropic_messages",
        auth_type="oauth_external",
        base_url_override="https://api.minimax.io/anthropic",
    ),
    "minimax-cn": EasybciOverlay(
        transport="anthropic_messages",
        base_url_env_var="MINIMAX_CN_BASE_URL",
    ),
    "deepseek": EasybciOverlay(
        transport="openai_chat",
        base_url_env_var="DEEPSEEK_BASE_URL",
    ),
    "alibaba": EasybciOverlay(
        transport="openai_chat",
        base_url_env_var="DASHSCOPE_BASE_URL",
    ),
    "alibaba-coding-plan": EasybciOverlay(
        transport="openai_chat",
        base_url_env_var="ALIBABA_CODING_PLAN_BASE_URL",
    ),
    "vercel": EasybciOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ),
    "opencode": EasybciOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="OPENCODE_ZEN_BASE_URL",
    ),
    "opencode-go": EasybciOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "huggingface": EasybciOverlay(
        transport="openai_chat",
        is_aggregator=True,
        base_url_env_var="HF_BASE_URL",
    ),
    "novita": EasybciOverlay(
        transport="openai_chat",
        is_aggregator=True,
    ...
    "nvidia": EasybciOverlay(
        transport="openai_chat",
        base_url_override="https://integrate.api.nvidia.com/v1",
        base_url_env_var="NVIDIA_BASE_URL",
    ),
    "xiaomi": EasybciOverlay(
        transport="openai_chat",
        base_url_env_var="XIAOMI_BASE_URL",
    ),
    "tencent-tokenhub": EasybciOverlay(
        transport="openai_chat",
        base_url_env_var="TOKENHUB_BASE_URL",
    ),
    "arcee": EasybciOverlay(
        transport="openai_chat",
    ...
    "azure-foundry": EasybciOverlay(
        transport="openai_chat",  # default; overridden by api_mode in config
        base_url_env_var="AZURE_FOUNDRY_BASE_URL",
    ),
    "bedrock": EasybciOverlay(
...
class ProviderDef:
    ...
    id: str
    name: str
    transport: str                        # openai_chat | anthropic_messages | responses
    ...
    doc: str = ""
    source: str = ""                      # "models.dev", "easybci", "user-config"
...
ALIASES: Dict[str, str] = {
    ...
    # Local server aliases → route to custom provider
    "lmstudio": "lmstudio",
    "lm-studio": "lmstudio",
    "lm_studio": "lmstudio",
    "ollama": "custom",
...
def get_provider(name: str) -> Optional[ProviderDef]:
    ...
    Resolution order:
      1. EasyBCI overlays (for providers not in models.dev: bci_team, etc.)
      2. models.dev catalog + EasyBCI overlay
    ...
    try:
        from easybci_agent.models_dev import get_provider_info as _mdev_provider
        mdev_info = _mdev_provider(canonical)
    except Exception:
        mdev_info = None

    overlay = EASYBCI_OVERLAYS.get(canonical)
    ...
    if mdev_info is not None:
        ...
        # Combine env vars: models.dev env + easybci extra
    ...
    if overlay is not None:
        # EasyBCI-only provider (not in models.dev)
        return ProviderDef(
            id=canonical,
...
def determine_api_mode(provider: str, base_url: str = "") -> str:
    ...
    # Direct provider checks for providers not in EASYBCI_OVERLAYS
    if provider == "bedrock":
