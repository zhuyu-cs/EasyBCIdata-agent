"""
Security advisory checker for EasyBCI Agent.
...
- **Acknowledgeable.** Once the user has read and acted on an advisory they
  can dismiss it via ``easybci doctor --ack <id>``; the ack is persisted to
...
1. ``easybci doctor`` (and ``easybci doctor --ack <id>``)
...
run in environments where the rest of EasyBCI failed to import.
"""
...
ADVISORIES: tuple[Advisory, ...] = (
    Advisory(
        ...
        summary=(
            ...
            "imported mistralai 2.4.6 — including easybci when configured "
            "with provider=mistral for STT — assume those credentials "
            "are exposed."
        ),
        ...
        remediation=(
            ...
            "Rotate API keys in ~/.easybci/.env (OpenRouter, Anthropic, OpenAI, "
            "GitHub, AWS, Google, Mistral, etc.).",
            ...
            "After cleanup: easybci doctor --ack shai-hulud-2026-05  to dismiss "
            "this warning.",
        ),
...
def _installed_version(pkg_name: str) -> Optional[str]:
    ...
    except ImportError:  # py<3.8 — EasyBCI requires 3.10+ but defensive.
        return None
    try:
...
def short_banner_lines(hits: list[AdvisoryHit]) -> list[str]:
    ...
    lines = [
        ...
        "  Run 'easybci doctor' for remediation steps.",
    ]
    if len(hits) > 1:
...
# ``~/.easybci/cache/advisory_banner_seen`` (a single line per advisory ID:
...
def _banner_cache_path() -> Optional[Path]:
    try:
        ...
        cache_dir = Path(get_easybci_home()) / "cache"
...
def render_doctor_section(hits: list[AdvisoryHit]) -> tuple[bool, list[str]]:
    """Render the security-advisory section for ``easybci doctor``.
...
def gateway_log_message(hits: list[AdvisoryHit]) -> Optional[str]:
    ...
    return (f"{len(fresh)} security advisories active "
            ...
            f"Run `easybci doctor` on the gateway host for details.")
