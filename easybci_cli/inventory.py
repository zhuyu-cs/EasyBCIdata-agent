...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (some provider /models endpoints
  return ~400 IDs including embeddings, rerankers, etc.).
"""
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            else f"run `easybci model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from easybci_cli.models import CANONICAL_PROVIDERS
