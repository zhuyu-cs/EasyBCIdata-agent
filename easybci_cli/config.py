"""
Configuration management for EasyBCI Agent.

Config files are stored in ~/.easybci/ for easy access:
- ~/.easybci/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.easybci/.env         - API keys and secrets

This module provides:
- easybci config          - Show current configuration
- easybci config edit     - Open config in editor
- easybci config set      - Set a specific value
- easybci config wizard   - Re-run setup wizard
"""
...
def _warn_config_parse_failure(config_path: Path, exc: Exception) -> None:
    ...
    A YAML parse error in ``~/.easybci/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``easybci logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  easybci config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    # Activation is via plugins.enabled (opt-in through `easybci plugins enable
    # observability/langfuse` or `easybci tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "EASYBCI_LANGFUSE_ENV",
    "EASYBCI_LANGFUSE_RELEASE",
    "EASYBCI_LANGFUSE_SAMPLE_RATE",
    "EASYBCI_LANGFUSE_MAX_CHARS",
    "EASYBCI_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("EASYBCI_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_easybci_home() / ".managed"
...
def is_managed() -> bool:
    """Check if EasyBCI is running in package-manager-managed mode.

    Two signals: the EASYBCI_MANAGED env var (set by the systemd service),
    or a .managed marker file in EASYBCI_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade easybci-agent"
...
def recommended_update_command() -> str:
    ...
    return get_managed_update_command() or "easybci update"


def format_managed_message(action: str = "modify this EasyBCI installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("EASYBCI_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this EasyBCI installation is managed by NixOS "
            f"(EASYBCI_MANAGED={env_hint}).\n"
            "Edit services.easybci-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this EasyBCI installation is managed by Homebrew "
            f"(EASYBCI_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade easybci-agent"
        )

    return (
        f"Cannot {action}: this EasyBCI installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall EasyBCI."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from EASYBCI_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, easybci_bin
    or None if container mode is not active, we're already inside the
    container, or EASYBCI_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("EASYBCI_DEV") == "1":
        return None

    from easybci_lib.constants import is_container
    if is_container():
        return None

    container_mode_file = get_easybci_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "easybci-agent")
    exec_user = info.get("exec_user", "easybci")
    easybci_bin = info.get("easybci_bin", "/data/current-package/bin/easybci")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "easybci_bin": easybci_bin,
    }
...
# Re-export from easybci_lib.constants — canonical definition lives there.
from easybci_lib.constants import get_easybci_home  # noqa: F811,E402
from easybci_lib.utils import atomic_replace
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the easybci group can
    share state with the gateway service.

    The mode can be overridden via the EASYBCI_HOME_MODE environment variable
    (e.g. EASYBCI_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse EASYBCI_HOME to reach a served subdirectory.
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("EASYBCI_HOME_MODE", "").strip()
...
def _is_container() -> bool:
    ...
    When EasyBCI runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("EASYBCI_CONTAINER") or os.environ.get("EASYBCI_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set EASYBCI_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into EASYBCI_HOME if the user doesn't have one yet."""
...
def ensure_easybci_home():
    """Ensure ~/.easybci directory structure exists with secure permissions.
    ...
    if is_managed():
        ...
        try:
            _ensure_easybci_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
        home.mkdir(parents=True, exist_ok=True)
        _secure_dir(home)
        for subdir in ("sessions", "logs", "logs/curator", "memories"):
            d = home / subdir
...
def _ensure_easybci_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"EASYBCI_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the EasyBCI-level retry loop that wraps the whole call.  Lower
    ...
    "terminal": {
        ...
        # skipped. When empty, EasyBCI auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), EasyBCI sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when EasyBCI
        ...
        #  "/home/user/.easybci/cache/documents:/output"]
        ...
        # bundled EasyBCI image, which drops to the `easybci` user via gosu).
    ...
    # Opt in via ``easybci chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.easybci/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: easybci sweeps the checkpoint base at startup
    ...
    # single read_file page exceeds these limits, EasyBCI truncates the
    ...
    "auxiliary": {
        ...
        # model; override via `easybci model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # When true, `easybci --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `easybci -c` muscle memory.  Default off so existing
        # users aren't surprised.  EASYBCI_TUI_RESUME=<id> always wins.
        ...
        "tool_progress_command": False,  # Enable /verbose command in gateway
        ...
        # that support message deletion (other platforms ignore and leave
        # the message in place).  Only affects slash-command
        ...
        "platforms": {},  # Per-platform display overrides: {"matrix": {"tool_progress": "all"}}
        # Gateway runtime-metadata footer appended to the FINAL message of a turn
        # (disabled by default to keep replies minimal). When enabled, renders
        # e.g. `model · 68% · ~/projects/easybci`. Per-platform overrides go under
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.easybci/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    "delegation": {
        ...
        # narrowing (e.g. toolsets=["web","neural"]) expresses "I want these
        ...
        # without human review (batch automation, etc.).
    ...
    # response. If not, EasyBCI feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before EasyBCI auto-pauses the goal and
    ...
    # always goes to ~/.easybci/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${EASYBCI_SKILL_DIR} and ${EASYBCI_SESSION_ID} in SKILL.md
    ...
    # external daemon.
    #
    # See `easybci curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # skipped), snapshot ~/.easybci/skills/ into
        # ~/.easybci/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `easybci curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for easybci-specific overrides; everything else
    ...
    #   off    — skip all approval prompts (equivalent to --yolo); HARDLINE
    #            destructive commands (rm -rf /, mkfs, dd to /dev/sd*, fork
    #            bomb, shutdown/reboot, kill -1) and the source-data
    #            immutability guard still fire — those cannot be bypassed.
    #
    # Default is "off" so the agent runs preprocessing / generated code
    # straight through without nagging the user. Recoverable shell ops
    # (mv, chmod, curl|sh, git reset --hard, etc.) execute unattended;
    # only catastrophic, unrecoverable commands stay blocked.
    "approvals": {
        "mode": "off",
        "timeout": 60,
        ...
        # through tools.slash_confirm — native yes/no buttons on adapters
        # that override ``send_slash_confirm``; text fallback elsewhere.  Users click "Always
        # Approve" to silence the prompt permanently; that flips this key to
        # false.  TUI has its own modal overlay (EASYBCI_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # stored approval from ~/.easybci/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or EASYBCI_ACCEPT_HOOKS=1.
    # Gateway / non-interactive runs need this (or one of the other
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `easybci doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``easybci_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow EasyBCI to lazy-install opt-in backend packages from PyPI
    ...
    "code_execution": {
        ...
        #     easybci-agent's own python (sys.executable). Maximum isolation
    ...
    # Logging — controls file logging to ~/.easybci/logs/.
    ...
    # update model picker lists without shipping a easybci-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `easybci model` invocation; network failures
    ...
    # Session storage — controls automatic cleanup of ~/.easybci/state.db.
    # state.db accumulates every session, message, tool call, and FTS5 index
    # entry forever.  Without auto-pruning, a heavy user (gateway)
    # reports 384MB+ databases with 68K+ messages, which slows down FTS5
    # inserts, /resume listing, and insights queries.
    "sessions": {
        # When true, prune ended sessions older than retention_days once
        # per (roughly) min_interval_hours at CLI/gateway startup.
        ...
        # default of ``easybci sessions prune``.
        "retention_days": 90,
    ...
    # ``easybci update`` behaviour.
    "updates": {
        # Run a full ``easybci backup``-style zip of EASYBCI_HOME before every
        # ``easybci update``.  Backups land in ``<EASYBCI_HOME>/backups/`` and
        # can be restored with ``easybci import <path>``.  Off by default —
        # on large EASYBCI_HOME directories the zip can add minutes to every
    ...
    "lsp": {
        ...
        #              ``<EASYBCI_HOME>/lsp/bin/`` on first use.
...
OPTIONAL_ENV_VARS = {
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'easybci model' for endpoint-specific config)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix, used to derive vendor hosts",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit access token for tool-gateway requests (optional; otherwise read from the EasyBCI auth store)",
    ...
    # EASYBCI_TOOL_PROGRESS and EASYBCI_TOOL_PROGRESS_MODE are deprecated —
    # now configured via display.tool_progress in config.yaml (off|new|all|verbose).
    # Gateway falls back to these env vars for backward compatibility.
    "EASYBCI_TOOL_PROGRESS": {
    ...
    "EASYBCI_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.easybci.config`` entries, then checks
    which ones are absent or empty under ``skills.config.<key>`` in the user's
    config.yaml.  Returns a list of dicts suitable for prompting.
    """
    try:
        from easybci_agent.skill_utils import discover_all_skill_config_vars, SKILL_CONFIG_PREFIX
    except Exception:
        return []
    ...
    try:
        ...
        # should never break `easybci update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older EasyBCI versions) write ``models`` as
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``easybci_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'easybci setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — EasyBCI won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'easybci doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("EASYBCI_TOOL_PROGRESS")
            old_mode = get_env_value("EASYBCI_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from EASYBCI_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from EASYBCI_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("EASYBCI_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from EASYBCI_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$EASYBCI_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_easybci_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `easybci plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `easybci curator status` has no
    ...
    #   3. Creates `~/.easybci/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_easybci_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_easybci_home() / "logs" / "curator"
    ...
    # ── v23 → v24: Remove dead config sections ──
    # Cleans up cron, browser, messaging toolsets, copilot provider,
    # kanban, session_reset, and platform_toolsets entries that reference
    # removed subsystems.
    if current_ver < 24:
        config = read_raw_config()
        touched = False
        _dead_keys = {"cron", "cron_mode", "browser", "kanban"}
        for dk in _dead_keys:
            if dk in config:
                del config[dk]
                touched = True
        # Remove dead toolset references from platform_toolsets
        _dead_toolsets = {"browser", "cron", "kanban", "todo"}
        platform_ts = config.get("platform_toolsets")
        if isinstance(platform_ts, dict):
            for plat, ts_list in list(platform_ts.items()):
                if isinstance(ts_list, list):
                    cleaned = [t for t in ts_list if t not in _dead_toolsets]
                    if len(cleaned) != len(ts_list):
                        platform_ts[plat] = cleaned
                        touched = True
        # Remove copilot provider config
        providers = config.get("providers")
        if isinstance(providers, dict):
            for dead_p in ("copilot", "copilot-acp", "github-copilot"):
                if dead_p in providers:
                    del providers[dead_p]
                    touched = True
        if touched:
            config["_config_version"] = 24
            save_config(config)
            if not quiet:
                print("  ✓ Cleaned dead config sections (cron, browser, kanban, copilot)")
    ...
    if current_ver < 25:
        ...
        if isinstance(model_cfg, dict):
            if model_cfg.get("api_mode") == "codex_responses":
                model_cfg["api_mode"] = "responses"
                touched = True
        providers = config.get("providers")
        if isinstance(providers, dict):
            for _pname, pcfg in providers.items():
                if isinstance(pcfg, dict) and pcfg.get("api_mode") == "codex_responses":
                    pcfg["api_mode"] = "responses"
                    touched = True
        if touched:
            config["_config_version"] = 25
            save_config(config)
            if not quiet:
                print("  ✓ Renamed api_mode codex_responses → responses")

    # v26: Remove messaging platform config sections (Discord, Telegram, Slack, etc.)
    if current_ver < 26:
        config = read_raw_config()
        touched = False
        _dead_platform_keys = {"discord", "telegram", "slack", "whatsapp", "mattermost", "matrix"}
        for dk in _dead_platform_keys:
            if dk in config:
                del config[dk]
                touched = True
        _dead_messaging_toolsets = {"discord", "discord_admin"}
        platform_ts = config.get("platform_toolsets")
        if isinstance(platform_ts, dict):
            for plat, ts_list in list(platform_ts.items()):
                if isinstance(ts_list, list):
                    cleaned = [t for t in ts_list if t not in _dead_messaging_toolsets]
                    if len(cleaned) != len(ts_list):
                        platform_ts[plat] = cleaned
                        touched = True
            for dead_plat in ("telegram", "discord", "slack", "whatsapp", "mattermost", "matrix"):
                if dead_plat in platform_ts:
                    del platform_ts[dead_plat]
                    touched = True
        if touched:
            config["_config_version"] = 26
            save_config(config)
            if not quiet:
                print("  ✓ Removed messaging platform config (Discord, Telegram, Slack, etc.)")
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: easybci config set <key> <value>")
    ...
    # metadata.easybci.config in their SKILL.md frontmatter.
    ...
    if missing_skill_config and interactive and not quiet:
        ...
        if answer in {"y", "yes"}:
            ...
            config = load_config()
            try:
                from easybci_agent.skill_utils import SKILL_CONFIG_PREFIX
            except Exception:
...
def _deep_merge(base: dict, override: dict) -> dict:
    ...
    recurses, so a user who overrides only a nested key will
    keep sibling defaults intact.
    """
    result = base.copy()
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _easybci_home / "config.yaml"`` local variable
    that appears in gateway/run.py, main.py, etc.
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.easybci/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.easybci/config.yaml.
    ...
    (which change ``EASYBCI_HOME`` and therefore ``get_config_path()``)
    don't collide.
    """
    with _CONFIG_LOCK:
        ensure_easybci_home()
...
def save_config(config: Dict[str, Any]):
    """Save configuration to ~/.easybci/config.yaml."""
    with _CONFIG_LOCK:
        if is_managed():
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.easybci/.env.
    ...
    interactive menu render (`easybci tools`, `easybci setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `easybci tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real EasyBCI env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.easybci/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.easybci/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.easybci/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.easybci/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to EasyBCI — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known EasyBCI vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.easybci/.env or environment."""
...
def show_config():
    ...
    print(color("│              🧠 EasyBCI Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    # Skill config
    try:
        from easybci_agent.skill_utils import discover_all_skill_config_vars, resolve_skill_config_values
    ...
    print(color("  easybci config edit     # Edit config file", Colors.DIM))
    print(color("  easybci config set <key> <value>", Colors.DIM))
    print(color("  easybci setup           # Run setup wizard", Colors.DIM))
    print()
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: easybci config set <key> <value>")
            print()
            print("Examples:")
            print("  easybci config set model anthropic/claude-sonnet-4")
            print("  easybci config set terminal.backend docker")
            print("  easybci config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'easybci config migrate' to add them")
        ...
        print("  easybci config           Show current configuration")
        print("  easybci config edit      Open config in editor")
        print("  easybci config set <key> <value>   Set a config value")
        print("  easybci config check     Check for missing/outdated config")
        print("  easybci config migrate   Update config with new options")
        print("  easybci config path      Show config file path")
        print("  easybci config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``easybci config`` UI so users
...
def _inject_platform_plugin_env_vars() -> None:
    ...
    try:
        ...
        # injector works regardless of CWD. Plugins live under
        # ``<repo>/services/plugins/`` after the 2026 architecture refactor.
        repo_root = Path(__file__).resolve().parents[1]
        platforms_dir = repo_root / "services" / "plugins" / "platforms"
