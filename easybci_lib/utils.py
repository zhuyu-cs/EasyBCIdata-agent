"""Shared utility functions for easybci-agent."""
...
def atomic_text_write(path: Union[str, Path], content: str) -> None:
    """Write *content* to *path* atomically (temp file + fsync + os.replace).

    If the process crashes or the disk fills mid-write, the previous version of
    the file stays intact instead of being left truncated, and a concurrent
    reader never observes a half-written file. Preserves symlinks and file mode,
    mirroring ``atomic_json_write``.
    """
    path = Path(path)
    path.parent.mkdir(parents=True, exist_ok=True)
