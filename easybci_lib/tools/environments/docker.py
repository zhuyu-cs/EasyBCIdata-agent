...
from easybci_lib.tools.environments.base import BaseEnvironment, _popen_bash
from easybci_lib.tools.environments.local import _EASYBCI_PROVIDER_ENV_BLOCKLIST
...
def _load_easybci_env_vars() -> dict[str, str]:
    """Load ~/.easybci/.env values without failing Docker command execution."""
    try:
        from easybci_cli.config import load_env
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``EASYBCI_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("EASYBCI_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using EASYBCI_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image entrypoint drops from root to the 'easybci'
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.easybci/sandboxes/). Non-persistent
        # mode uses tmpfs (ephemeral, fast, gone on cleanup).
        from easybci_lib.tools.environments.base import get_sandbox_dir
        ...
        try:
            from easybci_lib.tools.credential_files import (
        ...
        container_name = f"easybci-{uuid.uuid4().hex[:8]}"
        run_cmd = [
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        try:
            from easybci_lib.tools.env_passthrough import get_all_passthrough
            passthrough_keys = set(get_all_passthrough())
        except Exception:
            pass
        # Explicit docker_forward_env entries are an intentional opt-in and must
        # win over the generic EasyBCI secret blocklist. Only implicit passthrough
        # keys are filtered.
        forward_keys = explicit_forward_keys | (passthrough_keys - _EASYBCI_PROVIDER_ENV_BLOCKLIST)
        easybci_env = _load_easybci_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if value is None:
                value = easybci_env.get(key)
        ...
        if self._container_id:
            ...
            def _stop_then_force_rm():
                try:
                    ...
                    logger.warning("Background stop failed for %s: %s", container_id, e)

            try:
                threading.Thread(target=_stop_then_force_rm, daemon=True).start()
            except Exception as e:
                logger.warning("Failed to schedule stop for container %s: %s", container_id, e)

            if not persistent:
                def _delayed_force_rm():
                    try:
                        subprocess.run(
                            [docker_exe, "rm", "-f", container_id],
                            capture_output=True, timeout=10,
                        ...
                        pass
                try:
                    threading.Timer(3.0, _delayed_force_rm).start()
                except Exception:
                    pass
