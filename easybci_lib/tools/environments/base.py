"""Base class for all EasyBCI execution environment backends.
...
# EASYBCI_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("EASYBCI_DEBUG_INTERRUPT"))
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {EASYBCI_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_easybci_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__EASYBCI_CWD_{session_id}__"


# ---------------------------------------------------------------------------
# Path-guard scatter detection — flag tool calls that write outside the
# canonical mini-repo work_dir so the LLM can't accidentally pollute the
# user's filesystem.
# ---------------------------------------------------------------------------

_SCATTER_EXTS = (".py", ".pkl", ".npz")


def _find_work_dir_for_cwd(cwd: str | None) -> Path | None:
    """If ``cwd`` is inside a ``*_preprocess_work_dir``, return that ancestor."""
    if not cwd:
        return None
    try:
...
def _snapshot_work_dir_root(cwd: str | None) -> tuple[Path | None, set[str]]:
    """Return (work_dir, set_of_top_level_filenames) or (None, set())."""
    wd = _find_work_dir_for_cwd(cwd)
    if wd is None or not wd.exists():
        return None, set()
    try:
        names = {
            entry.name for entry in wd.iterdir()
            if entry.is_file() and (
                entry.name.endswith(_SCATTER_EXTS)
                or entry.name.endswith(".meta.json")
            )
        }
    except Exception:
        names = set()
...
def _scatter_warning_for_diff(
    pre: tuple[Path | None, set[str]],
    cwd: str | None,
) -> str | None:
    """If the post snapshot has new scatter files, return a warning message."""
    wd, pre_names = pre
    if wd is None:
        return None
    try:
    ...
    for name in new:
        if name.endswith(".py"):
            moves.append(f"  - {name}  →  code/{name}")
        else:
            moves.append(f"  - {name}  →  preprocessed_output/{name}")
    return (
        "[path_guard] terminal command produced scatter files at the work_dir "
        "root.\nMove them to canonical locations:\n" + "\n".join(moves) + "\n"
        "Always write scripts to <work_dir>/code/pipeline.py and binary "
        "outputs to <work_dir>/preprocessed_output/."
    )
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all EasyBCI backends.
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/easybci-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/easybci-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # caused ``C:/Users/.../easybci-snap-*.sh: No such file or directory``
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        parts.append("exit $__easybci_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"EASYBCI_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(self, proc: ProcessHandle, timeout: int = 120) -> dict:
        ...
        # --- Debug tracing (opt-in via EASYBCI_DEBUG_INTERRUPT=1) -------------
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __EASYBCI_CWD_{session}__ marker from stdout output.
    ...
    def execute(
        ...
        *,
        ...
        from easybci_lib.tools.terminal_tool import _rewrite_compound_background
        ...
        # Snapshot work_dir top-level files BEFORE so we can detect newly
        # created scatter files (pipeline_v3.py, *.pkl at root) and warn the
        # LLM to use canonical paths. Snapshot work_dir before tool dispatch
        # so we can diff against the post-dispatch state.
        _pre_snapshot = _snapshot_work_dir_root(effective_cwd)
        ...
        # Compare post-snapshot; if new top-level scatter files appeared,
        # append a warning to result['output'] (advisory only — terminal is
        # too unpredictable to auto-move).
        _scatter_warning = _scatter_warning_for_diff(_pre_snapshot, effective_cwd)
        if _scatter_warning and isinstance(result, dict):
            existing = result.get("output") or ""
            result["output"] = existing + "\n\n" + _scatter_warning
            result["path_guard_warning"] = _scatter_warning
    ...
    def _prepare_command(self, command: str) -> tuple[str, str | None]:
        ...
        from easybci_lib.tools.terminal_tool import _transform_sudo_command
