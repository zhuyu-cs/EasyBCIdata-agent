...
from easybci_lib.tools.environments.modal_utils import (
    BaseModalExecutionEnvironment,
    ModalExecStart,
    PreparedModalExec,
)
from easybci_lib.tools.managed_tool_gateway import resolve_managed_tool_gateway
...
class ManagedModalEnvironment(BaseModalExecutionEnvironment):
    """Gateway-owned Modal sandbox with EasyBCI-compatible execute/cleanup."""
    ...
    def __init__(
        ...
        if gateway is None:
            raise ValueError("Managed Modal requires a configured tool gateway")

        self._gateway_origin = gateway.gateway_origin.rstrip("/")
        self._user_token = gateway.user_token
    ...
    def _guard_unsupported_credential_passthrough(self) -> None:
        ...
        try:
            from easybci_lib.tools.credential_files import get_credential_file_mounts
        except Exception:
            return
