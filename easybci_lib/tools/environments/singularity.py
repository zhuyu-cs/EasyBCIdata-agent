...
_SNAPSHOT_STORE = get_easybci_home() / "singularity_snapshots.json"
...
def _get_scratch_dir() -> Path:
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "easybci") / "easybci-agent"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        self.instance_id = f"easybci_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "easybci-overlays"
    ...
    def _start_instance(self):
        ...
        try:
            from easybci_lib.tools.credential_files import get_credential_file_mounts, get_skills_directory_mount
