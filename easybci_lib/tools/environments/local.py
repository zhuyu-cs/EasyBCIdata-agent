...
from easybci_lib.tools.environments.base import BaseEnvironment, _pipe_stdin
...
# EasyBCI-internal env vars that should NOT leak into terminal subprocesses.
_EASYBCI_PROVIDER_ENV_FORCE_PREFIX = "_EASYBCI_FORCE_"
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from easybci_cli.config import OPTIONAL_ENV_VARS
...
_EASYBCI_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()


def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter EasyBCI-managed secrets from a subprocess environment."""
    try:
        from easybci_lib.tools.env_passthrough import is_env_passthrough as _is_passthrough
    except Exception:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_EASYBCI_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if key not in _EASYBCI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_EASYBCI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_EASYBCI_PROVIDER_ENV_FORCE_PREFIX):]
            sanitized[real_key] = value
        elif key not in _EASYBCI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_EASYBCI_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_EASYBCI_PROVIDER_ENV_FORCE_PREFIX):]
            run_env[real_key] = v
        elif k not in _EASYBCI_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    # npm …) into {EASYBCI_HOME}/home/ when that directory exists.  Only the
    ...
    try:
        from services.gateway.session_context import get_session_env, _UNSET, _VAR_MAP
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, EasyBCI trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        try:
            proc._easybci_pgid = os.getpgid(proc.pid)
    ...
    def _kill_process(self, proc):
        ...
        try:
            try:
                ...
                pgid = getattr(proc, "_easybci_pgid", None)
                if pgid is None:
                    raise
