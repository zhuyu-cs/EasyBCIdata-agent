...
from easybci_lib.tools.environments.base import _file_mtime_key
...
_FORCE_SYNC_ENV = "EASYBCI_FORCE_FILE_SYNC"
...
def iter_sync_files(container_base: str = "/root/.easybci") -> list[tuple[str, str]]:
    ...
    the hardcoded /root/.easybci to *container_base* because the remote
...
class FileSyncManager:
    ...
    def sync(self, *, force: bool = False) -> None:
        ...
        or ``EASYBCI_FORCE_FILE_SYNC=1`` is set.
    ...
    def sync_back(self, easybci_home: Path | None = None) -> None:
        """Pull remote changes back to the host filesystem.

        Downloads the remote ``.easybci/`` directory as a tar archive,
    ...
    def _sync_back_impl(self) -> None:
        ...
        with tempfile.NamedTemporaryFile(suffix=".tar") as tf:
            ...
            with tempfile.TemporaryDirectory(prefix="easybci-sync-back-") as staging:
    ...
    def _infer_host_path(self, remote_path: str,
        ...
        For example, if the mapping has ``/root/.easybci/skills/a.md`` →
        ``~/.easybci/skills/a.md``, a new remote file at
        ``/root/.easybci/skills/b.md`` maps to ``~/.easybci/skills/b.md``.
        """
        mapping = file_mapping if file_mapping is not None else []
        for host, remote in mapping:
