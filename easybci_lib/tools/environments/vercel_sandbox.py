...
Uses the Vercel Python SDK to run commands in cloud sandboxes through EasyBCI'
shared ``BaseEnvironment`` shell contract. When persistence is enabled, the
backend stores task-scoped snapshot metadata under ``EASYBCI_HOME`` and restores
...
def _snapshot_store_path() -> Path:
    return get_easybci_home() / _SNAPSHOT_STORE_NAME
...
class VercelSandboxEnvironment(BaseEnvironment):
    ...
    def _configure_attached_sandbox(self, *, requested_cwd: str) -> None:
        ...
        if self._remote_home == "/":
            container_base = "/.easybci"
        else:
            container_base = f"{self._remote_home.rstrip('/')}/.easybci"
    ...
    def _vercel_bulk_download(self, dest_tar_path: Path) -> None:
        remote_easybci = (
            "/.easybci"
            if self._remote_home == "/"
            else f"{self._remote_home.rstrip('/')}/.easybci"
        )
        archive_member = remote_easybci.lstrip("/")
        remote_tar = f"/tmp/.easybci_sync.{os.getpid()}.tar"
        sandbox = self._sandbox
        if sandbox is None:
            raise RuntimeError("Vercel sandbox is not attached")
