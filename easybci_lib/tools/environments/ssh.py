...
from easybci_lib.tools.environments.file_sync import (
    FileSyncManager,
    iter_sync_files,
...
class SSHEnvironment(BaseEnvironment):
    ...
    def __init__(self, host: str, user: str, cwd: str = "~",
        ...
        self.control_dir = Path(tempfile.gettempdir()) / "easybci-ssh"
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files(f"{self._remote_home}/.easybci"),
    ...
    def _ensure_remote_dirs(self) -> None:
        """Create base ~/.easybci directory tree on remote in one SSH call."""
        base = f"{self._remote_home}/.easybci"
    ...
    def _scp_upload(self, host_path: str, remote_path: str) -> None:
        ...
        scp_cmd.extend([host_path, f"{self.user}@{self.host}:{shlex.quote(remote_path)}"])
    ...
    def _ssh_bulk_upload(self, files: list[tuple[str, str]]) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="easybci-ssh-bulk-") as staging:
    ...
    def _ssh_bulk_download(self, dest: Path) -> None:
        """Download remote .easybci/ as a tar archive."""
        # Tar from / with the full path so archive entries preserve absolute
        # paths (e.g. home/user/.easybci/skills/f.py), matching _pushed_hashes keys.
        rel_base = f"{self._remote_home}/.easybci".lstrip("/")
        ssh_cmd = self._build_ssh_command()
