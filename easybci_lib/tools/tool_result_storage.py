...
2. **Per-result persistence** (maybe_persist_tool_result): After a tool
   ...
   SANDBOX temp dir (for example /tmp/easybci-results/{tool_use_id}.txt on
   standard Linux, or $TMPDIR/easybci-results/{tool_use_id}.txt on Termux)
...
from easybci_lib.tools.budget_config import (
...
STORAGE_DIR = "/tmp/easybci-results"
HEREDOC_MARKER = "EASYBCI_PERSIST_EOF"
        ...
        if callable(get_temp_dir):
            try:
                ...
                if temp_dir:
                    ...
                    return f"{temp_dir}/easybci-results"
    return STORAGE_DIR
...
def _heredoc_marker(content: str) -> str:
    ...
    return f"EASYBCI_PERSIST_{uuid.uuid4().hex[:8]}"
