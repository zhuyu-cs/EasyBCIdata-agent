...
transport, discovers their tools, and registers them into the easybci-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.easybci/config.yaml under the ``mcp_servers`` key.
...
# banners, server JSON startup logs, etc.) writes directly onto
...
# per-profile log file (~/.easybci/logs/mcp-stderr.log), tagged with the
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            easybci_home = os.path.expanduser(
                os.getenv(
                    "EASYBCI_HOME", os.path.join(os.path.expanduser("~"), ".easybci")
                )
            )
            candidates = [
                os.path.join(easybci_home, "node", "bin", resolved_command),
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → EasyBCI MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that EasyBCI gateways know how to render.
    ...
    try:
        from services.gateway.platforms.base import cache_image_from_bytes
        ...
        # gateway.platforms.base not importable in this process. Fall back to silently dropping — callers
...
class MCPServerTask:
    ...
    async def _run_stdio(self, config: dict):
        ...
        from easybci_lib.tools.osv_check import check_package_for_malware
        ...
        # (FastMCP banners, server startup JSON, etc.) don't dump onto
        # the user's TTY and corrupt the TUI.  Preserves debuggability via
        # ~/.easybci/logs/mcp-stderr.log.
    ...
    async def _run_http(self, config: dict):
        ...
        if self._auth_type == "oauth":
            try:
                from easybci_lib.tools.mcp_oauth_manager import get_manager
...
def _get_auth_error_types() -> tuple:
    ...
    try:
        ...
        pass
    try:
        from easybci_lib.tools.mcp_oauth import OAuthNonInteractiveError
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `easybci mcp login {server_name}` (or delete the tokens "
            f"file under ~/.easybci/mcp-tokens/ and restart). Do NOT retry "
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the EasyBCI config file.
    ...
    ``os.environ`` (which includes ``~/.easybci/.env`` loaded at startup).
    """
    try:
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through EasyBCI' MEDIA: tag convention and out to messaging
...
def _normalize_mcp_input_schema(schema: dict | None) -> dict:
    ...
    * MCP/Pydantic optional fields commonly arrive as
    ...
    def _strip_nullable_union(node):
        ...
        """
        from easybci_lib.tools.schema_sanitizer import strip_nullable_unions
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves EasyBCI's historical behavior of converting hyphens to
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the EasyBCI registry schema format.
...
def register_mcp_servers(servers: Dict[str, dict]) -> List[str]:
    ...
    from easybci_lib.tools.interrupt import is_interrupted as _is_interrupted, set_interrupt as _set_interrupt
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``easybci tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the EasyBCI registry.
...
def _kill_orphaned_mcp_children(include_active: bool = False) -> None:
    ...
    the parent cgroup on cancellation).

    Sends SIGTERM, waits 2 seconds, then escalates to SIGKILL for any
    survivors, avoiding shared-resource collisions when multiple easybci
