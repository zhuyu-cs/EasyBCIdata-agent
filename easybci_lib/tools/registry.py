"""Central registry for all easybci-agent tools.
...
Import chain (circular-import safe):
    easybci_lib/tools/registry.py  (no imports from model_tools or tool files)
           ^
    easybci_lib/tools/*.py  (import from easybci_lib.tools.registry at module level)
           ^
    model_tools.py  (imports easybci_lib.tools.registry + all tool modules)
           ^
...
def discover_builtin_tools(tools_dir: Optional[Path] = None) -> List[str]:
    ...
    Failures are logged loudly (logger.exception + stderr write) so that a
    silently-skipped neural-tool import doesn't leave the gateway running with
    a half-populated registry. ``_LAST_DISCOVERY_FAILURES`` keeps the most
    recent failure list for :func:`verify_required_tools` / :func:`repair_registry`.
    """
    global _LAST_DISCOVERY_FAILURES
    tools_path = Path(tools_dir) if tools_dir is not None else Path(__file__).resolve().parent
    module_names = [
        f"easybci_lib.tools.{path.stem}"
    ...
    for mod_name in module_names:
        try:
            ...
            # `logger.exception` includes traceback for whatever logging
            # handlers DO exist (agent.log if configured). The stderr write
            # is the belt-and-suspenders backup — this function runs at
            # `easybci_lib.model_tools` import time, BEFORE the gateway's
            # logging is initialised, so `logger.exception` alone may write
            # nowhere. The print survives that window and lands in
            # `~/.easybci/logs/gateway.stderr.log`.
            logger.exception("Could not import tool module %s: %s", mod_name, e)
            try:
                print(
                    f"[easybci registry] FAILED to import tool module "
                    f"{mod_name}: {type(e).__name__}: {e}\n"
...
# Module-level snapshot of the last `discover_builtin_tools` failure list.
# Populated on every call. Consumed by `verify_required_tools` /
# `repair_registry` so callers can ask "what blew up?" without re-running the
# discovery side effect.
_LAST_DISCOVERY_FAILURES: List[Tuple[str, str]] = []


def get_last_discovery_failures() -> List[Tuple[str, str]]:
    """Return [(module_name, error_string), ...] from the most recent
    ``discover_builtin_tools()`` invocation. Empty list if all imports
    succeeded (or discovery hasn't run yet)."""
    return list(_LAST_DISCOVERY_FAILURES)


def verify_required_tools() -> List[str]:
    """Compare the live registry against the canonical core tool list.

    Returns the sorted list of missing tool names. Empty list = healthy.

    The expected set is ``easybci_lib.toolsets._EASYBCI_CORE_TOOLS``, the
    single source of truth shared by the CLI / WebUI / `_NEURAL_ORCHESTRATION_TOOLS`
    bundles. Drift is intentionally fail-loud: when the toolsets file grows a
    new core tool, this check starts flagging gateways that didn't restart to
    pick it up. MCP / plugin tools are out of scope — they're optional and
    dynamically discovered.
    """
    try:
        from easybci_lib.toolsets import _EASYBCI_CORE_TOOLS
    except Exception:  # pragma: no cover — defensive
        logger.exception("verify_required_tools: cannot import _EASYBCI_CORE_TOOLS")
        return []
    expected = set(_EASYBCI_CORE_TOOLS)
    registered = set(registry.get_all_tool_names())
    return sorted(expected - registered)


def repair_registry() -> List[str]:
    """Re-run ``discover_builtin_tools()`` to retry previously-failed module
    imports.

    Python's import machinery caches modules in ``sys.modules`` on success, so
    re-importing a successfully-loaded module is a no-op (no double-register).
    A module that raised mid-import is NOT cached, so this call genuinely
    retries it. Returns the list of modules that imported successfully on
    this attempt (useful for logging).
    """
...
# timescales. Cache results for ~30 s so env-var flips via ``easybci tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``easybci tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``easybci tools enable foo``)
    ...
    def get_max_result_size(self, name: str, default: int | float | None = None) -> int | float:
        ...
        if default is not None:
            return default
        from easybci_lib.tools.budget_config import DEFAULT_RESULT_SIZE_CHARS
...
#
# Usage:
#   from easybci_lib.tools.registry import registry, tool_error, tool_result
#
