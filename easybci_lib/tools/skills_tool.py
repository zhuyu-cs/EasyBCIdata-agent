...
Usage:
    from easybci_lib.tools.skills_tool import skills_list, skill_view, check_skills_requirements
...
# All skills live in ~/.easybci/skills/ (seeded from bundled skills/ on install).
...
def load_env() -> Dict[str, str]:
    """Load profile-scoped environment variables from EASYBCI_HOME/.env."""
    env_path = get_easybci_home() / ".env"
...
def skill_matches_platform(frontmatter: Dict[str, Any]) -> bool:
    ...
    """
    from easybci_agent.skill_utils import skill_matches_platform as _impl
...
def _is_gateway_surface() -> bool:
    ...
    return bool(get_session_env("EASYBCI_SESSION_PLATFORM"))
...
def _gateway_setup_hint() -> str:
    try:
        from services.gateway.platforms.base import GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE

        return GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE
    except Exception:
        return f"Secure secret entry is not available. Load this skill in the local CLI to be prompted, or add the key to {display_easybci_home()}/.env manually."
...
def _parse_frontmatter(content: str) -> Tuple[Dict[str, Any], str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from easybci_agent.skill_utils import parse_frontmatter
...
def _get_category_from_path(skill_path: Path) -> Optional[str]:
    ...
    For paths like: ~/.easybci/skills/mlops/axolotl/SKILL.md -> "mlops"
    ...
    try:
        from easybci_agent.skill_utils import get_external_skills_dirs
...
def _get_disabled_skill_names() -> Set[str]:
    ...
    as a public re-export so existing callers don't need updating.
    """
    from easybci_agent.skill_utils import get_disabled_skill_names
...
def _get_session_platform() -> str:
    ...
    ``_is_skill_disabled`` respects ``EASYBCI_SESSION_PLATFORM``.
    """
    try:
        from services.gateway.session_context import get_session_env
...
def _is_skill_disabled(name: str, platform: str = None) -> bool:
    ...
    2. ``EASYBCI_PLATFORM`` environment variable
    3. ``EASYBCI_SESSION_PLATFORM`` from gateway session context
    """
    try:
        from easybci_cli.config import load_config
        config = load_config()
        skills_cfg = config.get("skills", {})
        resolved_platform = platform or os.getenv("EASYBCI_PLATFORM") or _get_session_platform()
...
def _find_all_skills(*, skip_disabled: bool = False) -> List[Dict[str, Any]]:
    """Recursively find all skills in ~/.easybci/skills/ and external dirs.

    Args:
        skip_disabled: If True, return ALL skills regardless of disabled
            state (used by ``easybci skills`` config UI). Default False
            filters out disabled skills.

    Returns:
        List of skill metadata dicts (name, description, category).
    """
    from easybci_agent.skill_utils import get_external_skills_dirs, iter_skill_index_files
            ...
            return json.dumps(
                {
                    ...
                    "skills": [],
                    "categories": [],
                    "message": f"No skills found. Skills directory created at {display_easybci_home()}/skills/",
                },
                ensure_ascii=False,
            )
...
def _serve_plugin_skill(
    ...
    *,
    ...
    from easybci_cli.plugins import _get_disabled_plugins, get_plugin_manager
        ...
        return json.dumps(
            {
                ...
                "error": (
                    ...
                    f"Re-enable with: easybci plugins enable {namespace}"
                ),
            },
            ensure_ascii=False,
        )
    ...
    if preprocess:
        try:
            from easybci_agent.skill_preprocessing import preprocess_skill_content
...
def skill_view(
    ...
    try:
        ...
        if ":" in name:
            from easybci_agent.skill_utils import is_valid_namespace, parse_qualified_name
        ...
        from easybci_agent.skill_utils import iter_skill_index_files
        ...
        for search_dir in all_dirs:
            ...
            # Strategy 2b: match by frontmatter `name` field when directory
            # name differs.  skills_list() reports the frontmatter name, so
            # skill_view must be able to resolve it even if the directory is
            # named differently.
            if not candidates:
                for found_skill_md in iter_skill_index_files(search_dir, "SKILL.md"):
                    try:
                        _head = found_skill_md.read_text(encoding="utf-8")[:500]
                        _fm, _ = _parse_frontmatter(_head)
                        if _fm.get("name") == name:
                            _record(found_skill_md.parent, found_skill_md)
                    except Exception:
                        continue
        ...
        # Strategy 4 (post-loop fallback): when the requested name is a
        # category-qualified form like ``bci/neural-io-index`` but the actual
        # frontmatter name is bare ``neural-io-index`` (directory is
        # ``bci/neural-io/SKILL.md`` — directory name ≠ frontmatter name),
        # Strategies 1-3 all miss. Retry with the suffix after the last "/"
        # so the LLM can use either the bare or category-qualified form. The
        # existing collision detection below catches cross-category ambiguity.
        if not candidates and "/" in name:
            bare_name = name.rsplit("/", 1)[-1]
            if bare_name and bare_name != name:
                for search_dir in all_dirs:
                    direct_path = search_dir / bare_name
                    if direct_path.is_dir() and (direct_path / "SKILL.md").exists():
                        _record(direct_path, direct_path / "SKILL.md")
                    elif direct_path.with_suffix(".md").exists():
                        _record(None, direct_path.with_suffix(".md"))
                    for found_skill_md in iter_skill_index_files(search_dir, "SKILL.md"):
                        if found_skill_md.parent.name == bare_name:
                            _record(found_skill_md.parent, found_skill_md)
                            continue
                        try:
                            _head = found_skill_md.read_text(encoding="utf-8")[:500]
                            _fm, _ = _parse_frontmatter(_head)
                            if _fm.get("name") == bare_name:
                                _record(found_skill_md.parent, found_skill_md)
                        except Exception:
                            continue
                    for found_md in search_dir.rglob(f"{bare_name}.md"):
                        if found_md.name != "SKILL.md":
                            _record(None, found_md)

        if len(candidates) > 1:
            # Resolve collisions by priority: directory-based skills (those with
            # a SKILL.md, found via Strategy 1/2) take precedence over flat .md
            # files (Strategy 3).  Only refuse if multiple candidates exist at
            # the same priority tier.
            dir_based = [(sd, smd) for sd, smd in candidates if smd.name == "SKILL.md"]
            flat_based = [(sd, smd) for sd, smd in candidates if smd.name != "SKILL.md"]

            if len(dir_based) == 1:
                # Exactly one directory-based skill — use it, ignore flat files
                candidates = dir_based
            elif len(dir_based) > 1:
                # Multiple directory-based skills collide — still ambiguous
                paths = [str(smd) for _, smd in dir_based]
                logging.getLogger(__name__).warning(
                    "Skill name collision for '%s': %d candidates — %s",
                    name, len(dir_based), "; ".join(paths),
                )
                return json.dumps(
                    {
                        "success": False,
                        "error": (
                            f"Ambiguous skill name '{name}': {len(dir_based)} skills "
            ...
            elif len(flat_based) > 1:
                # Multiple flat files collide with no directory-based skill
                paths = [str(smd) for _, smd in flat_based]
                logging.getLogger(__name__).warning(
                    "Skill name collision for '%s': %d candidates — %s",
                    name, len(flat_based), "; ".join(paths),
                )
                return json.dumps(
                    {
                        "success": False,
                        "error": (
                            f"Ambiguous skill name '{name}': {len(flat_based)} skills "
                            "match across your local skills dir and external_dirs. "
                            "Refusing to guess — load one explicitly by its categorized path."
                        ),
                        "matches": paths,
        ...
        if _outside_skills_dir or _injection_detected:
            ...
            if _outside_skills_dir:
                _warnings.append(f"skill file is outside the trusted skills directory (~/.easybci/skills/): {skill_md}")
            ...
            return json.dumps(
                {
                    ...
                    "error": (
                        ...
                        "Enable it with `easybci skills` or inspect the files directly on disk."
                    ),
                },
                ensure_ascii=False,
            )
        ...
        if file_path and skill_dir:
            from easybci_lib.tools.path_security import validate_within_dir, has_traversal_component
        ...
        # Check metadata.easybci.* first (agentskills.io convention), fall back to top-level
        easybci_meta = {}
        metadata = frontmatter.get("metadata")
        if isinstance(metadata, dict):
            easybci_meta = metadata.get("easybci", {}) or {}

        tags = _parse_tags(easybci_meta.get("tags") or frontmatter.get("tags", ""))
        related_skills = _parse_tags(
            easybci_meta.get("related_skills") or frontmatter.get("related_skills", "")
        )
        ...
        if available_env_names:
            try:
                from easybci_lib.tools.env_passthrough import register_env_passthrough
        ...
        if required_cred_files_raw:
            try:
                from easybci_lib.tools.credential_files import register_credential_files
...
def _skill_view_with_bump(args, **kw):
    ...
    try:
        ...
        if isinstance(parsed, dict) and parsed.get("success"):
            ...
            if resolved:
                from easybci_lib.tools.skill_usage import bump_use, bump_view
