...
def _is_easybci_provider_credential(name: str) -> bool:
    """True if ``name`` is a EasyBCI-managed provider credential (API key,
    token, or similar) per ``_EASYBCI_PROVIDER_ENV_BLOCKLIST``.
    ...
    Non-EasyBCI API keys (TENOR_API_KEY, NOTION_TOKEN, etc.) are NOT
    ...
    try:
        ...
        return False
    return name in _EASYBCI_PROVIDER_ENV_BLOCKLIST
...
def register_env_passthrough(var_names: Iterable[str]) -> None:
    ...
    Variables that are EasyBCI-managed provider credentials (from
    ``_EASYBCI_PROVIDER_ENV_BLOCKLIST``) are rejected here to preserve
    the ``execute_code`` sandbox's credential-scrubbing guarantee per
    GHSA-rhgp-j443-p4rf. A skill that needs to talk to a EasyBCI-managed
    ...
    Non-EasyBCI third-party API keys (TENOR_API_KEY, NOTION_TOKEN, etc.)
    ...
    for name in var_names:
        ...
        if not name:
            continue
        if _is_easybci_provider_credential(name):
            logger.warning(
                "env passthrough: refusing to register EasyBCI provider "
                "credential %r (blocked by _EASYBCI_PROVIDER_ENV_BLOCKLIST). "
