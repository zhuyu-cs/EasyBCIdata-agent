...
Usage:
    from easybci_lib.tools.file_operations import ShellFileOperations
    from easybci_lib.tools.terminal_tool import _active_environments
...
from easybci_lib.tools.binary_extensions import BINARY_EXTENSIONS

from easybci_agent.file_safety import (
...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__EASYBCI_FENCE_[A-Za-z0-9]+__\x07?'?")
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__EASYBCI_FENCE_" in line or "\x1b]" in line
...
def normalize_read_pagination(offset: Any = DEFAULT_READ_OFFSET,
    ...
    """
    from easybci_lib.tools.tool_output_limits import get_max_lines
...
class ShellFileOperations(FileOperations):
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        from easybci_lib.tools.tool_output_limits import get_max_line_length
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # Path-guard: redirect mis-targeted writes inside *_preprocess_work_dir/
        # (e.g. LLM writing `pipeline_v3.py` at work_dir root). In ``warn``
        # mode the path is silently normalised; in ``hard`` mode the write is
        # refused so the agent retries with the canonical path; in ``off``
        # nothing happens.
        try:
            from easybci_lib.tools.neural_processing.export.path_guard import (
                normalize_target_path, get_enforce_mode,
            )
            _mode = get_enforce_mode()
            if _mode != "off":
                _new_path, _warning = normalize_target_path(path)
                if _warning is not None:
                    if _mode == "hard":
                        return WriteResult(error=(
                            f"Write refused (tools.codegen.enforce=hard). "
                            + str(_warning)
                        ))
                    # warn mode: rewrite path silently, log via stderr
                    path = str(_new_path)
                    try:
            ...
            # Path-guard must never break a write. Swallow.
            pass
    ...
    def patch_v4a(self, patch_content: str) -> PatchResult:
        ...
        """
        # Import patch parser
        from easybci_lib.tools.patch_parser import parse_v4a_patch, apply_v4a_operations
    ...
    def _lsp_local_only(self) -> bool:
        ...
        if env is None:
            ...
            return False
        try:
            from easybci_lib.tools.environments.local import LocalEnvironment
    ...
    def _lsp_handles_extension(self, ext: str) -> bool:
        ...
        if not ext:
            return False
        try:
            from easybci_agent.lsp.servers import SERVERS
        except Exception:  # noqa: BLE001
            return False
        ext_lower = ext.lower()
        for srv in SERVERS:
    ...
    def _snapshot_lsp_baseline(self, path: str) -> None:
        ...
        if not self._lsp_local_only():
            return
        try:
            from easybci_agent.lsp import get_service
            svc = get_service()
        except Exception:  # noqa: BLE001
            return
        if svc is None:
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        if pre_content is not None and post_content is not None and pre_content != post_content:
            try:
                from easybci_agent.lsp.range_shift import build_line_shift
        ...
        if not diagnostics:
            return ""
        try:
            from easybci_agent.lsp.reporter import report_for_file, truncate
