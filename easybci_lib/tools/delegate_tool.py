...
from easybci_lib.toolsets import TOOLSETS
from easybci_lib.tools import file_state
from easybci_lib.tools.terminal_tool import set_approval_callback as _set_subagent_approval_cb
...
#   true            → _subagent_auto_approve (opt-in YOLO for batch)
...
# (easybci-* prefixed), and scenario toolsets.
#
...
_SUBAGENT_TOOLSETS = sorted(
    ...
    and not name.startswith("easybci-")
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``easybci-cli`` (which bundles
...
def _build_child_agent(
    ...
    if parent_enabled is not None:
        ...
        import easybci_lib.model_tools
    ...
    if toolsets:
        ...
        # Expand composite toolsets (e.g. easybci-cli) so that individual
    ...
    if override_acp_command:
        # If explicitly forcing an ACP transport override, the provider MUST be
        # an ACP provider so run_agent.py initializes the ACP client.
        effective_provider = "acp"
...
def delegate_task(
    ...
    import easybci_lib.model_tools as _model_tools
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'easybci auth'."
        )
...
def _load_config() -> dict:
    ...
    to the persistent config (easybci_cli/config.py load_config()) so that
    ``delegation.model`` / ``delegation.provider`` are picked up regardless
    of the entry point (CLI, gateway).
    """
    try:
        from easybci_lib.cli import CLI_CONFIG
    ...
    "parameters": {
        ...
        "properties": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "toolsets": {
                            "type": "array",
                            "items": {"type": "string"},
                            "description": f"Toolsets for this specific task. Available: {_TOOLSET_LIST_STR}. Use 'web' for network access, 'terminal' for shell.",
                        },
                        "acp_command": {
                            "type": "string",
                            "description": (
            ...
            "acp_command": {
                "type": "string",
                "description": (
                    "Override ACP command for child agents. "
                    ...
                    "Leave empty to use the parent's default transport (EasyBCI subagents)."
                ),
            },
            "acp_args": {
                "type": "array",
