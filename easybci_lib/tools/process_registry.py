...
from easybci_lib.tools.environments.local import _find_shell, _resolve_safe_cwd, _sanitize_subprocess_env
...
CHECKPOINT_PATH = get_easybci_home() / "processes.json"
...
class ProcessRegistry:
    ...
    def spawn_via_env(
        ...
        log_path = f"{temp_dir}/easybci_bg_{session.id}.log"
        pid_path = f"{temp_dir}/easybci_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/easybci_bg_{session.id}.exit"
