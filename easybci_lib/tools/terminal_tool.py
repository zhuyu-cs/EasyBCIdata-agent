...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or EasyBCI exit
...
# ---------------------------------------------------------------------------
from easybci_lib.tools.interrupt import is_interrupted, _interrupt_event  # noqa: F401 — re-exported
# display_easybci_home imported lazily at call site (stale-module safety during easybci update)
...
from easybci_lib.tools.environments.singularity import _get_scratch_dir
from easybci_lib.tools.tool_backend_helpers import (
    coerce_modal_mode,
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of easybci directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "easybci-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("EASYBCI_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("EASYBCI_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = os.getenv("EASYBCI_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (EASYBCI_INTERACTIVE=1).
    ...
    def read_password_thread():
        """Read password with echo disabled via /dev/tty."""
        tty_fd = None
        old_attrs = None
        try:
            import termios
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    If SUDO_PASSWORD is not set and in interactive mode (EASYBCI_INTERACTIVE=1):
    ...
    # interactive EasyBCI password prompt or the sudo -S password-pipe path.
    ...
    # silently without EasyBCI prompting.
    if not has_configured_password and not sudo_password and _sudo_nopasswd_works():
        return command, None

    if not has_configured_password and not sudo_password and os.getenv("EASYBCI_INTERACTIVE"):
...
from easybci_lib.tools.environments.local import LocalEnvironment as _LocalEnvironment
from easybci_lib.tools.environments.singularity import SingularityEnvironment as _SingularityEnvironment
from easybci_lib.tools.environments.ssh import SSHEnvironment as _SSHEnvironment
from easybci_lib.tools.environments.docker import DockerEnvironment as _DockerEnvironment
from easybci_lib.tools.environments.modal import ModalEnvironment as _ModalEnvironment
from easybci_lib.tools.environments.managed_modal import ManagedModalEnvironment as _ManagedModalEnvironment
from easybci_lib.tools.managed_tool_gateway import is_managed_tool_gateway_ready
import sys
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so EasyBCI can track lifecycle and output.
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, EasybciSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter=int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.easybci/.env or environment variables."
        )
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    if env_type == "local":
        ...
        if modal_state["selected_backend"] != "direct":
            if modal_state["managed_mode_blocked"]:
                raise ValueError(
                    ...
                    "a tool gateway is not configured and no direct "
                    "Modal credentials/config were found. Log in with `easybci model` or "
        ...
        from easybci_lib.tools.environments.daytona import DaytonaEnvironment as _DaytonaEnvironment
        ...
        from easybci_lib.tools.environments.vercel_sandbox import (
...
def _cleanup_inactive_envs(lifetime_seconds: int = 300):
    ...
    for task_id, env in envs_to_stop:
        ...
        try:
            from easybci_lib.tools.file_tools import clear_file_ops_cache
...
def _extract_shell_write_targets(command: str) -> list[str]:
    """Best-effort: pick out file paths the shell command writes to.

    Catches the common bypass patterns the skill_compliance_guard cares about:
      * ``> path`` / ``>> path``   — stdout redirect (file create / append)
      * ``2> path`` / ``2>> path`` — stderr redirect
      * ``| tee path`` / ``| tee -a path``
      * ``cp src dst`` / ``mv src dst``
      * ``touch path``

    Returns the raw path tokens as written by the user. The caller resolves
    relative paths against the active cwd. Variables and command-substitution
    targets (e.g. ``> $OUT``, ``> $(date).txt``) are returned verbatim and
    will simply fall outside any ``*_preprocess_work_dir/`` when the caller
    resolves them — that's fine because the guard ignores non-workdir paths.

    Not in scope (intentional): heredocs, python -c '...write...', dd, perl -i.
    These are exotic enough that the guard accepts the false-negative risk;
    the execute_code AST scanner is the right guard for ``python -c`` content.
    """
    targets: list[str] = []
    # File-descriptor-prefixed redirect: > >> 1> 1>> 2> 2>>
    for m in re.finditer(r'[12]?>>?\s*([^\s|;&<>()]+)', command):
        tok = m.group(1).strip('"\'')
        if tok and not tok.startswith("&"):
            targets.append(tok)
    # tee / tee -a / tee -ai
    for m in re.finditer(r'\btee\b\s+(?:-[aip]+\s+)*([^\s|;&<>()]+)', command):
        tok = m.group(1).strip('"\'')
        if tok:
            targets.append(tok)
    # cp / mv  src  dst   (dst is the last positional non-flag token)
    for m in re.finditer(
        r'\b(?:cp|mv)\b(?:\s+-\S+)*\s+\S+\s+([^\s|;&<>()]+)', command,
    ):
        tok = m.group(1).strip('"\'')
        if tok:
            targets.append(tok)
    # touch path1 [path2 ...]
    for m in re.finditer(r'\btouch\b((?:\s+[^\s|;&<>()]+)+)', command):
...
def _extract_python_script_target(command: str, cwd: str) -> "str | None":
    """Extract the absolute path of a .py file being run by python/python3.

    Returns None if the command doesn't look like python script execution.
    Handles: python script.py, python3 script.py, python -u script.py, etc.
    Does NOT match: python -c '...', python -m module, python (interactive).
    """
    from pathlib import Path as _P
    m = re.search(
        r'\bpython[23]?\b(?:\s+-[^\s]*)*\s+([^\s;&|<>]+\.py)\b', command
    )
    if not m:
        return None
...
def _is_tool_generated_script(abs_path: str) -> bool:
    """Check if a .py file was generated by EasyBCI's tool chain.

    Tool-generated scripts carry an ``EASYBCI_STEPS:`` marker in the
    first 15 lines. Scripts without it are custom/user-written.
    """
    from pathlib import Path as _P
    try:
        p = _P(abs_path)
        ...
        with p.open("r", encoding="utf-8", errors="ignore") as f:
            for i, line in enumerate(f):
                if i >= 15:
                    break
                if "EASYBCI_STEPS:" in line or "EASYBCI_GOAL:" in line:
                    return True
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so EasyBCI can track the process, then run "
...
def terminal_tool(
    ...
    try:
        ...
        # Workflow compliance guard: the work_dir is a sealed container.
        # Block (1) shell write targets inside work_dir, AND (2) python
        # execution of non-tool-generated scripts inside work_dir.
        # See easybci_lib/tools/neural_processing/skill_compliance_guard.py.
        try:
            from pathlib import Path as _Path
            from easybci_lib.tools.neural_processing.skill_compliance_guard import (
                evaluate as _evaluate_compliance, detect_work_dir as _detect_wd,
            )
            _effective_cwd = workdir or cwd or str(_Path.cwd())

            # (1) Block shell write targets inside work_dir
            for _t in _extract_shell_write_targets(command):
                _p = _Path(_t)
                _abs = str(_p) if _p.is_absolute() else str(_Path(_effective_cwd) / _p)
                _allowed, _denial = _evaluate_compliance(_abs, source="terminal")
                if not _allowed:
                    return json.dumps(_denial, ensure_ascii=False)

            # (2) Block python execution of non-tool-generated scripts
            _script = _extract_python_script_target(command, _effective_cwd)
            if _script and _detect_wd(_script):
                if not _is_tool_generated_script(_script):
                    _wd = _detect_wd(_script)
                    try:
                        _rel = _Path(_script).resolve(strict=False).relative_to(_wd).as_posix()
                    except ValueError:
                        _rel = _Path(_script).name
                    return json.dumps({
                        "error": (
                            f"workflow_compliance: refusing to execute '{_rel}' "
                            f"inside work_dir '{_wd}' — script lacks EASYBCI_STEPS "
                            "marker (not produced by the tool chain)."
                        ),
                        "work_dir": str(_wd),
                        "target": _rel,
                        "source": "terminal",
                        "remediation": [
                            "Do NOT run custom processing scripts inside the "
                            "work directory. Use EasyBCI's standard tools instead.",
                            "If a proven skill exists: batch_process_adaptive.",
                            "Otherwise: follow the new_pipeline workflow "
                            "(skill_view('pipeline')).",
                            "Only tool-generated scripts (with EASYBCI_STEPS: "
                            "header) can be executed here.",
                        ],
                    }, ensure_ascii=False)
        except Exception:
            # Guard must never break the tool — log and proceed.
            logger.debug("workflow_compliance_guard skipped for terminal", exc_info=True)
        ...
        if background:
            ...
            from easybci_lib.tools.approval import get_current_session_key
            from easybci_lib.tools.process_registry import process_registry
            ...
            try:
                ...
                if background and (notify_on_complete or watch_patterns):
                    from services.gateway.session_context import get_session_env as _gse
                    _gw_platform = _gse("EASYBCI_SESSION_PLATFORM", "")
                    if _gw_platform:
                        _gw_chat_id = _gse("EASYBCI_SESSION_CHAT_ID", "")
                        _gw_thread_id = _gse("EASYBCI_SESSION_THREAD_ID", "")
                        _gw_user_id = _gse("EASYBCI_SESSION_USER_ID", "")
                        _gw_user_name = _gse("EASYBCI_SESSION_USER_NAME", "")
            ...
            from easybci_lib.tools.tool_output_limits import get_max_bytes
...
def check_terminal_requirements() -> bool:
    ...
    try:
        ...
        if env_type == "local":
            ...
            from easybci_lib.tools.environments.docker import find_docker
            docker = find_docker()
            if not docker:
            ...
            if modal_state["selected_backend"] != "direct":
                if modal_state["managed_mode_blocked"]:
                    logger.error(
                        ...
                        "a tool gateway is not configured and no direct "
                        "Modal credentials/config were found. Log in with `easybci model` "
