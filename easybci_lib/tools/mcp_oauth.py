...
This module provides the glue:
    - ``EasybciTokenStorage``: persists tokens/client-info to disk so they
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "EasyBCI Agent"
"""
...
def _get_token_dir() -> Path:
    ...
    Uses EASYBCI_HOME so each profile gets its own OAuth tokens.
    Layout: ``EASYBCI_HOME/mcp-tokens/``
    """
    try:
        from easybci_lib.constants import get_easybci_home
        base = Path(get_easybci_home())
    except ImportError:
        base = Path(os.environ.get("EASYBCI_HOME", str(Path.home() / ".easybci")))
...
# ---------------------------------------------------------------------------
# EasybciTokenStorage -- persistent token/client-info on disk
# ---------------------------------------------------------------------------


class EasybciTokenStorage:
    """Persist OAuth tokens and client registration to JSON files.

    File layout::

        EASYBCI_HOME/mcp-tokens/<server_name>.json         -- tokens
        EASYBCI_HOME/mcp-tokens/<server_name>.client.json   -- client info
        EASYBCI_HOME/mcp-tokens/<server_name>.meta.json     -- oauth server metadata
    """
    ...
    async def get_tokens(self) -> "OAuthToken | None":
        ...
        if data is None:
            return None
        # EasyBCI records an absolute wall-clock ``expires_at`` alongside the
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to EasyBCI.</p></body></html>"
            ) if code else (
...
def _build_client_metadata(cfg: dict) -> "OAuthClientMetadata":
    ...
    client_name = cfg.get("client_name", "EasyBCI Agent")
    scope = cfg.get("scope")
