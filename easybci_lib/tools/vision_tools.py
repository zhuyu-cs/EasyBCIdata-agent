...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("EASYBCI_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
            ...
            pass
    try:
        from easybci_cli.config import cfg_get, load_config
        cfg = load_config()
...
# chat_completions) translate that into Anthropic
...
def _supports_media_in_tool_results(provider: str, model: str) -> bool:
    ...
    Providers covered today (per spec docs verified Apr-2026):
      ...
      * OpenAI Chat Completions: tool messages accept array content with
        ``image_url`` parts.
      * OpenAI Responses (Copilot/xAI): ``function_call_output.output``
...
async def _vision_analyze_native(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            temp_dir = get_easybci_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $EASYBCI_HOME/cache/vision/ and cleaned up
...
def _handle_vision_analyze(args: Dict[str, Any], **kw: Any) -> Awaitable[str]:
    ...
    try:
        from easybci_agent.auxiliary_client import _read_main_provider, _read_main_model
