...
from easybci_agent.file_safety import get_read_block_error
from easybci_lib.tools.binary_extensions import has_binary_extension
from easybci_lib.tools.file_operations import (
    ShellFileOperations,
...
def _get_live_tracking_cwd(task_id: str = "default") -> str | None:
    ...
    try:
        from easybci_lib.tools.terminal_tool import _resolve_container_task_id
    ...
    try:
        from easybci_lib.tools.terminal_tool import _active_environments, _env_lock
...
def _get_file_ops(task_id: str = "default") -> ShellFileOperations:
    ...
    with task_lock:
        ...
        if terminal_env is None:
            from easybci_lib.tools.terminal_tool import _task_env_overrides
...
def _check_file_reqs():
    ...
    from easybci_lib.tools import check_file_requirements
...
def _handle_write_file(args, **kw):
    ...
    if "content" not in args:
        return tool_error(
            ...
            "payload, or use execute_code with easybci_tools.write_file() for very "
            "large files."
        )
    ...
    # Skill-compliance guard: refuse writes that bypass the pipeline
    # 14-step flow (see easybci_lib/tools/neural_processing/skill_compliance_guard.py).
    # Only fires inside *_preprocess_work_dir/ trees; everywhere else this is a
    # no-op. Releases all writes once middle_process/proposal.confirmed exists.
    try:
        ...
        _raw_path = args["path"]
        _abs = str(_raw_path) if _Path(_raw_path).is_absolute() else str(_Path.cwd() / _raw_path)
        _allowed, _denial = _evaluate_compliance(_abs, source="write_file")
        if not _allowed:
            import json as _json
            return _json.dumps(_denial, ensure_ascii=False)
    except Exception:
        # Guard must never break the tool — log and proceed.
        logger.debug("skill_compliance_guard skipped for write_file", exc_info=True)
