...
- **Cross-process token reload** via mtime-based disk watch. When an external
  process refreshes tokens on disk, the next auth flow
...
`mcp_tool.py`, and `easybci_cli/mcp_config.py`. This module is the ONLY place
...
# ---------------------------------------------------------------------------
# EasybciMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_easybci_provider_class() -> Optional[type]:
    ...
    class EasybciMCPOAuthProvider(OAuthClientProvider):
        ...
        This makes external-process refreshes (another CLI instance)
        ...
        def __init__(self, *args: Any, server_name: str = "", **kwargs: Any):
            ...
            self._easybci_server_name = server_name
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`EasybciTokenStorage` persisting an absolute
            ...
            from easybci_lib.tools.mcp_oauth import EasybciTokenStorage
            if (
                isinstance(storage, EasybciTokenStorage)
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._easybci_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from easybci_lib.tools.mcp_oauth import EasybciTokenStorage
                        if isinstance(storage, EasybciTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._easybci_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from easybci_lib.tools.mcp_oauth import EasybciTokenStorage
            if not isinstance(storage, EasybciTokenStorage):
                return
    ...
    return EasybciMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_EASYBCI_PROVIDER_CLS: Optional[type] = _make_easybci_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`EasybciMCPOAuthProvider` directly using the helpers
        extracted from ``tools.mcp_oauth``. The subclass injects a pre-flow
        disk-watch hook so external token refreshes (other CLI
        instances) are visible to running MCP sessions.

        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _EASYBCI_PROVIDER_CLS is None:
            logger.warning(
        ...
        from easybci_lib.tools.mcp_oauth import (
            EasybciTokenStorage,
            _OAUTH_AVAILABLE,
        ...
        storage = EasybciTokenStorage(server_name)
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``easybci mcp remove <name>`` and (indirectly) by
        ``easybci mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
            self._entries.pop(server_name, None)

        from easybci_lib.tools.mcp_oauth import remove_oauth_tokens
    ...
    async def invalidate_if_disk_changed(self, server_name: str) -> bool:
        ...
        """
        from easybci_lib.tools.mcp_oauth import _get_token_dir, _safe_filename
