...
~/.easybci/skills/. Existing skills (bundled, hub-installed, or user-created)
...
from easybci_lib.utils import atomic_replace, is_truthy_value
...
def _guard_agent_created_enabled() -> bool:
    ...
    on via `easybci config set skills.guard_agent_created true`.
    """
    try:
...
# All skills live in ~/.easybci/skills/ (single source of truth)
...
def _find_skill(name: str) -> Optional[Dict[str, Any]]:
    ...
    Searches the local skills dir (~/.easybci/skills/) first, then any
    external dirs configured via skills.external_dirs.  Supports both
    bare names (e.g. "spike_sorting") and categorized paths
    (e.g. "bci/operators/spike/spike_sorting").  Returns {"path": Path} or None.
    """
    from easybci_agent.skill_utils import EXCLUDED_SKILL_DIRS, get_all_skills_dirs
    for skills_dir in get_all_skills_dirs():
        if not skills_dir.exists():
            continue

        # Direct path lookup (handles categorized names like "bci/operators/spike/spike_sorting")
        direct = skills_dir / name
        if direct.is_dir() and (direct / "SKILL.md").exists():
            return {"path": direct}

        # Recursive lookup by bare directory name
...
def _validate_file_path(file_path: str) -> Optional[str]:
    ...
    """
    from easybci_lib.tools.path_security import has_traversal_component
    ...
    if not normalized.parts or normalized.parts[0] not in ALLOWED_SUBDIRS:
        ...
        # SKILL.md is the skill's manifest — it's auto-written by action='create'
        # from the `content` arg. Surface that hint so LLMs that tried
        # `write_file('SKILL.md', ...)` know exactly how to retry without
        # the curator having to re-explain the action taxonomy.
        hint = ""
        if Path(file_path).name.lower() == "skill.md":
            hint = (
                " To create a new skill (whose SKILL.md is auto-generated "
                "from your YAML frontmatter + markdown body), call "
                "skill_manage(action='create', name='<skill-name>', "
                "category='<group>', content='<full SKILL.md text>'). "
                "write_file is only for placing supporting files under "
                "references/, templates/, scripts/, or assets/ — never the "
                "SKILL.md itself."
            )
        return f"File must be under one of: {allowed}. Got: '{file_path}'.{hint}"
...
def _resolve_skill_target(skill_dir: Path, file_path: str) -> Tuple[Optional[Path], Optional[str]]:
    ...
    from easybci_lib.tools.path_security import validate_within_dir
...
def _patch_skill(
    ...
    from easybci_lib.tools.fuzzy_match import fuzzy_find_and_replace
    ...
    if match_error:
        ...
        err_msg = match_error
        try:
            from easybci_lib.tools.fuzzy_match import format_no_match_hint
...
def skill_manage(
    ...
    if action == "create":
        ...
        result = {"success": False, "error": "Use the separate 'skills_list' tool to list skills, not skill_manage(action='list'). skill_manage is for create/edit/patch/delete only."}

    else:
        result = {"success": False, "error": f"Unknown action '{action}'. Use: create, edit, patch, delete, write_file, remove_file. To list skills, use the 'skills_list' tool instead."}
    ...
    if result.get("success"):
        ...
        try:
            from easybci_lib.tools.skill_usage import bump_patch, forget, mark_agent_created
            from easybci_lib.tools.skill_provenance import is_background_review
            if action == "create":
...
SKILL_MANAGE_SCHEMA = {
    ...
    "description": (
        ...
        f"New skills go to {display_easybci_home()}/skills/; existing skills can be modified wherever they live.\n\n"
        ...
        "consolidation from pruning without guessing, so downstream consumers get updated "
        ...
        "will refuse with a message pointing the user to `easybci curator unpin <name>`. "
        ...
        "properties": {
            ...
            "file_path": {
                ...
                "description": (
                    ...
                    "Do NOT use write_file to create SKILL.md — use action='create' "
                    "(which auto-writes SKILL.md from `content`)."
                )
            },
            "file_content": {
                "type": "string",
            ...
            "absorbed_into": {
                ...
                "description": (
                    ...
                    "downstream tooling will have to guess at intent."
                )
            },
        },
