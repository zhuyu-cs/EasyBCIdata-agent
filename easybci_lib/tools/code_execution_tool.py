...
Lets the LLM write a Python script that calls EasyBCI tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `easybci_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `easybci_tools.py` with file-based RPC stubs
...
def _scrub_child_env(source_env, is_passthrough=None):
    ...
    if is_passthrough is None:
        try:
            from easybci_lib.tools.env_passthrough import is_env_passthrough as _ep
        except Exception:
...
# ---------------------------------------------------------------------------
# easybci_tools.py code generator
# ---------------------------------------------------------------------------
...
def generate_easybci_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the easybci_tools.py stub module.
...
"""Auto-generated EasyBCI tools RPC stubs."""
...
def _connect():
    ...
    EASYBCI_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["EASYBCI_RPC_SOCKET"]
...
"""Auto-generated EasyBCI tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("EASYBCI_RPC_DIR") or os.path.join(tempfile.gettempdir(), "easybci_rpc")
_seq = 0
...
def _rpc_server_loop(
    ...
    """
    from easybci_lib.model_tools import handle_function_call
...
def _execute_remote(
    ...
    The script and the generated easybci_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/easybci_exec_{sandbox_id}"
    ...
    try:
        ...
        tools_src = generate_easybci_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/easybci_tools.py", tools_src)
        ...
        env_prefix = (
            f"EASYBCI_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("EASYBCI_TIMEZONE", "").strip()
        if tz:
            env_prefix += f" TZ={tz}"
...
# ---------------------------------------------------------------------------
# Skill-compliance AST helper — sniff out python file-write sinks so the
# guard can intercept sandbox scripts that try to bulk-produce preprocessing
# data products (mne Raw.save / np.save / df.to_pickle / Path.write_text)
# bypassing the standard pipeline 14-step flow.
# ---------------------------------------------------------------------------


def _extract_python_write_targets(source: str) -> List[str]:
    """Best-effort AST scan: pull literal-string path arguments out of common
    file-write sinks. Variables and non-literal expressions are skipped on
    purpose — those will only matter if the path turns out to be a sensitive
    one, and at that point the evaluate() check would have to dynamically
    inspect runtime state, which is out of scope for a static guard.

    Sinks covered:

    * ``open(path, mode='w'/'wb'/'a'/'ab'/'x'...)``
    * ``Path(path).write_text`` / ``Path(path).write_bytes``
    * ``np.save / np.savez / np.savetxt(path, ...)``
    * ``<obj>.to_pickle / .to_csv / .to_hdf / .to_parquet / .to_feather(path, ...)``
    * ``<obj>.save(path, ...)``  — covers mne Raw.save, Epochs.save, ICA.save,
      keras.Model.save, sklearn.dump-style saves

    Returns ``[]`` if the source has a SyntaxError (sandbox will error out
    anyway when it tries to run the bad code).
    """
    import ast as _ast
    targets: List[str] = []
    try:
    ...
    # Methods whose first positional arg is the destination path (most cases).
    _PATH_FIRST_ARG_METHODS = {
        "to_pickle", "to_csv", "to_hdf", "to_parquet", "to_feather",
        "save", "savez", "savetxt",
    }
    # Methods whose path lives in the *receiver*, not in their args.
    # Pattern: Path(p).write_text(content)  → path is in func.value.args[0].
    _RECEIVER_PATH_METHODS = {"write_text", "write_bytes"}

    def _str_of(node: _ast.AST) -> Optional[str]:
    ...
    for node in _ast.walk(tree):
        ...
        func = node.func
        # open(path, mode='w')
        if isinstance(func, _ast.Name) and func.id == "open" and node.args:
            mode: Optional[str] = None
            if len(node.args) >= 2:
                mode = _str_of(node.args[1])
            for kw in node.keywords or []:
                if kw.arg == "mode":
                    mode = _str_of(kw.value)
            # Default mode is "r" (read); only flag writes. If we can't tell
            # the mode (variable / non-literal), conservatively skip — false
            # positives on read-only opens would block legitimate inspection.
            if mode is None:
                continue
            if mode in _WRITE_MODES:
                p = _str_of(node.args[0])
                if p:
                    targets.append(p)
            continue
        if isinstance(func, _ast.Attribute):
            # Path('/x').write_text(content) / .write_bytes(content)
            if func.attr in _RECEIVER_PATH_METHODS:
                recv = func.value
                # Direct: Path('/x').write_text(...)
                if (
                    isinstance(recv, _ast.Call)
                    and isinstance(recv.func, _ast.Name)
                    and recv.func.id in ("Path", "PurePath", "PosixPath", "WindowsPath")
                    and recv.args
                ):
                    p = _str_of(recv.args[0])
                    if p:
                        targets.append(p)
                # Other receivers (e.g. ``my_path.write_text(...)``) → variable,
                # skip per static-analysis policy.
                continue
            # <obj>.METHOD(path, ...) — path is first positional arg
            if func.attr in _PATH_FIRST_ARG_METHODS and node.args:
                p = _str_of(node.args[0])
                if p:
                    targets.append(p)
    return targets
...
def execute_code(
    ...
    # Skill-compliance guard: AST-scan the sandbox script for write sinks
    # (open(..., 'w'), Path.write_text, mne Raw.save, np.save, df.to_pickle,
    # etc.). When any literal-string path argument matches a sensitive
    # path under *_preprocess_work_dir/, refuse the execution with a hint
    # pointing at the standard preprocessing tools. See
    # easybci_lib/tools/neural_processing/skill_compliance_guard.py.
    try:
        from pathlib import Path as _Path
        from easybci_lib.tools.neural_processing.skill_compliance_guard import (
            evaluate as _evaluate_compliance,
        )
        for _target in _extract_python_write_targets(code):
            _p = _Path(_target)
            _abs = str(_p) if _p.is_absolute() else str(_Path.cwd() / _p)
            _allowed, _denial = _evaluate_compliance(_abs, source="execute_code")
            if not _allowed:
    ...
    from easybci_lib.tools.interrupt import is_interrupted as _is_interrupted
    ...
    # --- Set up temp directory with easybci_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="easybci_sandbox_")
    # Use /tmp on macOS to avoid the long /var/folders/... path that pushes
    # Unix domain socket paths past the 104-byte macOS AF_UNIX limit.
    # On Linux, tempfile.gettempdir() already returns /tmp.
    _sock_tmpdir = "/tmp" if sys.platform == "darwin" else tempfile.gettempdir()
    sock_path = os.path.join(_sock_tmpdir, f"easybci_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated easybci_tools module.
        tools_src = generate_easybci_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "easybci_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        # --- Start RPC server ---
        # AF_UNIX stream socket on sock_path, chmod 0600 for
        ...
        # See ``_scrub_child_env`` for the rules.
        child_env = _scrub_child_env(os.environ)
        child_env["EASYBCI_RPC_SOCKET"] = rpc_endpoint
        ...
        # makes ``open()``'s default encoding UTF-8.
        # Belt-and-suspenders for environments with a C/POSIX locale
        # (containers, minimal base images).
        child_env["PYTHONIOENCODING"] = "utf-8"
        child_env["PYTHONUTF8"] = "1"
        # Ensure the easybci-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from easybci_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _easybci_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _easybci_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # EASYBCI_TIMEZONE is an internal EasyBCI setting and must not leak
        # into child processes.
        _tz_name = os.getenv("EASYBCI_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("EASYBCI_TIMEZONE", None)

        # Per-profile HOME isolation: redirect system tool configs into
        # {EASYBCI_HOME}/home/ when that directory exists.
        from easybci_lib.constants import get_subprocess_home
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.easybci/.env')).
...
def _load_config() -> dict:
    ...
    ``easybci --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (easybci-agent's python). Reproducible and the
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``easybci tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and easybci-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.easybci/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call EasyBCI tools programmatically. "
        ...
        f"Available via `from easybci_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into easybci_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from easybci_tools import {import_str}` "
