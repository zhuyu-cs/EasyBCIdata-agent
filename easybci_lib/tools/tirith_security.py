...
it is automatically downloaded from GitHub releases to $EASYBCI_HOME/bin/tirith.
...
def _get_easybci_home() -> str:
    """Return the EasyBCI home directory, respecting EASYBCI_HOME env var."""
    return str(get_easybci_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_easybci_home(), ".tirith-install-failed")
...
def _easybci_bin_dir() -> str:
    """Return $EASYBCI_HOME/bin, creating it if needed."""
    d = os.path.join(_get_easybci_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $EASYBCI_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_easybci_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $EASYBCI_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $EASYBCI_HOME/bin/tirith
    ...
    easybci_bin = os.path.join(_easybci_bin_dir(), "tirith")
    if os.path.isfile(easybci_bin) and os.access(easybci_bin, os.X_OK):
        _resolved_path = easybci_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return easybci_bin
