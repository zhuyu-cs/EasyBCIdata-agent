#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the EasyBCI Skills Hub.
...
Used by easybci_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            easybci_meta = metadata.get("easybci", {})
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            easybci_meta = metadata.get("easybci", {})
            if isinstance(easybci_meta, dict):
                raw_tags = easybci_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by BCI Team) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.easybci/skills/ during setup.  They are discoverable via the Skills Hub
    ...
    def __init__(self):
        ...
        self._optional_dir = get_optional_skills_dir(
            Path(__file__).parent.parent.parent / "optional-skills"
        )
...
# ---------------------------------------------------------------------------
# EasyBCI centralized index source
# ---------------------------------------------------------------------------

EASYBCI_INDEX_URL = ""
EASYBCI_INDEX_CACHE_FILE = INDEX_CACHE_DIR / "easybci-index.json"
EASYBCI_INDEX_TTL = 6 * 3600  # 6 hours


def _load_easybci_index() -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for EASYBCI_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    if EASYBCI_INDEX_CACHE_FILE.exists():
        try:
            age = time.time() - EASYBCI_INDEX_CACHE_FILE.stat().st_mtime
            if age < EASYBCI_INDEX_TTL:
                return json.loads(EASYBCI_INDEX_CACHE_FILE.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Fetch from docs site
    try:
        resp = httpx.get(EASYBCI_INDEX_URL, timeout=15, follow_redirects=True)
        if resp.status_code != 200:
            logger.debug("EasyBCI index fetch returned %d", resp.status_code)
            return _load_stale_index_cache()
        data = resp.json()
    except (httpx.HTTPError, json.JSONDecodeError) as e:
        logger.debug("EasyBCI index fetch failed: %s", e)
        return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        EASYBCI_INDEX_CACHE_FILE.parent.mkdir(parents=True, exist_ok=True)
        EASYBCI_INDEX_CACHE_FILE.write_text(json.dumps(data))
    except OSError:
        pass
...
class EasybciIndexSource(SkillSource):
    """Skill source backed by the centralized EasyBCI Skills Index.
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_easybci_index()
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "easybci-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "easybci-index"),
            identifier=entry.get("identifier", ""),
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if source_filter == "all":
        for src in sources:
            if (src.source_id() == "easybci-index"
