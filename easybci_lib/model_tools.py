...
from easybci_lib.tools.registry import discover_builtin_tools, registry
from easybci_lib.toolsets import resolve_toolset, validate_toolset
...
#   - cli.py, easybci_cli/*      -> inline on startup (no event loop)
...
def _clear_tool_defs_cache() -> None:
    ...
    schema dependencies change (e.g. execute_code sandbox reconfigured)."""
...
def get_tool_definitions(
    ...
    # mode, etc.) without needing an explicit invalidate hook on every
    # config-writer.
    if quiet_mode:
        try:
            from easybci_cli.config import get_config_path
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        from easybci_lib.toolsets import get_all_toolsets
    ...
    if "execute_code" in available_tool_names:
        from easybci_lib.tools.code_execution_tool import SANDBOX_ALLOWED_TOOLS, build_execute_code_schema, _get_execution_mode
    ...
    try:
        from easybci_lib.tools.schema_sanitizer import sanitize_tool_schemas
...
_AGENT_LOOP_TOOLS = {"pipeline_tracker", "memory", "session_search", "delegate_task"}
...
def handle_function_call(
    ...
    try:
        ...
        if function_name not in _READ_SEARCH_TOOLS:
            try:
                from easybci_lib.tools.file_tools import notify_other_tool_call
