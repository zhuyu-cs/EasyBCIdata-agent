#!/usr/bin/env python3
"""
EasyBCI Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the EasyBCI Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills easybci-agent-dev,github-auth
...
os.environ["EASYBCI_QUIET"] = "1"  # Our own modules
...
try:
    from easybci_cli.pt_input_extras import install_shift_enter_alias, install_ctrl_enter_alias
...
from easybci_agent.usage_pricing import (
    CanonicalUsage,
    estimate_usage_cost,
    format_duration_compact,
    format_token_count_compact,
)
from easybci_agent.markdown_tables import (
    is_table_divider,
    looks_like_table_row,
    realign_markdown_tables,
)
# NOTE: `from easybci_agent.account_usage import ...` is deliberately NOT at module
# top — it transitively pulls the OpenAI SDK chain (~230 ms cold) and is only
# needed when the user runs `/limits`. Lazy-imported inside the handler below.
from easybci_cli.banner import _format_context_length, format_banner_version_label

_COMMAND_SPINNER_FRAMES = ("⠋", "⠙", "⠹", "⠸", "⠼", "⠴", "⠦", "⠧", "⠇", "⠏")

from easybci_lib.cli_branding import BRAND_NAME as _BRAND_NAME, branded_model_label as _branded_model_label
...
from easybci_lib.utils import base_url_host_matches, is_truthy_value

_easybci_home = get_easybci_home()
_project_env = Path(__file__).parent / '.env'
load_easybci_dotenv(easybci_home=_easybci_home, project_env=_project_env)
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.easybci/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If EASYBCI_IGNORE_USER_CONFIG=1 is set (via ``easybci chat --ignore-user-config``),
    the user config at ``~/.easybci/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({EASYBCI_HOME}/config.yaml)
    user_config_path = _easybci_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("EASYBCI_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain EasyBCI, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me EasyBCI - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        try:
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and EasybciCLI.__init__ checks "default" first.
            ...
            # location is model.provider (written by `easybci model`).
    ...
    # Normalize config key: the new config system (easybci_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && easybci` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _EASYBCI_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_EASYBCI_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["EASYBCI_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.easybci/logs/.
...
try:
    from easybci_cli.skin_engine import init_skin_from_config
...
try:
    from easybci_agent.auxiliary_client import neuter_async_httpx_del
...
from easybci_lib.model_tools import get_tool_definitions, get_toolset_for_tool

# Extracted CLI modules
from easybci_cli.banner import build_welcome_banner
from easybci_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
from easybci_lib.toolsets import get_all_toolsets, get_toolset_info, validate_toolset

# Resource cleanup imports for safe shutdown (terminal VMs)
from easybci_lib.tools.terminal_tool import cleanup_all_environments as _cleanup_all_terminals
from easybci_lib.tools.terminal_tool import set_sudo_password_callback, set_approval_callback
from easybci_lib.tools.skills_tool import set_secret_capture_callback
from easybci_cli.callbacks import prompt_for_secret
...
def _normalize_git_bash_path(p: Optional[str]) -> Optional[str]:
    """Normalize a Git Bash-style path — no-op on Linux.

    On Linux there is no Git Bash path translation needed; return
    the path unchanged.
    """
    return p
...
def _setup_worktree(repo_root: str = None) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run easybci -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"easybci-{short_id}"
    branch_name = f"easybci/{wt_name}"
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`easybci_cli.config.load_config` (the authoritative loader that
    ...
    try:
        ...
        _easybci_home_maint = _get_easybci_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_easybci_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`easybci_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Also prunes orphaned ``easybci/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not entry.name.startswith("easybci-"):
            continue
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``easybci/easybci-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``easybci -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (b.startswith("easybci/easybci-") or b.startswith("pr-"))
    ]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from easybci_cli.skin_engine import get_active_skin
...
from easybci_lib.constants import is_termux as _is_termux_environment
...
def _preserve_ctrl_enter_newline() -> bool:
    ...
    WSL, SSH sessions, and Windows Terminal all send Ctrl+Enter
...
def _bind_prompt_submit_keys(kb, handler) -> None:
    ...
    Exception: in WSL, SSH sessions, and Windows Terminal,
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``EasybciCLI._busy_command()``.
        """
        yield self

# ASCII Art - EASYBCI-AGENT logo (full width, single line - requires ~95 char terminal)
EASYBCI_AGENT_LOGO = """[bold #FFD700]██╗  ██╗███████╗██████╗ ███╗   ███╗███████╗███████╗       █████╗  ██████╗ ███████╗███╗   ██╗████████╗[/]
[bold #FFD700]██║  ██║██╔════╝██╔══██╗████╗ ████║██╔════╝██╔════╝      ██╔══██╗██╔════╝ ██╔════╝████╗  ██║╚══██╔══╝[/]
[#FFBF00]███████║█████╗  ██████╔╝██╔████╔██║█████╗  ███████╗█████╗███████║██║  ███╗█████╗  ██╔██╗ ██║   ██║[/]
[#FFBF00]██╔══██║██╔══╝  ██╔══██╗██║╚██╔╝██║██╔══╝  ╚════██║╚════╝██╔══██║██║   ██║██╔══╝  ██║╚██╗██║   ██║[/]
[#CD7F32]██║  ██║███████╗██║  ██║██║ ╚═╝ ██║███████╗███████║      ██║  ██║╚██████╔╝███████╗██║ ╚████║   ██║[/]
...
# ASCII Art - EasyBCI Caduceus (compact, fits in left panel)
EASYBCI_CADUCEUS = """[#CD7F32]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⢀⣀⡀⠀⣀⣀⠀⢀⣀⡀⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
[#CD7F32]⠀⠀⠀⠀⠀⠀⢀⣠⣴⣾⣿⣿⣇⠸⣿⣿⠇⣸⣿⣿⣷⣦⣄⡀⠀⠀⠀⠀⠀⠀[/]
[#FFBF00]⠀⢀⣠⣴⣶⠿⠋⣩⡿⣿⡿⠻⣿⡇⢠⡄⢸⣿⠟⢿⣿⢿⣍⠙⠿⣶⣦⣄⡀⠀[/]
[#FFBF00]⠀⠀⠉⠉⠁⠶⠟⠋⠀⠉⠀⢀⣈⣁⡈⢁⣈⣁⡀⠀⠉⠀⠙⠻⠶⠈⠉⠉⠀⠀[/]
[#FFD700]⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀⣴⣿⡿⠛⢁⡈⠛⢿⣿⣦⠀⠀⠀⠀⠀⠀⠀⠀⠀⠀[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        ...
        agent_name = _skin.get_branding("agent_name", "EasyBCI Agent") if _skin else "EasyBCI Agent"
        line1 = agent_name
    ...
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- BCI Team[/]\n"
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.easybci/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.easybci/config.yaml on first use)
        config_path.parent.mkdir(parents=True, exist_ok=True)
        
        # Save back atomically while preserving comments, ordering, quotes, and
        # readable Unicode in user-edited config.yaml.
        from easybci_lib.utils import atomic_roundtrip_yaml_update
...
class EasybciCLI:
    """
    Interactive CLI for the EasyBCI Agent.
    ...
    def __init__(
        ...
        """
        Initialize the EasyBCI CLI.

        Args:
            model: Model to use (default: from env or claude-sonnet)
            toolsets: List of toolsets to enable (default: all)
            provider: Inference provider ("auto", "openrouter", "zai", "kimi-coding", "minimax", "minimax-cn")
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from easybci_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("EASYBCI_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("EASYBCI_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("EASYBCI_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `easybci chat --ignore-rules` in easybci_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("EASYBCI_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("EASYBCI_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # Turn-scope ETA state. Updated by `_on_agent_status` when
        # the agent emits a progress event with scope='turn'; cleared at turn end.
        self._turn_eta_smoothed: Optional[int] = None
        self._turn_eta_emitted_at: float = 0.0
        self._turn_eta_confidence: Optional[str] = None
        ...
        # it's shared across all EasyBCI processes for this EASYBCI_HOME.
        ...
        self._history_file = _easybci_home / ".easybci_history"
    ...
    def _format_prompt_elapsed(
        ...
        *,
        ...
        """Format the turn timer for the bottom status bar.

        Two modes:

        * **Live turn** (``live=True``) — show ONLY the countdown the agent
          gave us (``⏱ est +Xs``). The raw elapsed clock is intentionally
          hidden so the user sees one number (time to completion), not two
          competing ones. When no ETA is available yet, render a bare ``⏱``
          placeholder so the bar doesn't collapse.
        * **Idle / done** (``live=False``) — show the most recent turn's
          total elapsed (``⏲ 1m 20s``). Before the first turn this reads
          ``⏲ 0s``.

        The ``eta_suffix`` parameter is whatever ``cli_eta.format_eta_suffix``
        returned — countdown text without leading decoration. Empty string
        means "no ETA active".
        """
        if live:
            if eta_suffix:
    ...
    def _get_status_bar_snapshot(self) -> Dict[str, Any]:
        ...
        # Compute the turn-scope ETA suffix from the most recent
        # turn-scope progress event. Falls back to "" when no ETA is active.
        try:
            from easybci_lib.cli_eta import compute_remaining_eta, format_eta_suffix
            _now = time.time()
            _remaining = compute_remaining_eta(
                eta_seconds=getattr(self, "_turn_eta_smoothed", None),
                emitted_at=getattr(self, "_turn_eta_emitted_at", 0.0),
                now=_now,
            )
            _eta_suffix = format_eta_suffix(
                remaining_seconds=_remaining,
                confidence=getattr(self, "_turn_eta_confidence", None),
            )
        except Exception:
            _eta_suffix = ""
        snapshot = {
    ...
    def _scrollback_box_width(width: Optional[int] = None) -> int:
        """Return width for printed scrollback box rules and response panels.

        Follows terminal width with a small margin so content uses available
        space rather than being capped at a narrow fixed column count.
        """
        if width is None:
            try:
                ...
                width = 80
        return max(40, int(width or 80) - 2)
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            if width < 52:
                text = f"🧠 {_branded_model_label(snapshot['model_short'])} · {duration_label}"
                return self._trim_status_bar_text(text, width)
            if width < 76:
                parts = [f"🧠 {_branded_model_label(snapshot['model_short'])}", percent_label]
            ...
            compressions = snapshot.get("compressions", 0)
            parts = [f"🧠 {_branded_model_label(snapshot['model_short'])}", context_label, percent_label]
            if compressions:
                parts.append(f"🗜️ {compressions}")
            parts.append(duration_label)
            ...
            return f"🧠 {_branded_model_label(getattr(self, 'model', None))}"
    ...
    def _get_status_bar_fragments(self):
        ...
        try:
            ...
            if width < 52:
                frags = [
                    ("class:status-bar", " 🧠 "),
                    ("class:status-bar-strong", _branded_model_label(snapshot["model_short"])),
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from easybci_cli.models import normalize_opencode_model_id, opencode_model_api_mode
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from easybci_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "🧠 EasyBCI")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - EasybciCLI._status_bar_display_width(label)
    ...
    def _ensure_runtime_credentials(self) -> bool:
        ...
        if not isinstance(api_key, str) or not api_key:
            ...
            if _has_custom_base:
                ...
                print("\n⚠️  Provider resolver returned an empty API key. "
                      "Set OPENROUTER_API_KEY or run: easybci setup")
                return False
        if not isinstance(base_url, str) or not base_url:
            print("\n⚠️  Provider resolver returned an empty base URL. "
                  "Check your provider config or run: easybci setup")
            return False
        ...
        # `easybci chat --model <provider-name>` sends the provider name
        ...
        # If model is still empty (e.g. user ran `easybci auth add <provider>`
        # without `easybci model`), fall back to the provider's first catalog
    ...
    def _init_agent(self, *, model_override: str = None, runtime_override: dict = None, request_overrides: dict | None = None) -> bool:
        ...
        if self._resumed and self._session_db and not self.conversation_history:
            ...
            if not session_meta:
                ...
                _cprint(f"{_DIM}Use a session ID from a previous CLI run (easybci sessions list).{_RST}")
                return False
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``easybci doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``easybci doctor`` so the banner stays
        small.
        """
        try:
    ...
    def show_banner(self):
        ...
        if ctx_len and ctx_len <= 8192:
            ...
            self._console_print(
                "[dim]   EasyBCI needs 16k–32k minimum. Tool schemas + system prompt alone use ~4k–8k.[/]"
            )
    ...
    def _display_resumed_history(self):
        ...
        for i, (role, text) in enumerate(entries):
            if role == "user":
                ...
                lines.append("  ◆ EasyBCI: ", style=f"bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="")
            else:
                lines.append("  ◆ EasyBCI: ", style=f"dim bold {_assistant_label_c}")
                msg_lines = text.splitlines()
                lines.append(msg_lines[0] + "\n", style="dim")
                for ml in msg_lines[1:]:
                    lines.append(f"            {ml}\n", style="dim")
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.easybci/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from easybci_cli.clipboard import save_clipboard_image

        img_dir = get_easybci_home() / "images"
    ...
    def _handle_rollback_command(self, command: str):
        ...
        """
        from easybci_lib.tools.checkpoint_manager import format_checkpoint_list
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: easybci --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for EasyBCI config/state.
        ...
        """
        from easybci_cli.backup import (
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_easybci_home()}/state-snapshots/):\n")
    ...
    def _handle_paste_command(self):
        ...
        from easybci_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run easybci chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            from easybci_lib.model_tools import check_tool_availability
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'easybci setup' to configure[/]")
        except Exception:
    ...
    def _show_status(self):
        ...
        self._console_print(
            f"  {api_indicator} [{accent_color}]{_branded_model_label(model_short)}[/] "
    ...
    def show_help(self):
        ...
        from easybci_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from easybci_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with EasyBCI!{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in easybci_cli/colors.py
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from easybci_cli.main import _relative_time
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("EASYBCI_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
                            "max_iterations": self.max_turns,
                            "reasoning_config": self.reasoning_config,
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            from services.gateway.config import load_gateway_config, Platform
        ...
        home = None  # No messaging platforms configured
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `easybci gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                return
            _cprint("  Tip:   Use /history or `easybci sessions list` to find sessions.")
            return
        ...
        # Resolve title or ID
        from easybci_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `easybci sessions list` to see available sessions.")
            return
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.easybci/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``easybci --resume <id>``
        ...
        saved_dir = get_easybci_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"easybci_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: easybci --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from easybci_cli.curses_ui import curses_single_select
    ...
    def _prompt_text_input_modal(
        ...
        *,
        ...
        self._slash_confirm_deadline = (_time.monotonic() + timeout) if timeout > 0 else 0
        self._invalidate()
        ...
        try:
            while True:
                try:
                    ...
                    if self._slash_confirm_deadline > 0:
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        # and Copilot caps win over the raw models.dev entry
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `easybci model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from easybci_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from easybci_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from easybci_cli.model_switch import switch_model, parse_model_flags
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from easybci_cli.commands import resolve_command
    ...
    def _handle_gquota_command(self, cmd_original: str) -> None:
        ...
        try:
            from easybci_agent.google_oauth import get_valid_access_token, GoogleOAuthError, load_credentials
            from easybci_agent.google_code_assist import retrieve_user_quota, CodeAssistError
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to easybci_cli.curator so the CLI and the `easybci curator`
        ...
        try:
            from easybci_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to easybci_cli.skills_hub."""
        from easybci_cli.skills_hub import handle_skills_slash
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print("  " + "-" * 55)
            print("  Platforms: CLI, WebUI (api_server)")
            print()
            print("  To start the gateway:")
            print("    python cli.py --gateway")
            print()
            print(f"  Configuration file: {display_easybci_home()}/config.yaml")
            print()

        except Exception as e:
            print(f"  Error loading gateway config: {e}")
            print()
            print(f"  Configure settings in {display_easybci_home()}/config.yaml")
            print()
    ...
    def process_command(self, command: str) -> bool:
        ...
        # change in easybci_cli/commands.py instead of touching every dispatch site.
        from easybci_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from easybci_cli.config import reload_env
            count = reload_env()
            ...
            try:
                from easybci_cli.plugins import get_plugin_manager
                mgr = get_plugin_manager()
                plugins = mgr.list_plugins()
                if not plugins:
                    print("No plugins installed.")
                    print(f"Drop plugin directories into {display_easybci_home()}/plugins/ to get started.")
                else:
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from easybci_cli.commands import COMMANDS
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        _cprint(
            ...
            f"EasyBCI keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from easybci_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_easybci_home()}/skins/\n")
            return
    ...
    def _toggle_yolo(self):
        ...
        current = is_truthy_value(os.environ.get("EASYBCI_YOLO_MODE"))
        if current:
            os.environ.pop("EASYBCI_YOLO_MODE", None)
            _cprint(
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while EasyBCI is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while EasyBCI is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while EasyBCI is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from easybci_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
            model = getattr(agent, "model", None) or getattr(self, "model", None)
    ...
    def _handle_debug_command(self):
        ...
        from easybci_cli.debug import run_debug_share
    ...
    def _show_usage(self):
        ...
        if rl_state and rl_state.has_data:
            from easybci_agent.rate_limit_tracker import format_rate_limit_display
            print()
        ...
        # Account usage display removed (easybci_agent.account_usage module deleted).
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by easybci_logging not
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from easybci_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.easybci/skills/ and queue a note for the
        next user turn.
    ...
    # ====================================================================
    # Agent status callback (turn-scope ETA + stage progress relay)
    # ====================================================================

    def _on_agent_status(self, event_type: str, message: str, progress: dict | None = None) -> None:
        """Receive agent status_callback events.

        Only ``scope == "turn"`` progress is rendered by the CLI: it feeds
        the bottom status bar's turn-scope ETA (picked up by
        ``_format_prompt_elapsed`` in ``_get_status_bar_snapshot`` on the
        next render, which appends ``→ est +Xs``).

        ``scope == "stage"`` events are intentionally dropped here — they
        still flow to the WebUI over SSE (Gateway forwards them unchanged),
        but the CLI does NOT print them. The historical inline
        ``[QC 30%]`` line wrote raw ANSI to stderr while prompt_toolkit
        owned the terminal, which garbled the display on many terminals.

        Lifecycle / warn events are best-effort logged through stderr; we
        intentionally don't interrupt prompt_toolkit's render here.
        """
        try:
            if event_type == "progress" and isinstance(progress, dict):
                if progress.get("scope") == "turn":
                    ...
                    if isinstance(new_eta, int) and new_eta > 0:
                        ...
                        conf = progress.get("confidence")
                        if isinstance(conf, str):
                            self._turn_eta_confidence = conf
        except Exception as exc:
            logger.debug("_on_agent_status error: %s", exc)
    ...
    def _on_tool_progress(self, event_type: str, function_name: str = None, preview: str = None, function_args: dict = None, **kwargs):
        ...
        if event_type == "tool.completed":
            ...
            if function_name and self.tool_progress_mode in {"all", "new"}:
                ...
                try:
                    from easybci_agent.display import get_cute_tool_message
    ...
    def _on_tool_start(self, tool_call_id: str, function_name: str, function_args: dict):
        ...
        try:
            from easybci_agent.display import capture_local_edit_snapshot
    ...
    def _on_tool_complete(self, tool_call_id: str, function_name: str, function_args: dict, function_result: str):
        ...
        try:
            from easybci_agent.display import render_edit_diff_with_delta
    ...
    def _clarify_callback(self, question: str, choices: list) -> str:
    ...
    def _approval_callback(self, command: str, description: str,
        ...
        with self._approval_lock:
            timeout = int(CLI_CONFIG.get("approvals", {}).get("timeout", 0))
            response_queue = queue.Queue()
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        if images:
            try:
                from easybci_agent.image_routing import (
        ...
        if isinstance(message, str) and "@" in message:
            try:
                from easybci_agent.context_references import preprocess_context_references
        ...
        try:
            ...
            # Reset turn-scope ETA state for this new turn.
            self._turn_eta_smoothed = None
            self._turn_eta_emitted_at = 0.0
            self._turn_eta_confidence = None
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _easybci_home / "interrupt_debug.log"
            ...
            # Clear turn-scope ETA so the next turn starts clean.
            self._turn_eta_smoothed = None
            self._turn_eta_emitted_at = 0.0
            self._turn_eta_confidence = None
    ...
    def _print_exit_summary(self):
        ...
        if msg_count > 0:
            ...
            print(f"  easybci --resume {self.session_id}")
            if session_title:
                print(f"  easybci -c \"{session_title}\"")
            print()
            ...
            try:
                from easybci_cli.skin_engine import get_active_goodbye
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from easybci_cli.skin_engine import get_active_prompt_symbol
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from easybci_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to EasyBCI Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to EasyBCI Agent! Type your message or /help for commands."
        ...
        try:
            ...
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    ...
                    f"(EASYBCI_REDACT_SECRETS={_redact_raw}). "
        ...
        # after an OpenClaw→EasyBCI migration (especially migrations done by
        ...
        try:
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from easybci_cli.config import get_config_path as _get_cfg_path_resid
        ...
        _agent_busy_no_modal = Condition(
            lambda: bool(self._agent_running)
        ...
        @kb.add('escape', filter=_agent_busy_no_modal, eager=True)
        def handle_escape_interrupt(event):
            """ESC interrupts the running agent (same as first Ctrl+C press)."""
            if self._agent_running and self.agent:
                print("\n⚡ Interrupting agent...")
                self.agent.interrupt()
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from easybci_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "EasyBCI Agent")
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if line_count >= 5 and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _easybci_home / "pastes"
        ...
        # Dynamic prompt: shows EasyBCI symbol when agent is working,
        ...
        def get_hint_text():
            ...
            if cli_ref._slash_confirm_state:
                countdown = ''
                if cli_ref._slash_confirm_deadline > 0:
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("EasyBCI needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'EasyBCI needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("EasyBCI needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = EasybciCLI._compute_model_picker_viewport(
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``EASYBCI_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("EASYBCI_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: easybci setup"
            )
            _run_cleanup()
        ...
        try:
            import easybci_lib.tools.lazy_deps as _lazy_deps_mod
            _lazy_deps_mod._PATCH_STDOUT_ACTIVE = True
            with patch_stdout():
            ...
            _lazy_deps_mod._PATCH_STDOUT_ACTIVE = False
            self._should_exit = True
...
def main(
    ...
    """
    EasyBCI Agent CLI - Interactive AI Assistant
    ...
    try:
        from easybci_cli.stdio import configure_windows_stdio
    ...
    os.environ["EASYBCI_INTERACTIVE"] = "1"

    # Handle gateway mode
    if gateway:
        import asyncio
        from services.gateway.run import start_gateway
        print("Starting EasyBCI Gateway (messaging platforms)...")
    ...
    # Default to easybci-cli toolset
    toolsets_list = None
    if toolsets:
    ...
    # mode registers its own inside EasybciCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  EASYBCI_SIGTERM_GRACE overrides the 1.5 s
        ...
        if quiet:
            ...
            if cli._ensure_runtime_credentials():
                ...
                if cli._init_agent(
                    ...
                    # machine-readable (no styled "EasyBCI" box, no tool-gen
            ...
            # Single-query mode (`easybci chat -q "…"`): skip the welcome
