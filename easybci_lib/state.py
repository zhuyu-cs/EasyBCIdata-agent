#!/usr/bin/env python3
"""
SQLite State Store for EasyBCI Agent.
...
- Session source tagging ('cli', 'api_server') for filtering
"""
...
DEFAULT_DB_PATH = get_easybci_home() / "state.db"
...
# Only SessionDB.__init__ writes to this; other module failures
# are reported via their own caller's error handling.
_last_init_error: Optional[str] = None
_last_init_error_lock = threading.Lock()

# Paths for which we've already logged a WAL-fallback WARNING. Prevents
# re-logging the same filesystem-incompat warning on every connection.
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    Used by :class:`SessionDB` to get fallback behavior on
...
CREATE TABLE IF NOT EXISTS messages (
    ...
    responses_reasoning_items TEXT,
    responses_message_items TEXT,
    tool_status TEXT,
    tool_duration REAL
);
...
CREATE TABLE IF NOT EXISTS archived_work_dirs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
...
CREATE INDEX IF NOT EXISTS idx_archived_wd_session_orig
    ON archived_work_dirs(session_id, original_path);
"""
...
class SessionDB:
    ...
    # With multiple easybci processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None):
        ...
        try:
            ...
            # ``easybci_lib.state._set_last_init_error(None)`` explicitly.
    ...
    def _init_schema(self):
        ...
        # ── Pre-reconcile column renames ───────────────────────────────
        # Column renames must run BEFORE _reconcile_columns(), otherwise
        # reconcile would ADD the new column alongside the old one (leaving
        # data orphaned in the old column).  Each entry is idempotent: it
        # only renames when the old column exists AND the new one does not.
        for _table, _old_col, _new_col in (
            ("messages", "codex_reasoning_items", "responses_reasoning_items"),
            ("messages", "codex_message_items", "responses_message_items"),
        ):
            try:
                _live_cols = {
                    row[1]
                ...
                continue
            if _old_col in _live_cols and _new_col not in _live_cols:
                try:
                    cursor.execute(
                        f'ALTER TABLE "{_table}" '
                        f'RENAME COLUMN "{_old_col}" TO "{_new_col}"'
                    )
                    logger.info(
                        "Renamed %s.%s -> %s", _table, _old_col, _new_col
                    )
                except sqlite3.OperationalError as _exc:
                    logger.warning(
                        "Failed to rename %s.%s -> %s: %s",
                        _table, _old_col, _new_col, _exc,
                    )
    ...
    def update_token_counts(
        ...
        # 0 rows.  Under concurrent load the initial create_session()
        # may have failed due to SQLite locking.
    ...
    def set_session_title(
        ...
        ``source`` records how the title was set: ``'heuristic'`` (auto-derived
        from the first user message), ``'llm'`` (auxiliary-LLM summary at
        end-of-run), or ``'manual'`` (user rename via PATCH /api/sessions
        or the /title slash command). When ``source`` is None the
        ``title_source`` column is left unchanged so legacy callers retain
    ...
    def get_session_title_source(self, session_id: str) -> Optional[str]:
        """Get the title_source for a session ('heuristic'|'llm'|'manual'|None)."""
        with self._lock:
            try:
                cursor = self._conn.execute(
    ...
    def set_heuristic_title_from_message(
        self, session_id: str, user_message: str
    ) -> bool:
        """Write an instant heuristic title derived from the first user message.

        No-op when the session already has any non-NULL title. Derives the
        title by collapsing whitespace and truncating to ~50 chars (with an
        ellipsis suffix). On unique-title collision (rare — two sessions
        opened with identical first messages within the same DB) appends a
        ``" (N)"`` suffix to find a free slot rather than raising — heuristic
        writes must never break dispatch.

        Returns True iff a title was written.
        """
        if not session_id or not user_message:
            return False

        # Don't clobber any existing title (manual rename pre-first-message,
        # resumed session with a prior LLM title, etc.).
        try:
        ...
        # Derive: strip + collapse whitespace runs, then truncate.
        collapsed = re.sub(r'\s+', ' ', user_message).strip()
        if not collapsed:
            return False
        TARGET = 50
        if len(collapsed) > TARGET:
            derived = collapsed[:TARGET].rstrip() + "…"
        else:
            derived = collapsed

        # sanitize_title removes control chars and re-enforces max length.
        try:
            base = self.sanitize_title(derived)
        except ValueError:
            base = self.sanitize_title(derived[:TARGET])
        if not base:
            return False

        # Soft collision handling: try base, then "base (2)", "base (3)", ...
        # so heuristic writes never raise on a unique-index conflict.
        attempt = base
        for n in range(2, 12):
            try:
                return self.set_session_title(session_id, attempt, source='heuristic')
            except ValueError:
                attempt = f"{base} ({n})"
                # Re-sanitize in case the suffix pushed us over MAX_TITLE_LENGTH
                if len(attempt) > SessionDB.MAX_TITLE_LENGTH:
                    truncated = base[: SessionDB.MAX_TITLE_LENGTH - len(f" ({n})")]
                    attempt = f"{truncated} ({n})"
    ...
    def set_session_work_dir(self, session_id: str, work_dir: Optional[str]) -> bool:
        ...
        recover the path across restarts. Pass None to clear.
        """
        def _do(conn):
            cursor = conn.execute(
    ...
    def get_session_work_dir(self, session_id: str) -> Optional[str]:
        """Get the persisted mini-repo work_dir for a session, or None."""
        with self._lock:
            try:
                cursor = self._conn.execute(
                    "SELECT work_dir FROM sessions WHERE id = ?", (session_id,)
                )
                row = cursor.fetchone()
            except sqlite3.OperationalError:
                return None
    ...
    def recent_work_dirs(self, limit: int = 5) -> list[str]:
        """Distinct, most-recently-touched work_dir paths (across sessions).

        Orders by ``started_at`` DESC (the session's row timestamp); dedupes
        by path (first occurrence wins). Returns at most ``limit`` entries.
        Silently returns [] when the underlying table doesn't exist yet
        (fresh install) or has no rows with a non-null work_dir.
        """
        if limit <= 0:
            return []
        with self._lock:
            try:
                rows = self._conn.execute(
                    ...
                    (max(limit * 2, 20),),  # over-fetch so we can dedupe below
                ).fetchall()
            except sqlite3.OperationalError:
                return []
        seen: set[str] = set()
    ...
    def has_archived_in_session(
        self, session_id: str, original_path: str
    ) -> bool:
        """Return True iff this session has already archived this work_dir."""
        if not session_id:
            return False
        with self._lock:
            try:
                cursor = self._conn.execute(
                    "SELECT 1 FROM archived_work_dirs "
                    "WHERE session_id = ? AND original_path = ? LIMIT 1",
    ...
    def record_archived(
        self, session_id: str, *, original: str, archived_to: str
    ) -> None:
        """Record that ``original`` has been archived to ``archived_to`` in this session.

        Subsequent ``has_archived_in_session(session_id, original)`` returns True.
        """
        if not session_id:
            return
        def _do(conn):
            conn.execute(
                "INSERT INTO archived_work_dirs"
                "(session_id, original_path, archived_to, archived_at) "
                "VALUES (?, ?, ?, ?)",
                (session_id, original, archived_to, time.time()),
            )
            return 1
        try:
            self._execute_write(_do)
        except Exception as exc:
            logger.warning("record_archived failed: %s", exc)
    ...
    def append_message(
        ...
        responses_reasoning_items: Any = None,
        responses_message_items: Any = None,
        ...
        responses_items_json = (
            json.dumps(responses_reasoning_items)
            if responses_reasoning_items else None
        )
        responses_message_items_json = (
            json.dumps(responses_message_items)
            if responses_message_items else None
        )
        ...
        def _do(conn):
            cursor = conn.execute(
                """INSERT INTO messages (session_id, role, content, tool_call_id,
                   ...
                   reasoning, reasoning_content, reasoning_details, responses_reasoning_items,
                   responses_message_items, tool_status, tool_duration)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)""",
                (
                    session_id,
                    role,
                    ...
                    responses_message_items_json,
                    tool_status,
                    float(tool_duration) if tool_duration is not None else None,
                ),
            )
        ...
        def _do(conn):
            ...
            for msg in messages:
                ...
                responses_reasoning_items = (
                    msg.get("responses_reasoning_items") if role == "assistant" else None
                )
                responses_message_items = (
                    msg.get("responses_message_items") if role == "assistant" else None
                )
                ...
                responses_items_json = (
                    json.dumps(responses_reasoning_items) if responses_reasoning_items else None
                )
                responses_message_items_json = (
                    json.dumps(responses_message_items) if responses_message_items else None
                )
                tool_calls_json = json.dumps(tool_calls) if tool_calls else None
                ...
                conn.execute(
                    ...
                    (
                        ...
                        responses_items_json,
                        responses_message_items_json,
                        msg.get("tool_status") if role == "tool" else None,
                        float(msg["tool_duration"]) if role == "tool" and msg.get("tool_duration") is not None else None,
                    ),
                )
                total_messages += 1
                if tool_calls is not None:
    ...
    def get_messages_as_conversation(
        ...
        with self._lock:
            ...
            rows = self._conn.execute(
                ...
                "responses_reasoning_items, responses_message_items "
        ...
        for row in rows:
            ...
            if row["role"] == "assistant":
                ...
                if row["responses_reasoning_items"]:
                    try:
                        msg["responses_reasoning_items"] = json.loads(row["responses_reasoning_items"])
                    except (json.JSONDecodeError, TypeError):
                        logger.warning("Failed to deserialize responses_reasoning_items, falling back to None")
                        msg["responses_reasoning_items"] = None
                if row["responses_message_items"]:
                    try:
                        msg["responses_message_items"] = json.loads(row["responses_message_items"])
                    except (json.JSONDecodeError, TypeError):
    ...
    def maybe_auto_prune_and_vacuum(
        ...
        startup from long-lived entrypoints (CLI, gateway).
