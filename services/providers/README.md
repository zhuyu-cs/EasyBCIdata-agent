...
Registry and ABC for every inference provider EasyBCI knows about.
...
`$EASYBCI_HOME/plugins/model-providers/<name>/` (per-user overrides). The
...
- `easybci_cli/auth.py` extends `PROVIDER_REGISTRY` with every api-key
  profile it sees (skipping `copilot`, `kimi-coding`, `kimi-coding-cn`,
  `zai`, `openrouter`, `custom` — those need bespoke token resolution).
- `easybci_cli/models.py` extends `CANONICAL_PROVIDERS` and calls
  `profile.fetch_models()` inside `provider_model_ids()`.
- `easybci_cli/doctor.py` adds a `/models` health check for each
  `auth_type="api_key"` profile.
- `easybci_cli/config.py` injects every `env_var` into
  `OPTIONAL_ENV_VARS` so the setup wizard knows about it.
- `easybci_cli/runtime_provider.py` reads `profile.api_mode` as a fallback
...
under `$EASYBCI_HOME/plugins/model-providers/` for a private plugin).
