...
1. Bundled plugins: ``services/plugins/model-providers/<name>/`` (shipped with easybci-agent)
2. User plugins: ``$EASYBCI_HOME/plugins/model-providers/<name>/``
...
from services.providers.base import OMIT_TEMPERATURE, ProviderProfile  # noqa: F401
...
def register_provider(profile: ProviderProfile) -> None:
    ...
    plugins under ``$EASYBCI_HOME/plugins/model-providers/`` can override
...
def _user_plugins_dir() -> Path | None:
    """Return ``$EASYBCI_HOME/plugins/model-providers/`` if it exists."""
    try:
        from easybci_lib.constants import get_easybci_home

        d = get_easybci_home() / "plugins" / "model-providers"
...
def _import_plugin_dir(plugin_dir: Path, source: str) -> None:
    ...
    # Give bundled plugins a stable import path (``services.plugins.model_providers.<name>``)
    ...
    # multiple EASYBCI_HOME profiles don't alias each other.
    safe_name = plugin_dir.name.replace("-", "_")
    if source == "bundled":
        module_name = f"services.plugins.model_providers.{safe_name}"
    else:
        module_name = f"_easybci_user_provider_{safe_name}"
...
def _discover_providers() -> None:
    ...
    Order:
      ...
      2. User plugins at ``$EASYBCI_HOME/plugins/model-providers/<name>/``
    ...
    # 1. Bundled plugins — shipped with easybci-agent.
    ...
    # 2. User plugins — under $EASYBCI_HOME/plugins/model-providers/<name>/.
    ...
    try:
        ...
        import services.providers as _pkg
