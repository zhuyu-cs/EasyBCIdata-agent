...
Platform adapters inherit from this and implement the required methods.
"""
...
from easybci_lib.utils import normalize_proxy_url
...
def _reply_anchor_for_event(event) -> str | None:
    """Return reply_to id for threaded replies."""
...
def resolve_proxy_url(
    ...
    *,
    ...
    Check order:
      0. *platform_env_var* (e.g. ``MATRIX_PROXY``) — highest priority
...
def proxy_kwargs_for_bot(proxy_url: str | None) -> dict:
    """Build kwargs for an aiohttp-backed bot client with proxy.
...
from easybci_lib.constants import get_easybci_dir


GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    "Secure secret entry is not supported over messaging. "
    "Load this skill in the local CLI to be prompted, or add the key to ~/.easybci/.env manually."
)
...
# (e.g. some platform file URLs expire after a short window).
# ---------------------------------------------------------------------------

# Default location: {EASYBCI_HOME}/cache/images/ (legacy: image_cache/)
IMAGE_CACHE_DIR = get_easybci_dir("cache/images", "image_cache")
        ...
        for attempt in range(retries + 1):
            try:
                response = await client.get(
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; EasybciAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                )
...
#
# Same pattern as image cache -- videos from platforms are downloaded
...
#
# Same pattern as image cache -- documents from platforms are downloaded
# here so the agent can reference them by local file path.
# ---------------------------------------------------------------------------

DOCUMENT_CACHE_DIR = get_easybci_dir("cache/documents", "document_cache")
...
class MessageEvent:
    ...
    # Platform-specific update identifier.  Adapters that expose monotonic
    # update sequence numbers populate this field; others leave it as None.
    # Used by ``/restart`` to record the triggering update so the new gateway
    # can avoid processing the same ``/restart`` twice if a graceful-shutdown
    # ACK times out.
    ...
    # Auto-loaded skill(s) for topic/channel bindings (platform
    # channel_skill_bindings).  A single name or ordered list.
    auto_skill: Optional[str | list[str]] = None

    # Per-channel ephemeral system prompt (platform channel_prompts).
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?easybci\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+easybci[.!?\s]*$", re.IGNORECASE),
)
...
class SendResult:
    ...
    # platform messages (e.g. an edit_message overflow split-and-deliver),
...
def merge_pending_message_event(
    ...
    *,
    ...
    instead of replacing the pending turn. This is used for bursty platform
...
def resolve_channel_skills(
    ...
    Config format::
        ...
        channel_skill_bindings:
          - id: "C0123"          # platform-specific channel/forum ID
            skills: ["skill-a", "skill-b"]
          - id: "D0ABCDE"
            skill: "solo-skill"  # single string also accepted

    Prefers an exact match on *channel_id*; falls back to *parent_id*
    (useful for forum threads inheriting the parent channel's
    binding).
...
class BasePlatformAdapter(ABC):
    ...
    def message_len_fn(self) -> Callable[[str], int]:
        ...
        Python ``len`` (some platforms count UTF-16 code units).
        """
        return len
    ...
    def _acquire_platform_lock(self, scope: str, identity: str, resource_desc: str) -> bool:
        ...
        from services.gateway.status import acquire_scoped_lock
    ...
    def _release_platform_lock(self) -> None:
        ...
        if not identity:
            return
        from services.gateway.status import release_scoped_lock
    ...
    def set_session_store(self, session_store: Any) -> None:
        ...
        has an active session before processing messages.
        """
    ...
    async def edit_message(
        ...
        *,
        ...
        sequence.  Most platforms (Matrix, etc.) treat it as a no-op
        because their edit APIs have no notion of message lifecycle state
        — an edit is an edit.  Platforms that
    ...
    async def send_slash_confirm(
        ...
        Platforms with inline-button support (Matrix, Feishu) should
    ...
    async def send_clarify(
        ...
        Adapters with native button UIs SHOULD
        ...
        if choices:
            ...
            from easybci_lib.tools.clarify_gateway import mark_awaiting_text
    ...
    async def send_typing(self, chat_id: str, metadata=None) -> None:
        ...
        metadata: optional dict with platform-specific context (e.g. thread_id).
        """
        pass
    ...
    def extract_media(content: str) -> Tuple[List[Tuple[str, bool]], str]:
        ...
        rendered JPG is 1-2 MB but a platform's photo upload may recompress
        it to ~200 KB at 1280px) can use ``[[as_document]]`` to request
        unmodified delivery via sendDocument instead of sendPhoto. The
        ...
        same response is delivered as a document.
        ...
        Returns:
            ...
            The is_voice element is always False (kept for API compatibility).
        """
        media = []
        cleaned = content
        ...
        media_pattern = re.compile(
            r'''[`"']?MEDIA:\s*(?P<path>`[^`\n]+`|"[^"\n]+"|'[^'\n]+'|(?:~/|/)\S+(?:[^\S\n]+\S+)*?\.(?:png|jpe?g|gif|webp|mp4|mov|avi|mkv|webm|epub|pdf|zip|rar|7z|docx?|xlsx?|pptx?|txt|csv|apk|ipa)(?=[\s`"',;:)\]}]|$)|\S+)[`"']?'''
        )
    ...
    async def _keep_typing(
        ...
        """No-op typing indicator loop (messaging platforms removed)."""
    ...
    def pause_typing_for_chat(self, chat_id: str) -> None:
        """No-op (messaging platforms removed)."""
        pass
    ...
    # (e.g. an adapter could add 👀/✅/❌ reactions).
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          EASYBCI_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          EASYBCI_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          EASYBCI_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("EASYBCI_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("EASYBCI_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("EASYBCI_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        # Start typing indicator task (no-op, awaits stop_event)
        ...
        try:
            ...
            if response:
                ...
                # where photo upload recompression destroys legibility.
                force_document_attachments = "[[as_document]]" in response

                # Extract MEDIA:<path> tags before other processing
                ...
                # so they're delivered with original bytes (no platform-side
                # photo recompression).
                ...
                for media_path, _is_voice in media_files:
                    ...
                    if (_ext in _IMAGE_EXTS
                        ...
                        _non_image_media.append(media_path)
                ...
                for media_path in _non_image_media:
                    if human_delay > 0:
                        await asyncio.sleep(human_delay)
                    try:
                        ...
                        if not media_result.success:
                            logger.warning("[%s] Failed to send media: %s", self.name, media_result.error)
            ...
            # not at the top of this task.  _easybci_run_generation is set on
    ...
    def format_message(self, content: str) -> str:
        ...
        (e.g., MarkdownV2 escaping for adapters that need it).
    ...
    def truncate_message(
        ...
        Args:
            ...
            len_fn: Optional length function for measuring string length.
                     Defaults to ``len`` (Unicode code-points).
        ...
        while remaining:
            ...
            # When _len != len, headroom is measured in the custom unit.
            # We need codepoint-based slice positions that stay within
            # the custom-unit budget.
            #
            ...
            # causing MarkdownV2 parse errors on adapters that use it.
