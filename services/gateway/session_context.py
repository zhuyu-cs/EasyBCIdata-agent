"""
Session-scoped context variables for the EasyBCI gateway.

Replaces the previous ``os.environ``-based session state
(``EASYBCI_SESSION_PLATFORM``, ``EASYBCI_SESSION_CHAT_ID``, etc.) with
...
messages arrive at the same time the old code did:
    ...
    os.environ["EASYBCI_SESSION_THREAD_ID"] = str(context.source.thread_id)
...
``os.getenv("EASYBCI_SESSION_*", ...)`` calls.  Existing tool code only
needs to replace the import + call site:

    # before
    import os
    platform = os.getenv("EASYBCI_SESSION_PLATFORM", "")

    # after
    from services.gateway.session_context import get_session_env
    platform = get_session_env("EASYBCI_SESSION_PLATFORM", "")
"""
...
# When a contextvar holds _UNSET, we fall back to os.environ (CLI compat).
...
_SESSION_PLATFORM: ContextVar = ContextVar("EASYBCI_SESSION_PLATFORM", default=_UNSET)
_SESSION_CHAT_ID: ContextVar = ContextVar("EASYBCI_SESSION_CHAT_ID", default=_UNSET)
_SESSION_CHAT_NAME: ContextVar = ContextVar("EASYBCI_SESSION_CHAT_NAME", default=_UNSET)
_SESSION_THREAD_ID: ContextVar = ContextVar("EASYBCI_SESSION_THREAD_ID", default=_UNSET)
_SESSION_USER_ID: ContextVar = ContextVar("EASYBCI_SESSION_USER_ID", default=_UNSET)
_SESSION_USER_NAME: ContextVar = ContextVar("EASYBCI_SESSION_USER_NAME", default=_UNSET)
_SESSION_KEY: ContextVar = ContextVar("EASYBCI_SESSION_KEY", default=_UNSET)
_SESSION_ID: ContextVar = ContextVar("EASYBCI_SESSION_ID", default=_UNSET)


_VAR_MAP = {
    "EASYBCI_SESSION_PLATFORM": _SESSION_PLATFORM,
    "EASYBCI_SESSION_CHAT_ID": _SESSION_CHAT_ID,
    "EASYBCI_SESSION_CHAT_NAME": _SESSION_CHAT_NAME,
    "EASYBCI_SESSION_THREAD_ID": _SESSION_THREAD_ID,
    "EASYBCI_SESSION_USER_ID": _SESSION_USER_ID,
    "EASYBCI_SESSION_USER_NAME": _SESSION_USER_NAME,
    "EASYBCI_SESSION_KEY": _SESSION_KEY,
    "EASYBCI_SESSION_ID": _SESSION_ID,
}
...
def get_session_env(name: str, default: str = "") -> str:
    """Read a session context variable by its legacy ``EASYBCI_SESSION_*`` name.

    Drop-in replacement for ``os.getenv("EASYBCI_SESSION_*", default)``.
    ...
    2. ``os.environ`` (only when the context variable was never set in
       this context — i.e. CLI, and test processes that
    ...
    if var is not None:
        ...
        if value is not _UNSET:
            return value
    # Fall back to os.environ for CLI, and test compatibility
