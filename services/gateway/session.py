...
class SessionSource:
    ...
    thread_id: Optional[str] = None  # For forum topics, etc.
    ...
    guild_id: Optional[str] = None  # Matrix server scope or platform-specific guild/workspace
...
def build_session_context_prompt(
    ...
    *,
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_easybci_home()}/output/)"
    )
...
def build_session_key(
    ...
    Group/channel rules:
      ...
      - thread_id differentiates threads within that parent chat.  When
        ...
        conversations (forum topics, threads).
        ...
        if self._db and not skip_db:
            try:
                self._db.append_message(
                    ...
                    responses_reasoning_items=message.get("responses_reasoning_items") if message.get("role") == "assistant" else None,
                    responses_message_items=message.get("responses_message_items") if message.get("role") == "assistant" else None,
                    tool_status=message.get("tool_status") if message.get("role") == "tool" else None,
                    tool_duration=message.get("tool_duration") if message.get("role") == "tool" else None,
                )
            except Exception as e:
                logger.debug("Session DB operation failed: %s", e)
