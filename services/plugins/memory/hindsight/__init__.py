...
Or via $EASYBCI_HOME/hindsight/config.json (profile-scoped), falling back to
...
from easybci_agent.memory_provider import MemoryProvider
from easybci_lib.constants import get_easybci_home
from easybci_lib.tools.registry import tool_error
...
def _check_local_runtime() -> tuple[bool, str | None]:
    ...
    so EasyBCI can degrade gracefully instead of repeatedly trying to start
...
def _load_config() -> dict:
    ...
    Resolution order:
      1. $EASYBCI_HOME/hindsight/config.json  (profile-scoped)
    ...
    profile_path = get_easybci_home() / "hindsight" / "config.json"
    ...
    return {
        ...
        "banks": {
            "easybci": {
                "bankId": os.environ.get("HINDSIGHT_BANK_ID", "easybci"),
...
def _embedded_profile_name(config: dict[str, Any]) -> str:
    """Return the Hindsight embedded profile name for this EasyBCI config."""
    profile = config.get("profile", "easybci")
    return str(profile or "easybci")
...
def _resolve_bank_id_template(template: str, fallback: str, **placeholders: str) -> str:
    ...
    Supported placeholders (each is sanitized before substitution):
      {profile}   — active EasyBCI profile name (from agent_identity)
      {workspace} — EasyBCI workspace name (from agent_workspace)
      {platform}  — "cli", "matrix", etc.
      {user}      — platform user id (gateway sessions)
      {session}   — current session id

    Missing/empty placeholders are rendered as the empty string and then
    collapsed — e.g. ``easybci-{user}`` with no user becomes ``easybci``.
...
class HindsightMemoryProvider(MemoryProvider):
    ...
    def __init__(self):
        ...
        self._retain_context = "conversation between EasyBCI Agent and the User"
    ...
    def save_config(self, values, easybci_home):
        """Write config to $EASYBCI_HOME/hindsight/config.json."""
        import json
        from pathlib import Path
        config_dir = Path(easybci_home) / "hindsight"
    ...
    def post_setup(self, easybci_home: str, config: dict) -> None:
        ...
        from easybci_cli.memory_setup import _curses_select
        ...
        if mode == "cloud":
            ...
            if llm_key:
                ...
                env_path = Path(easybci_home) / ".env"
                existing_llm_key = ""
                if env_path.exists():
        ...
        provider_config.setdefault("bank_id", "easybci")
        ...
        self.save_config(provider_config, easybci_home)
        ...
        if mode == "local_embedded":
            ...
            config_path = Path(easybci_home) / "hindsight" / "config.json"
            try:
            ...
            if not llm_api_key:
                llm_api_key = _load_simple_env(Path(easybci_home) / ".env").get("HINDSIGHT_LLM_API_KEY", "")
            if not llm_api_key:
    ...
    def get_config_schema(self):
        return [
            ...
            {"key": "bank_id", "description": "Memory bank name (static fallback when bank_id_template is unset)", "default": "easybci"},
            {"key": "bank_id_template", "description": "Optional template to derive bank_id dynamically. Placeholders: {profile}, {workspace}, {platform}, {user}, {session}. Example: easybci-{profile}", "default": ""},
            ...
            {"key": "retain_context", "description": "Context label for retained memories", "default": "conversation between EasyBCI Agent and the User"},
        ...
        if self._client is None:
            if self._mode == "local_embedded":
                ...
                logger.debug("Creating HindsightEmbedded client (profile=%s, provider=%s)",
                             self._config.get("profile", "easybci"), llm_provider)
                kwargs = dict(
                    profile=self._config.get("profile", "easybci"),
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        banks = cfg_get(self._config, "banks", "easybci", default={})
        static_bank_id = self._config.get("bank_id") or banks.get("bankId", "easybci")
        ...
        self._retain_context = self._config.get("retain_context", "conversation between EasyBCI Agent and the User")
        ...
        if self._client is not None:
            try:
                if self._mode == "local_embedded":
                    ...
                    # When EasyBCI created/used that client on the shared async loop,
