...
```bash
easybci memory setup    # select "hindsight"
```
...
```bash
easybci config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.easybci/.env
```
...
EasyBCI spins up a local Hindsight daemon with built-in PostgreSQL. Requires an LLM API key for memory extraction and synthesis. The daemon starts automatically in the background on first use and stops after 5 minutes of inactivity.
...
Daemon startup logs: `~/.easybci/logs/hindsight-embed.log`
Daemon runtime logs: `~/.hindsight/profiles/<profile>.log`

To open the Hindsight web UI (local embedded mode only):
```bash
hindsight-embed -p easybci ui start
```
...
Config file: `~/.easybci/hindsight/config.json`
...
| Key | Default | Description |
|-----|---------|-------------|
| `bank_id` | `easybci` | Memory bank name (static fallback used when `bank_id_template` is unset or resolves empty) |
| `bank_id_template` | — | Optional template to derive the bank name dynamically. Placeholders: `{profile}`, `{workspace}`, `{platform}`, `{user}`, `{session}`. Example: `easybci-{profile}` isolates memory per active EasyBCI profile. Empty placeholders collapse cleanly (e.g. `easybci-{user}` with no user becomes `easybci`). |
...
| `retain_context` | `conversation between EasyBCI Agent and the User` | Context label for retained memories |
...
The LLM API key is stored in `~/.easybci/.env` as `HINDSIGHT_LLM_API_KEY`.
