...
```bash
easybci memory setup    # select "byterover"
```

Or manually:
```bash
easybci config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.easybci/.env
```
...
Working directory: `$EASYBCI_HOME/byterover/` (profile-scoped).
