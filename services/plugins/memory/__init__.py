...
1. Bundled providers: ``plugins/memory/<name>/`` (shipped with easybci-agent)
2. User-installed providers: ``$EASYBCI_HOME/plugins/<name>/``
...
Usage:
    from services.plugins.memory import discover_memory_providers, load_memory_provider
...
def _get_user_plugins_dir() -> Optional[Path]:
    """Return ``$EASYBCI_HOME/plugins/`` or None if unavailable."""
    try:
        from easybci_lib.constants import get_easybci_home
        d = get_easybci_home() / "plugins"
...
def load_memory_provider(name: str) -> Optional["MemoryProvider"]:
    ...
    (``$EASYBCI_HOME/plugins/<name>/``) directories.  Bundled takes
...
def _load_provider_from_dir(provider_dir: Path) -> Optional["MemoryProvider"]:
    ...
    module_name = f"services.plugins.memory.{name}" if _is_bundled else f"_easybci_user_memory.{name}"
    ...
    if module_name in sys.modules:
        ...
        for parent in ("services", "services.plugins", "services.plugins.memory"):
            if parent not in sys.modules:
                parent_path = Path(__file__).parent
                if parent == "services.plugins":
...
def discover_plugin_cli_commands() -> List[dict]:
    ...
    module_name = f"services.plugins.memory.{active_provider}.cli" if _is_bundled else f"_easybci_user_memory.{active_provider}.cli"
    try:
