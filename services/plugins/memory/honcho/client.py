...
Resolution order for config file:
  1. $EASYBCI_HOME/honcho.json  (instance-local, enables isolated EasyBCI instances)
...
from easybci_cli.profiles import _get_default_easybci_home
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active EasyBCI profile.

    Resolution order:
      1. EASYBCI_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``easybci.<profile>``
      3. Fallback: ``"easybci"`` (default profile)
    """
    explicit = os.environ.get("EASYBCI_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $EASYBCI_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.easybci/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_easybci_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_easybci_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into EasyBCI system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.easybci
    ...
    def from_global_config(
        ...
        Resolution: $EASYBCI_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active EasyBCI profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.easybci block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. EasyBCI session title (from /title command)
          3. Gateway session key (stable per-chat identifier from gateway platforms)
          4. per-session strategy — EasyBCI session_id ({timestamp}_{hex})
        ...
        # (e.g. "agent:main:matrix:dm:8439114563"). Sanitize colons to hyphens
        ...
        # per-session: inherit EasyBCI session_id (new Honcho session each run)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'easybci honcho setup' or set HONCHO_API_KEY. "
    ...
    # the "go run easybci honcho setup" hint they used to.
    try:
        from easybci_lib.tools.lazy_deps import FeatureUnavailable, ensure as _lazy_ensure
    ...
    try:
        ...
        raise ImportError(
            ...
            "(or run `easybci honcho setup` to configure)."
        )
    ...
    if not resolved_base_url or resolved_timeout is None:
        try:
            ...
            easybci_cfg = load_config()
            honcho_cfg = easybci_cfg.get("honcho", {})
