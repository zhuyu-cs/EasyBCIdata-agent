...
> **Honcho docs:** <https://docs.honcho.dev/v3/guides/integrations/easybci>
...
```bash
easybci honcho setup    # full interactive wizard (cloud or local)
easybci memory setup    # generic picker, also works
```

Or manually:
```bash
easybci config set memory.provider honcho
echo "HONCHO_API_KEY=***" >> ~/.easybci/.env
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `$EASYBCI_HOME/honcho.json` | Profile-local (isolated EasyBCI instances) |
| 2 | `~/.easybci/honcho.json` | Default profile (shared host blocks) |
| 3 | `~/.honcho/config.json` | Global (cross-app interop) |

Host key is derived from the active EasyBCI profile: `easybci` (default) or `easybci.<profile>`.
...
| 4 | `per-session` strategy | EasyBCI session ID (`20260415_a3f2b1`) |
| 5 | `per-repo` strategy | Git root directory name (`easybci-agent`) |
| 6 | `per-directory` strategy | Current directory basename (`src`) |
| 7 | `global` strategy | Workspace name (`easybci`) |

Gateway platforms always resolve via priority 3 (per-chat isolation) regardless of `sessionStrategy`. The strategy setting only affects CLI sessions.

If `sessionPeerPrefix` is `true`, the peer name is prepended: `eri-easybci-agent`.

#### What each strategy produces

- **`per-directory`** — basename of `$PWD`. Opening easybci in `~/code/myapp` and `~/code/other` gives two separate sessions. Same directory = same session across runs.
...
- **`per-session`** — EasyBCI session ID (timestamp + hex). Every `easybci` invocation starts a fresh Honcho session. Falls back to `per-directory` if no session ID is available.
- **`global`** — workspace name. One session for everything. Memory accumulates across all directories and runs.

### Multi-Profile Pattern

Multiple EasyBCI profiles can share one workspace while maintaining separate AI identities. Config resolution is **host block > root > env var > default** — host blocks inherit from root, so shared settings only need to be declared once:
...
Both profiles see the same user (`yourname`) in the same shared environment (`easybci`), but each AI peer builds its own observations, conclusions, and behavior patterns. The coder's memory stays code-oriented; the main agent's stays broad.

Host key is derived from the active EasyBCI profile: `easybci` (default) or `easybci.<profile>` (e.g. `easybci -p coder` → host key `easybci.coder`).
...
| `EASYBCI_HONCHO_HOST` | Host key override |

## CLI Commands

| Command | Description |
|---------|-------------|
| `easybci honcho setup` | Full interactive setup wizard |
| `easybci honcho status` | Show resolved config for active profile |
| `easybci honcho enable` / `disable` | Toggle Honcho for active profile |
| `easybci honcho mode <mode>` | Change recall or observation mode |
| `easybci honcho peer --user <name>` | Update user peer name |
| `easybci honcho peer --ai <name>` | Update AI peer name |
| `easybci honcho tokens --context <N>` | Set context token budget |
| `easybci honcho tokens --dialectic <N>` | Set dialectic max chars |
| `easybci honcho map <name>` | Map current directory to a session name |
| `easybci honcho sync` | Create host blocks for all EasyBCI profiles |
