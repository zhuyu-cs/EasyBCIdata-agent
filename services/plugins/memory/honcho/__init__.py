...
Config: Uses the existing Honcho config chain:
  1. $EASYBCI_HOME/honcho.json (profile-scoped)
...
# ---------------------------------------------------------------------------
# Tool schemas (moved from easybci_lib/tools/honcho_tools.py)
# ---------------------------------------------------------------------------
...
class HonchoMemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, easybci_home):
        """Write config to $EASYBCI_HOME/honcho.json (Honcho SDK native format)."""
        import json
        from pathlib import Path
        config_path = Path(easybci_home) / "honcho.json"
        existing = {}
    ...
    def post_setup(self, easybci_home: str, config: dict) -> None:
        ...
        import types
        from services.plugins.memory.honcho.cli import cmd_setup
        cmd_setup(types.SimpleNamespace())

    def initialize(self, session_id: str, **kwargs) -> None:
        """Initialize Honcho session manager.

        Handles: automated-context guard, recall_mode, session name resolution,
        ...
        try:
            ...
            if agent_context in ("flush",) or platform in ():
                logger.debug("Honcho skipped: automated/flush context (agent_context=%s, platform=%s)",
                             agent_context, platform)
                self._automated_skipped = True
                return
            ...
            try:
                ...
                # behave as they did before. New setups via `easybci honcho setup`
    ...
    def _do_session_init(self, cfg, session_id: str, **kwargs) -> None:
        ...
        from services.plugins.memory.honcho.client import get_honcho_client
        from services.plugins.memory.honcho.session import HonchoSessionManager
        ...
        # Skip under per-session strategy: every EasyBCI run creates a fresh
        ...
        try:
            if not session.messages and cfg.session_strategy != "per-session":
                ...
                mem_dir = str(get_easybci_home() / "memories")
    ...
    def _ensure_session(self) -> bool:
        ...
        try:
            self._do_session_init(
                self._config,
                self._lazy_init_session_id or "easybci-default",
