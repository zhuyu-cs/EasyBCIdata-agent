...
_DEFAULT_CONTAINER_TAG = "easybci"
...
def _load_supermemory_config(easybci_home: str) -> dict:
    config = _default_config()
    config_path = Path(easybci_home) / "supermemory.json"
...
def _save_supermemory_config(values: dict, easybci_home: str) -> None:
    config_path = Path(easybci_home) / "supermemory.json"
    existing = {}
    if config_path.exists():
        try:
...
class SupermemoryMemoryProvider(MemoryProvider):
    ...
    def get_config_schema(self):
        # Only prompt for the API key during `easybci memory setup`.
        # All other options are documented for $EASYBCI_HOME/supermemory.json
    ...
    def save_config(self, values, easybci_home):
        ...
        _save_supermemory_config(sanitized, easybci_home)

    def initialize(self, session_id: str, **kwargs) -> None:
        from easybci_lib.constants import get_easybci_home
        self._easybci_home = kwargs.get("easybci_home") or str(get_easybci_home())
        self._session_id = session_id
        self._turn_count = 0
        self._config = _load_supermemory_config(self._easybci_home)
        ...
        self._write_enabled = agent_context not in ("flush", "subagent")
    ...
    def sync_turn(self, user_content: str, assistant_content: str, *, session_id: str = "") -> None:
        ...
        metadata = {"source": "easybci", "type": "conversation_turn"}
        ...
        def _run():
            try:
                self._client.add_memory(
                    content.strip(),
                    metadata={"source": "easybci_memory", "target": target, "type": "explicit_memory"},
