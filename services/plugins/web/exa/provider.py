...
def _get_exa_client() -> Any:
    ...
    """
    import easybci_lib.tools.web_tools as _wt
    ...
    client.headers["x-exa-integration"] = "easybci-agent"
...
class ExaWebSearchProvider(WebSearchProvider):
    ...
    def availability_diagnostic(self) -> tuple[str, str]:
        """Two-state diagnostic: missing package vs missing key.

        ``EXA_API_KEY`` unset is the most common reason; the SDK is lazily
        installed by :func:`_get_exa_client` so package-missing rarely
        surfaces here, but we report it when the key IS set yet ``exa_py``
        still cannot be imported (e.g. lazy install was disabled).
        """
        if not os.getenv("EXA_API_KEY", "").strip():
            return (
                "EXA_API_KEY environment variable not set",
                "set EXA_API_KEY in ~/.easybci/.env or via WebUI Settings → Web Search",
            )
        try:
            ...
            return (
                ...
                "run: easybci doctor websearch --install exa",
            )
