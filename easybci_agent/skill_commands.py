...
from easybci_agent.skill_preprocessing import (
...
def _resolve_skill_commands_platform() -> Optional[str]:
    ...
    try:
        ...
        resolved_platform = (
            os.getenv("EASYBCI_PLATFORM")
            or get_session_env("EASYBCI_SESSION_PLATFORM")
        )
    except Exception:
        resolved_platform = os.getenv("EASYBCI_PLATFORM")
...
def _load_skill_payload(skill_identifier: str, task_id: str | None = None) -> tuple[dict[str, Any], Path | None, str] | None:
    ...
    try:
        from easybci_lib.tools.skills_tool import SKILLS_DIR, skill_view
...
def _inject_skill_config(loaded_skill: dict[str, Any], parts: list[str]) -> None:
    ...
    If the loaded skill's frontmatter declares ``metadata.easybci.config``
    ...
    try:
        ...
        lines = ["", f"[Skill config (from {display_easybci_home()}/config.yaml):"]
...
def _build_skill_message(
    ...
    from easybci_lib.tools.skills_tool import SKILLS_DIR
...
def scan_skill_commands() -> Dict[str, Dict[str, Any]]:
    """Scan ~/.easybci/skills/ and return a mapping of /command -> skill info.
    ...
    _skill_commands = {}
    try:
        from easybci_lib.tools.skills_tool import SKILLS_DIR, _parse_frontmatter, skill_matches_platform, _get_disabled_skill_names
        ...
        for scan_dir in dirs_to_scan:
            for skill_md in iter_skill_index_files(scan_dir, "SKILL.md"):
                ...
                try:
                    ...
                    # platform command names downstream.
...
def reload_skills() -> Dict[str, Any]:
    ...
    Rescans ``~/.easybci/skills/`` and any ``skills.external_dirs`` so the
...
def resolve_skill_command_key(command: str) -> Optional[str]:
    ...
    (some platforms disallow hyphens, so ``/claude-code`` is registered as
