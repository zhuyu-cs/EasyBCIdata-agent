...
from easybci_lib.utils import safe_json_loads
from easybci_agent.tool_result_classification import file_mutation_result_landed
...
def _resolve_skill_manage_paths(args: dict) -> list[Path]:
    ...
    from easybci_lib.tools.skill_manager_tool import _find_skill, _resolve_skill_dir
...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in EasyBCI' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("EASYBCI_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def _detect_tool_failure(tool_name: str, result: str | None) -> tuple[bool, str]:
    ...
    # Any tool returning status=approval_required is a pending state, not a
    # failure. Skip the rest of the heuristics so the UI does not paint it red.
    if isinstance(result, str):
        ...
        # neither the CLI nor the WebUI paints it red.
        if isinstance(_peek, dict) and _peek.get("suppressed") is True:
            return False, ""

    if tool_name == "terminal":
        data = safe_json_loads(result)
        if isinstance(data, dict):
            # approval_required is a *pending* state, not a failure — the
            # command will be re-issued (or run as-is in yolo mode) once the
            # user resolves the prompt. Reporting it as red/failed makes
            # every approval prompt look like the program is breaking.
