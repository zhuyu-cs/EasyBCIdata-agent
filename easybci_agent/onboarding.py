...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time EasyBCI starts and finds ``~/.openclaw/``.

    Points users at ``easybci claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``easybci claw cleanup`` is mentioned as the
    ...
    return (
        ...
        "To port your config, memory, and skills over to EasyBCI, run "
        "`easybci claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `easybci claw cleanup` (renames it to ~/.openclaw.pre-migration — "
