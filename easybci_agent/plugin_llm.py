...
Plugins built on EasyBCI Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing EasyBCI
...
:class:`~easybci_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors EasyBCI' main config
...
handles every provider, fallback chain, and per-task override EasyBCI
supports.
"""
...
def _resolve_attribution(
    *,
    ...
    if provider_override:
        ...
        try:
            from easybci_agent.auxiliary_client import _read_main_provider
    ...
    if isinstance(response_model, str) and response_model.strip():
        ...
        try:
            from easybci_agent.auxiliary_client import _read_main_model
...
class PluginLlm:
    ...
    Instances are constructed by :class:`easybci_cli.plugins.PluginContext`
    ...
    async def _invoke_async(
        ...
        *,
        ...
        from easybci_agent.auxiliary_client import async_call_llm
        merged_extra = dict(extra_body or {})
        if profile_override:
            merged_extra.setdefault("metadata", {})["auth_profile"] = profile_override
