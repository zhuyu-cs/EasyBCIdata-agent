...
from easybci_agent.moonshot_schema import is_moonshot_model, sanitize_moonshot_tools
from easybci_agent.prompt_builder import DEVELOPER_ROLE_MODELS
from easybci_agent.transports.base import ProviderTransport
from easybci_agent.transports.types import NormalizedResponse, ToolCall, Usage


def _build_gemini_thinking_config(model: str, reasoning_config: dict | None) -> dict | None:
    """Translate EasyBCI/OpenRouter-style reasoning config to Gemini thinkingConfig."""
    ...
    # Gemini 2.5 accepts thinkingBudget; don't guess a budget from EasyBCI'
    ...
    # is stricter (low/high). Clamp EasyBCI' wider effort set to what each
...
class ChatCompletionsTransport(ProviderTransport):
    ...
    def convert_messages(
        ...
        """Messages are already in OpenAI format — sanitize OpenAI Responses API leaks only.

        Strips OpenAI Responses API fields (``responses_reasoning_items`` /
        ``responses_message_items`` on the message, ``call_id`` /
        ``response_item_id`` on tool_calls) that strict chat-completions
        ...
        for msg in sanitized:
            if not isinstance(msg, dict):
                continue
            msg.pop("responses_reasoning_items", None)
            msg.pop("responses_message_items", None)
            tool_calls = msg.get("tool_calls")
            if isinstance(tool_calls, list):
                for tc in tool_calls:
    ...
    def build_kwargs(
        ...
        **params,
        ...
        params (all optional):
            ...
            # Qwen-specific
            qwen_prepare_fn: callable | None — runs AFTER Responses API sanitization
        ...
        """
        # Responses API sanitization: drop reasoning_items / call_id / response_item_id
        ...
        # Developer role swap for models in DEVELOPER_ROLE_MODELS (GPT-5 family)
        ...
        is_bci_team = params.get("is_bci_team", False)
    ...
    def _build_kwargs_from_profile(self, profile, model, sanitized, tools, params):
        ...
        """
        from services.providers.base import OMIT_TEMPERATURE
...
from easybci_agent.transports import register_transport  # noqa: E402
