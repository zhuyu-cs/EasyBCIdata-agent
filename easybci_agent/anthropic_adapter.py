"""Anthropic Messages API adapter for EasyBCI Agent.

Translates between EasyBCI's internal OpenAI-style message format and
Anthropic's Messages API. Follows the same pattern as the openai
...
from easybci_lib.utils import base_url_host_matches, normalize_proxy_env_vars
...
# EasyBCI effort → Anthropic adaptive-thinking effort (output_config.effort).
...
def _prefer_refreshable_claude_code_token(env_token: str, creds: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    EasyBCI historically persisted setup tokens into ANTHROPIC_TOKEN. That makes
...
def resolve_anthropic_token() -> Optional[str]:
    ...
    Priority:
      1. ANTHROPIC_TOKEN env var (OAuth/setup token saved by EasyBCI)
    ...
    # 1. EasyBCI-managed OAuth/setup token env var
    ...
    # This remains as a compatibility fallback for pre-migration EasyBCI configs.
...
# ── EasyBCI-native PKCE OAuth flow ────────────────────────────────────────
# Mirrors the flow used by Claude Code, pi-ai, and OpenCode.
# Stores credentials in ~/.easybci/.anthropic_oauth.json (our own file).

_OAUTH_CLIENT_ID = "9d1c250a-e61b-44d9-88ed-5944d1962f5e"
_OAUTH_TOKEN_URL = "https://console.anthropic.com/v1/oauth/token"
_OAUTH_REDIRECT_URI = "https://console.anthropic.com/oauth/code/callback"
_OAUTH_SCOPES = "org:create_api_key user:profile user:inference"
_EASYBCI_OAUTH_FILE = get_easybci_home() / ".anthropic_oauth.json"
...
def run_easybci_oauth_login_pure() -> Optional[Dict[str, Any]]:
    """Run EasyBCI-native OAuth PKCE flow and return credential state."""
    import time
    import webbrowser
    ...
    print()
    print("Authorize EasyBCI with your Claude Pro/Max subscription.")
    print()
...
def read_easybci_oauth_credentials() -> Optional[Dict[str, Any]]:
    """Read EasyBCI-managed OAuth credentials from ~/.easybci/.anthropic_oauth.json."""
    if _EASYBCI_OAUTH_FILE.exists():
        try:
            data = json.loads(_EASYBCI_OAUTH_FILE.read_text(encoding="utf-8"))
            if data.get("accessToken"):
                return data
        except (json.JSONDecodeError, OSError, IOError) as e:
            logger.debug("Failed to read EasyBCI OAuth credentials: %s", e)
    return None
...
def _content_parts_to_anthropic_blocks(parts: Any) -> List[Dict[str, Any]]:
    ...
    Used for multimodal tool results (e.g. vision tool screenshots). Each
...
def convert_messages_to_anthropic(
    ...
    *base_url* is a Kimi / Moonshot host), unsigned thinking blocks
    ...
    # Multimodal screenshots (base64 images) sit inside tool_result
    ...
    if is_oauth:
        ...
        for block in system:
            if isinstance(block, dict) and block.get("type") == "text":
                text = block.get("text", "")
                text = text.replace("EasyBCI Agent", "Claude Code")
                text = text.replace("EasyBCI agent", "Claude Code")
                text = text.replace("easybci-agent", "claude-code")
                text = text.replace("BCI Team", "Anthropic")
                block["text"] = text
