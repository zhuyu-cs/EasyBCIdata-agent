...
Two modes:
  ...
  native  — attach images as OpenAI-style ``image_url`` content parts on the
            user turn. Provider adapters (Anthropic, Gemini, Bedrock, OpenAI
            Responses, OpenAI chat.completions) already translate these into
            their vendor-specific multimodal formats.
...
def _sniff_mime_from_bytes(raw: bytes) -> Optional[str]:
    ...
    upstream platforms lie about content-type. A platform may serve a PNG
    with ``content_type=image/webp`` for proxied/animated stickers, custom
    emoji previews, or images uploaded via certain bots. Anthropic strictly
    validates that declared media_type matches the actual bytes and returns
    HTTP 400 on mismatch, so we sniff to be safe.
    """
    if not raw:
        return None
