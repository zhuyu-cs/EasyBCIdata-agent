...
# outside that subset before sending EasyBCI tool schemas to Google.
...
def sanitize_gemini_schema(schema: Any) -> Dict[str, Any]:
    ...
    EasyBCI tool schemas are OpenAI-flavored JSON Schema and may contain keys
    ...
    # OpenAI / OpenRouter / Anthropic accept typed enums (e.g. an
    # ``auto_archive_duration: {type: integer, enum: [60, 1440, 4320, 10080]}``
    # field on a thread-management tool), so we only drop the ``enum`` when
    # it would collide with Gemini's rule.
