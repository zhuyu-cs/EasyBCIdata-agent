...
Resolution order for text tasks (auto mode):
  1. User's main provider + main model (used regardless of provider type —
     aggregators, direct API-key providers, native Anthropic, etc.)
...
Payment / credit exhaustion fallback:
  ...
  their OpenRouter balance but has another provider available.
"""
...
#       `patch("easybci_agent.auxiliary_client.OpenAI", ...)` — tests see the proxy,
...
from easybci_lib.constants import OPENROUTER_BASE_URL
from easybci_lib.utils import base_url_host_matches, base_url_hostname, normalize_proxy_env_vars
...
def _compression_threshold_for_model(model: Optional[str]) -> Optional[float]:
    ...
    consumed before EasyBCI triggers summarization.  Higher values delay
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``EASYBCI_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``EASYBCI_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("EASYBCI_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("EASYBCI_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_AUTH_JSON_PATH = get_easybci_home() / "auth.json"
...
# calls to the OpenAI Responses API so callers don't need any changes.
...
class _ResponsesCompletionsAdapter:
    """Drop-in shim that accepts chat.completions.create() kwargs and
    routes them through the OpenAI Responses streaming API."""
    ...
    def create(self, **kwargs) -> Any:
        ...
        if isinstance(extra_body, dict):
            ...
            if isinstance(reasoning_cfg, dict):
                if reasoning_cfg.get("enabled") is False:
                    ...
                    # Falsy values (None, "", 0) fall back to the default
                    # rather than being forwarded to the backend, which
                    # rejects e.g. {"effort": null} with a 400.
                    effort = reasoning_cfg.get("effort") or "medium"
                    # Backend rejects "minimal"; clamp to "low".
                    if effort == "minimal":
                        effort = "low"
        ...
        def _timeout_message() -> str:
            return f"Responses API auxiliary stream exceeded {float(total_timeout):.1f}s total timeout"
        ...
        def _close_client_on_timeout() -> None:
            ...
            if callable(close):
                try:
                    ...
                    logger.debug("Responses auxiliary: client close during timeout failed", exc_info=True)
            # The cached auxiliary client wraps this same ``self._client``
            # (or *is* a ``ResponsesAuxiliaryClient`` whose ``_real_client`` is
            ...
            try:
                ...
                logger.debug("Responses auxiliary: cache eviction on timeout failed", exc_info=True)
        ...
        def _check_cancelled() -> None:
            ...
            try:
                ...
                if is_interrupted():
                    raise InterruptedError("Responses auxiliary stream interrupted")
        ...
        try:
            ...
            # the Responses API backend can return empty response.output from
            ...
            if isinstance(_output, list) and not _output:
                if collected_output_items:
                    ...
                    logger.debug(
                        "Responses auxiliary: backfilled %d output items from stream events",
                    ...
                    logger.debug(
                        "Responses auxiliary: synthesized from %d deltas (%d chars)",
            ...
            logger.debug("Responses auxiliary API call failed: %s", exc)
            raise
        finally:
...
class _ResponsesChatShim:
    """Wraps the adapter to provide client.chat.completions.create()."""

    def __init__(self, adapter: _ResponsesCompletionsAdapter):
        self.completions = adapter


class ResponsesAuxiliaryClient:
    """OpenAI-client-compatible wrapper that routes through the Responses API.
    ...
    def __init__(self, real_client: OpenAI, model: str):
        ...
        adapter = _ResponsesCompletionsAdapter(real_client, model)
        self.chat = _ResponsesChatShim(adapter)
...
class _AsyncResponsesCompletionsAdapter:
    """Async version of the Responses API adapter.

    Wraps the sync adapter via asyncio.to_thread() so async consumers
    (web_tools, session_search) can await it as normal.
    """

    def __init__(self, sync_adapter: _ResponsesCompletionsAdapter):
...
class _AsyncResponsesChatShim:
    def __init__(self, adapter: _AsyncResponsesCompletionsAdapter):
        self.completions = adapter


class AsyncResponsesAuxiliaryClient:
    """Async-compatible wrapper matching AsyncOpenAI.chat.completions.create()."""

    def __init__(self, sync_wrapper: "ResponsesAuxiliaryClient"):
        sync_adapter = sync_wrapper.chat.completions
        async_adapter = _AsyncResponsesCompletionsAdapter(sync_adapter)
        self.chat = _AsyncResponsesChatShim(async_adapter)
...
class _AnthropicCompletionsAdapter:
    ...
    def create(self, **kwargs) -> Any:
        from easybci_agent.anthropic_adapter import build_anthropic_kwargs
        ...
        if temperature is not None:
            from easybci_agent.anthropic_adapter import _forbids_sampling_params
...
class AsyncAnthropicAuxiliaryClient:
    def __init__(self, sync_wrapper: "AnthropicAuxiliaryClient"):
        ...
        self.api_key = sync_wrapper.api_key
        self.base_url = sync_wrapper.base_url
        # See AsyncResponsesAuxiliaryClient: mirror _real_client so cache
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``easybci_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _maybe_wrap_anthropic(
    ...
    - It's already an Anthropic/Gemini wrapper.
    ...
    if _safe_isinstance(client_obj, ResponsesAuxiliaryClient):
        return client_obj
    try:
        from easybci_agent.gemini_native_adapter import GeminiNativeClient
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from easybci_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                try:
                    from services.providers import get_provider_profile as _gpf_aux
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            try:
                from services.providers import get_provider_profile as _gpf_aux2
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `easybci setup` or `easybci model` and enter a valid http(s) base URL."
        ) from exc
...
def _try_custom_endpoint() -> Tuple[Optional[Any], Optional[str]]:
    ...
    if custom_mode == "responses":
        real_client = OpenAI(api_key=custom_key, base_url=_clean_base, **_extra)
        return ResponsesAuxiliaryClient(real_client, model), model
...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    ...
    # base_url (e.g. Responses API endpoint) would leak into Anthropic requests.
...
# burns ~1 RTT, gets 402 again, then falls back. On a long LCM
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second easybci
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``easybci config aux reset``)."""
...
def _evict_cached_client_instance(target: Any) -> bool:
    ...
    Walks both sync and async wrappers (``ResponsesAuxiliaryClient``,
    ``AnthropicAuxiliaryClient``, ``AsyncResponsesAuxiliaryClient``, etc.) via
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "anthropic":
            from easybci_agent.anthropic_adapter import read_claude_code_credentials, _refresh_oauth_token, resolve_anthropic_token
...
def _resolve_auto(main_runtime: Optional[Dict[str, Any]] = None) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    Priority:
      ...
      2. OpenRouter → custom → Responses → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `easybci model` but the
    #    old OPENAI_BASE_URL lingers in ~/.easybci/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: easybci model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.easybci/.env",
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    """Convert a sync client to its async counterpart, preserving Responses API routing."""
    from openai import AsyncOpenAI

    if isinstance(sync_client, ResponsesAuxiliaryClient):
        return AsyncResponsesAuxiliaryClient(sync_client), model
    if isinstance(sync_client, AnthropicAuxiliaryClient):
        return AsyncAnthropicAuxiliaryClient(sync_client), model
    try:
        from easybci_agent.gemini_native_adapter import GeminiNativeClient, AsyncGeminiNativeClient
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        try:
            from easybci_agent.model_metadata import _infer_provider_from_url
            from services.providers import get_provider_profile as _gpf_async
...
def resolve_provider_client(
    ...
    Responses API providers, an adapter handles the translation
    transparently.
    ...
    Args:
        ...
        raw_responses: If True, suppress automatic ResponsesAuxiliaryClient
            wrapping — return the raw OpenAI client so the caller can
            access responses.stream() directly.
        explicit_base_url: Optional direct OpenAI-compatible endpoint.
        explicit_api_key: Optional API key paired with explicit_base_url.
        api_mode: API mode override.  One of "chat_completions",
            "responses", or None (auto-detect).  When set to
            "responses", the client is wrapped in
            ResponsesAuxiliaryClient to route through the Responses API.
    ...
    def _needs_responses_wrap(client_obj) -> bool:
        """Decide if a plain OpenAI client should be wrapped for Responses API.

        Returns True only when api_mode is explicitly "responses".
        Already-wrapped clients (ResponsesAuxiliaryClient) are skipped.
        """
        if isinstance(client_obj, ResponsesAuxiliaryClient):
            return False
        if raw_responses:
            return False
    ...
    def _wrap_if_needed(client_obj, final_model_str: str, base_url_str: str = "",
        ...
        Handles two cases:
        - ``ResponsesAuxiliaryClient`` when the endpoint needs the Responses API
          (explicit ``api_mode=responses``).
        ...
        """
        if _needs_responses_wrap(client_obj):
            logger.debug(
                "resolve_provider_client: wrapping client in ResponsesAuxiliaryClient "
                "(api_mode=%s, model=%s, base_url=%s)",
                api_mode or "auto-detected", final_model_str,
                base_url_str[:60] if base_url_str else "")
            return ResponsesAuxiliaryClient(client_obj, final_model_str)
    ...
    if provider == "custom":
        if explicit_base_url:
            ...
            if base_url_host_matches(custom_base, "api.kimi.com"):
                ...
                try:
                    from services.providers import get_provider_profile as _gpf_custom
        ...
        # Try custom first, then API-key providers (Responses excluded here:
        # falling through to the Responses API with no model is a stale-constant trap).
    ...
    try:
        from easybci_cli.runtime_provider import _get_named_custom_provider
        ...
        # entries that coincidentally match a canonical provider
        ...
        if custom_entry:
            ...
            if custom_base:
                ...
                # OpenAI-wire paths (chat_completions / responses) need the
                ...
                # responses or inherited auto-detect (via _wrap_if_needed).
                # _wrap_if_needed reads the closed-over `api_mode` (the task-level
                # override). Named-provider entry api_mode=responses also
                # flows through here.
                if entry_api_mode in {"responses"} and not isinstance(
                    client, ResponsesAuxiliaryClient
                ):
                    client = ResponsesAuxiliaryClient(client, final_model)
                else:
    ...
    try:
        ...
        logger.debug("easybci_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if base_url_host_matches(base_url, "api.kimi.com"):
            ...
            try:
                from services.providers import get_provider_profile as _gpf_main
        ...
        # Honor api_mode for any API-key provider (e.g. xAI Responses API).
        # Also rewraps Anthropic-wire endpoints (Kimi Coding Plan
        # api.kimi.com/coding, /anthropic-suffixed gateways) so named providers
        # like kimi-coding land on the right transport without needing
    ...
    if pconfig.auth_type == "external_process":
        ...
        try:
            from easybci_agent.bedrock_adapter import has_aws_credentials, resolve_bedrock_region
...
def get_async_text_auxiliary_client(task: str = "", *, main_runtime: Optional[Dict[str, Any]] = None):
    ...
    For standard providers returns (AsyncOpenAI, model). For Responses API returns
    (AsyncResponsesAuxiliaryClient, model) which wraps the Responses API.
...
def get_auxiliary_extra_body() -> dict:
    ...
    Returns empty dict (no provider-specific tags needed).
    """
    return {}
...
def auxiliary_max_tokens_param(value: int) -> dict:
    ...
    The Responses adapter translates max_tokens internally, so we use max_tokens
    for it as well.
    """
    ...
    if (not or_key
            and base_url_hostname(custom_base) in {"api.openai.com"}):
...
def _resolve_task_provider_model(
    ...
    "chat_completions", "responses", or None (auto-detect).
    """
    cfg_provider = None
    cfg_model = None
    cfg_base_url = None
...
def _build_call_kwargs(
    ...
    if max_tokens is not None:
        # Responses adapter handles max_tokens internally; OpenRouter uses max_tokens.
        # Direct OpenAI api.openai.com with newer models needs max_completion_tokens.
        # ZAI vision models (glm-4v-flash, glm-4v-plus, etc.) reject max_tokens with
        # error code 1210 (invalid API parameter) on multimodal requests — skip it.
...
def _validate_llm_response(response: Any, task: str = None) -> Any:
    ...
    # Allow SimpleNamespace responses from adapters (ResponsesAuxiliaryClient,
...
def call_llm(
    ...
    *,
    ...
    try:
        ...
        # (invalid API parameter) when max_tokens is passed on multimodal
        ...
        # OpenAI OAuth or another provider available.
        #
        ...
        # OAuth tokens that authenticate but whose endpoint is down,
