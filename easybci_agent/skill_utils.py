...
from easybci_lib.constants import get_config_path, get_skills_dir
...
def get_disabled_skill_names(platform: str | None = None) -> Set[str]:
    ...
    Args:
        platform: Explicit platform name (e.g. ``"matrix"``).  When
            *None*, resolves from ``EASYBCI_PLATFORM`` or
            ``EASYBCI_SESSION_PLATFORM`` env vars.  Falls back to the
...
# which becomes the dominant cost of ``easybci`` startup when ~120 skills
...
def get_external_skills_dirs() -> List[Path]:
    ...
    paths that resolve to the local ``~/.easybci/skills/`` are silently skipped.

    Cached in-process, keyed on ``config.yaml`` mtime — the function is
    called once per skill during banner / tool-registry scans, and YAML
    parsing a non-trivial config dominates ``easybci`` cold-start time
    ...
    for entry in raw_dirs:
        ...
        p = Path(expanded)
        # Resolve relative paths against EASYBCI_HOME, not cwd
        if not p.is_absolute():
            p = (easybci_home / p).resolve()
        else:
            p = p.resolve()
        if p == local_skills:
            continue
...
def get_all_skills_dirs() -> List[Path]:
    """Return all skill directories: local ``~/.easybci/skills/`` first, then external.
...
def extract_skill_conditions(frontmatter: Dict[str, Any]) -> Dict[str, List]:
    ...
    if not isinstance(metadata, dict):
        metadata = {}
    easybci = metadata.get("easybci") or {}
    if not isinstance(easybci, dict):
        easybci = {}
    return {
        "fallback_for_toolsets": easybci.get("fallback_for_toolsets", []),
        "requires_toolsets": easybci.get("requires_toolsets", []),
        "fallback_for_tools": easybci.get("fallback_for_tools", []),
        "requires_tools": easybci.get("requires_tools", []),
    }
...
def extract_skill_config_vars(frontmatter: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not isinstance(metadata, dict):
        return []
    easybci = metadata.get("easybci")
    if not isinstance(easybci, dict):
        return []
    raw = easybci.get("config")
    if not raw:
        return []
