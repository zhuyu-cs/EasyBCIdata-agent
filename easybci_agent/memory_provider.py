...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - easybci_home (str): The active EASYBCI_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.easybci``.
          - platform (str): "cli", "matrix", etc.

        kwargs may also include:
          - agent_context (str): "primary", "subagent", or "flush".
            Providers should skip writes for non-primary contexts (automated system
            prompts would corrupt user representations).
          - agent_identity (str): Profile name (e.g. "coder"). Use for
            per-profile provider identity scoping.
          - agent_workspace (str): Shared workspace name (e.g. "easybci").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'easybci memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], easybci_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'easybci memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``easybci_home`` is the active EASYBCI_HOME directory path.
