...
skills and maintains the collection. It runs inactivity-triggered (no
...
def _state_file() -> Path:
    return get_easybci_home() / "skills" / ".curator_state"
...
def _load_config() -> Dict[str, Any]:
    """Read curator.* config from ~/.easybci/config.yaml. Tolerates missing file."""
    try:
...
def should_run_now(now: Optional[datetime] = None) -> bool:
    ...
    ``easybci update``. On first observation we seed ``last_run_at`` to "now"
    and defer the first real pass by one full interval. Users who want to
    run it sooner can always invoke ``easybci curator run`` (with or without
    ...
    if last is None:
        ...
        try:
            ...
            state["last_run_summary"] = (
                ...
                "interval; use `easybci curator run --dry-run` to preview now"
            )
            save_state(state)
...
def apply_automatic_transitions(now: Optional[datetime] = None) -> Dict[str, int]:
    ...
    from easybci_lib.tools import skill_usage as _u
...
CURATOR_REVIEW_PROMPT = (
    "You are running as EasyBCI' background skill CURATOR. This is an "
    ...
    "into ~/.easybci/skills/.archive/) is the maximum destructive action. "
    ...
    "to find: easybci-config-*, easybci-dashboard-*, gateway-*, "
    ...
    "~/.easybci/skills/<umbrella>/references/ && mv ... <umbrella>/"
...
def _reports_root() -> Path:
    ...
    Lives under the profile-aware logs dir (``~/.easybci/logs/curator/``)
    ...
    authored skill data in ``~/.easybci/skills/``.

    ``ensure_easybci_home()`` pre-creates this dir on every CLI launch and
    ...
    that bypasses both.
    """
    root = get_easybci_home() / "logs" / "curator"
    try:
...
def _parse_structured_summary(
    ...
    # Prefer PyYAML when available — every easybci install already has it
...
def _build_rename_summary(
    *,
    ...
    Format::
        ...
        full report: easybci curator status
        keep an umbrella stable: easybci curator pin document-tools
    ...
    lines.append("full report: easybci curator status")
    ...
    if consolidated:
        ...
        if umbrellas:
            example = umbrellas[0]
            lines.append(
                f"keep an umbrella stable: easybci curator pin {example}"
            )
    return "\n".join(lines)
...
def _render_report_markdown(p: Dict[str, Any]) -> str:
    ...
    # on disk still lives under ~/.easybci/skills/.archive/ (every removal is
    ...
    if consolidated:
        ...
        lines.append(
            ...
            "The original directory was moved to `~/.easybci/skills/.archive/` for "
            "safety and can be restored via `easybci curator restore <name>` if the "
    ...
    if pruned:
        ...
        lines.append(
            ...
            "Directories live under `~/.easybci/skills/.archive/`. "
            "Restore any via `easybci curator restore <name>`._\n"
        )
        SHOW = 50
    ...
    lines.append("- Restore an archived skill: `easybci curator restore <name>`")
    lines.append("- All archives live under `~/.easybci/skills/.archive/` and are recoverable by `mv`")
...
def run_curator_review(
    ...
    if dry_run:
        ...
        try:
            from easybci_agent import curator_backup
    ...
    # real pass out. We still record a summary so `easybci curator status`
    ...
    def _llm_pass():
        ...
        # recorded in state so `easybci curator status` can point at it.
        try:
            after_report = skill_usage.agent_created_report()
        except Exception:
            after_report = []
...
def _resolve_review_model(cfg: Dict[str, Any]) -> tuple[str, str]:
    ...
    — so it participates in the canonical aux-model plumbing (``easybci model`` →
...
def _run_llm_review(prompt: str) -> Dict[str, Any]:
    ...
    # (canonical aux-task slot, wired through `easybci model` → auxiliary
