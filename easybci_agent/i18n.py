"""Lightweight internationalization (i18n) for EasyBCI static user-facing messages.

Scope (thin slice, by design): only the highest-impact static strings shown
to the user by EasyBCI itself -- approval prompts, a handful of gateway slash
...
Catalog files live under ``easybci_lib/locales/<lang>.yaml``.  Each
...
Language resolution order:
    ...
    2. ``EASYBCI_LANGUAGE`` environment variable (for tests / quick override)
...
def _locales_dir() -> Path:
    ...
    Locales live alongside the cross-cutting library subpackage at
    ``easybci_lib/locales/`` so wheel installs and editable checkouts
    discover them via the package layout, not via the repo root.
    """
    # easybci_agent/i18n.py -> easybci_agent/ -> repo root -> easybci_lib/locales/
    return Path(__file__).resolve().parent.parent / "easybci_lib" / "locales"
...
def _load_catalog(lang: str) -> dict[str, str]:
    ...
    try:
        import yaml  # PyYAML is already an easybci dependency
...
def reset_language_cache() -> None:
    ...
    Call after :func:`easybci_cli.config.save_config` if a running process
...
def get_language() -> str:
    ...
    env_lang = os.environ.get("EASYBCI_LANGUAGE")
    if env_lang:
