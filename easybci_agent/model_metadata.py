...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. EasyBCI also honours EASYBCI_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("EASYBCI_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
_PROVIDER_PREFIXES: frozenset[str] = frozenset({
    "openrouter",
    "gemini", "zai", "kimi-coding", "kimi-coding-cn", "stepfun", "minimax", "minimax-oauth", "minimax-cn", "anthropic", "deepseek",
...
# Minimum context length required to run EasyBCI Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    "gpt-5": 400000,                  # GPT-5.x base, mini
    "gpt-4.1": 1047576,
    "gpt-4": 128000,
    # Google
    "gemini": 1048576,
    ...
    # so these hardcoded fallbacks prevent EasyBCI from probing-down to
...
try:
    from services.providers import list_providers as _list_providers
...
def _get_context_cache_path() -> Path:
    ...
    return get_easybci_home() / "context_length_cache.yaml"
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # EasyBCI grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    BCI Team uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
def _resolve_bci_team_context_length(
    model: str,
    base_url: str = "",
    api_key: str = "",
) -> Tuple[Optional[int], str]:
    """Resolve BCI Team Portal model context length.

    Tries the live BCI Team inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    BCI Team model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the BCI Team /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, BCI Team path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  BCI Team URLs
    ...
    if provider == "bedrock" or (
        ...
        try:
            from easybci_agent.bedrock_adapter import get_bedrock_context_length
    ...
    if effective_provider:
        from easybci_agent.models_dev import lookup_models_dev_context
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets EasyBCI sends to providers:
