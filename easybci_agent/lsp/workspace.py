...
1. **Workspace gate** — the upper-level "is this directory a project?"
   check.  EasyBCI only runs LSP when the cwd (or the file being edited)
   sits inside a git worktree.  Files outside any git root never
   trigger LSP, even if a server is configured.  This keeps gateway
   users on user-home cwd's from spawning daemons.
