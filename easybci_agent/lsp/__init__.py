"""Language Server Protocol (LSP) integration for EasyBCI Agent.

EasyBCI runs full language servers (pyright, gopls, rust-analyzer,
...
checks.  This keeps users on user-home cwd's (e.g. gateway
...
from easybci_agent.lsp.manager import LSPService
...
def get_service() -> Optional[LSPService]:
    ...
    with _service_lock:
        ...
        if not _atexit_registered:
            ...
            # ``easybci chat`` exit would leak pyright processes that
