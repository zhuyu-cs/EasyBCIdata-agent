...
``easybci_cli/main.py``) so the LSP module ships self-contained.
"""
...
def register_subparser(subparsers: argparse._SubParsersAction) -> None:
    """Wire the ``easybci lsp`` subcommand tree into the main argparse."""
...
def run_lsp_command(args: argparse.Namespace) -> int:
    """Top-level dispatcher for ``easybci lsp <subcommand>``."""
...
def _cmd_status(emit_json: bool) -> int:
    ...
    from easybci_agent.lsp.install import detect_status
...
def _cmd_install(server_id: str) -> int:
    from easybci_agent.lsp.install import try_install, INSTALL_RECIPES, detect_status
...
def _cmd_restart() -> int:
    from easybci_agent.lsp import shutdown_service
...
def _cmd_which(server_id: str) -> int:
    from easybci_agent.lsp.install import INSTALL_RECIPES, easybci_lsp_bin_dir
    import os
    import shutil as _shutil

    recipe = INSTALL_RECIPES.get(server_id)
    bin_name = (recipe or {}).get("bin", server_id)
    staged = easybci_lsp_bin_dir() / bin_name
    if staged.exists():
...
def _backend_warnings() -> list:
    ...
    """
    import shutil as _shutil
    from easybci_agent.lsp.install import easybci_lsp_bin_dir
    notes: list = []
    bash_installed = _shutil.which("bash-language-server") is not None or (
        (easybci_lsp_bin_dir() / "bash-language-server").exists()
    )
