...
from easybci_agent.lsp import eventlog
from easybci_agent.lsp.client import (
    DIAGNOSTICS_DOCUMENT_WAIT,
    LSPClient,
    file_uri,
)
from easybci_agent.lsp.servers import (
    ServerContext,
    ServerDef,
    SpawnSpec,
...
from easybci_agent.lsp.workspace import (
    clear_cache,
    is_inside_workspace,
...
DEFAULT_IDLE_TIMEOUT = 86400  # seconds; 24h — persistent for continuous BCI agent sessions
...
class LSPService:
    ...
    def create_from_config(cls) -> Optional["LSPService"]:
        """Build a service from ``easybci_cli.config`` settings.
    ...
    def enabled_for(self, file_path: str) -> bool:
        ...
        timeout cost — until the service is restarted (``easybci lsp
        ...
        if delta:
            ...
            if baseline:
                if line_shift is not None:
                    ...
                    from easybci_agent.lsp.range_shift import shift_baseline
    ...
    # ------------------------------------------------------------------
    # status / introspection (used by ``easybci lsp status``)
    # ------------------------------------------------------------------
