...
appropriate.  All installs go to a EasyBCI-owned bin staging dir,
``<EASYBCI_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``easybci lsp
  status``.
...
# ``<EASYBCI_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
def easybci_lsp_bin_dir() -> Path:
    """Return the EasyBCI-owned bin staging dir for LSP servers."""
    home = os.environ.get("EASYBCI_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".easybci")
    p = Path(home) / "lsp" / "bin"
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    staged = easybci_lsp_bin_dir() / name
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = easybci_lsp_bin_dir().parent  # <EASYBCI_HOME>/lsp/
    ...
    for c in candidates:
        if c.exists():
            ...
            link = easybci_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a easybci-owned target dir.
    ...
    console script.
    """
    pip_target = easybci_lsp_bin_dir().parent / "python-packages"
    ...
    if bin_path.exists():
        link = easybci_lsp_bin_dir() / bin_name
        if not link.exists():
            try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``easybci lsp status`` CLI to give users a quick
