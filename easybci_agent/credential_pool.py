...
from easybci_cli.config import get_env_value, load_env
import easybci_cli.auth as auth_mod
...
class CredentialPool:
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        try:
            if self.provider == "anthropic":
                from easybci_agent.anthropic_adapter import refresh_anthropic_oauth_pure

                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("easybci_pkce"),
                )
                updated = replace(
                    entry,
                ...
                if entry.source == "claude_code":
                    try:
                        from easybci_agent.anthropic_adapter import _write_claude_code_credentials
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("easybci_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
                    ...
                    return synced
            # For bci_team: another process may have consumed the refresh token
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other EasyBCI profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `easybci auth remove <provider> <N>` is stable across all source types.
    try:
        from easybci_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        from easybci_agent.anthropic_adapter import read_claude_code_credentials, read_easybci_oauth_credentials

        for source_name, creds in (
            ("easybci_pkce", read_easybci_oauth_credentials()),
        ...
        # EasyBCI auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from easybci_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.easybci/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `easybci auth remove minimax-oauth <N>` flow works.
        ...
        try:
            from easybci_cli.auth import get_provider_auth_state
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.easybci/.env over os.environ — the user's config file is the
    # authoritative source for EasyBCI credentials. Stale env vars from parent
    # processes (test scripts, etc.) should not override deliberate
    ...
    # Honour user suppression — `easybci auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.easybci/.env.
    ...
    try:
        from easybci_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
        def _is_source_suppressed(_p, _s):  # type: ignore[misc]
            return False
    if provider == "openrouter":
        # Prefer ~/.easybci/.env over os.environ
...
def _prune_stale_seeded_entries(entries: List[PooledCredential], active_sources: Set[str]) -> bool:
    retained = [
        ...
        or not (
            ...
            or entry.source in {"claude_code", "easybci_pkce"}
        )
    ]
