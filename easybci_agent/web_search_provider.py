...
``kind: backend``) or ``~/.easybci/plugins/web/<name>/`` (user, opt-in via
``plugins.enabled``).
...
class WebSearchProvider(abc.ABC):
    ...
    def name(self) -> str:
        ...
        user-visible names. Examples: ``brave-free``,
        ``searxng``, ``firecrawl``.
        """

    @property
    def display_name(self) -> str:
        """Human-readable label shown in ``easybci tools``. Defaults to ``name``."""
        return self.name
    ...
    def is_available(self) -> bool:
        ...
        runs at tool-registration time and on every ``easybci tools`` paint.
        """
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        """Return provider metadata for the ``easybci tools`` picker.

        Used by ``easybci_cli/tools_config.py`` to inject this provider as a
    ...
    def availability_diagnostic(self) -> tuple[str, str]:
        """Return ``(reason_phrase, fix_hint)`` when ``is_available()`` is False.

        Subclasses override to give the user actionable diagnostics ("set
        TAVILY_API_KEY", "set EXA_API_KEY"). The default — used only
        when the subclass forgot to override — returns a generic placeholder
        rather than raising, so older third-party providers keep working.
        """
