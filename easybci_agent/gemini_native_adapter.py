...
EasyBCI keeps ``api_mode='chat_completions'`` for the ``gemini`` provider so the
...
Why this exists
---------------
Google's OpenAI-compatible endpoint has been brittle for EasyBCI's multi-turn
...
from easybci_agent.gemini_schema import sanitize_gemini_tool_parameters
...
def probe_gemini_tier(
    ...
    *,
    ...
    - ``"free"``    -- key is on the free tier (unusable with EasyBCI)
...
_FREE_TIER_GUIDANCE = (
    ...
    "gemini-2.5-flash). EasyBCI typically makes 3-10 API calls per user turn, "
...
class GeminiAPIError(Exception):
    """Error shape compatible with EasyBCI retry/error classification."""
...
class GeminiNativeClient:
        ...
        **_: Any,
        ...
        if not (api_key or "").strip():
            raise RuntimeError(
                ...
                "Set GOOGLE_API_KEY or GEMINI_API_KEY in your environment / ~/.easybci/.env "
                "(get one at https://aistudio.google.com/app/apikey), or run `easybci setup` "
    ...
    def _headers(self) -> Dict[str, str]:
        headers = {
            ...
            "User-Agent": "easybci-agent (gemini-native)",
        }
