...
# cli.py) or `EASYBCI_REDACT_SECRETS=false` in ~/.easybci/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("EASYBCI_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
_PREFIX_PATTERNS = [
    ...
    r"gAAAA[A-Za-z0-9_=-]{20,}",        # OpenAI Responses API encrypted reasoning blobs
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across EasyBCI — used by
    ``easybci config``, ``easybci status``, ``easybci dump``, and anywhere
